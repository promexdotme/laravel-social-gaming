...
foreach (['dice','wheel','plinko','mines','crash','royal_steps'] as $slug) {
 echo $slug.' settings '.json_encode(['edge'=>settings('cedar_'.$slug.'_house_edge',in_array($slug,['mines','crash','royal_steps'])?3:1),'min'=>settings('cedar_'.$slug.'_min_bet',10),'max'=>settings('cedar_'.$slug.'_max_bet',50000)]).PHP_EOL;
}
...
foreach (['under','over'] as $condition) {
 for($target=($condition==='under'?100:199);$target<=($condition==='under'?9800:9899);$target++) {
  ...
  $mult=floor(99/($wins/100)*10000)/10000;
  $rtp=$wins/10000*$mult;
...
for($mines=1;$mines<=24;$mines++) {
 ...
 for($safe=1;$safe<=25-$mines;$safe++) {
  ...
  $rtp=$p*CedarMath::minesMultiplier($mines,$safe,3);
...
foreach(CedarMath::STEPS as $i=>$mult) echo 'Royal step '.($i+1).' survival '.(floor(9700/$mult)/100).'% RTP '.(floor(9700/$mult)*$mult/100).'%'.PHP_EOL;
foreach([1.01,2,10,100] as $auto) echo "Crash auto $auto RTP ".(97*$auto/($auto+0.01)).'% (strictly before rounded crash)'.PHP_EOL;
echo 'Cap example: 50000 wager at Royal step 10 pays 1000000, effective RTP '.(0.0097*1000000/50000*100).'%'.PHP_EOL;
