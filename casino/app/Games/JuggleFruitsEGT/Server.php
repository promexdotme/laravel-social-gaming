                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[10],"multiplier":1},"1":{"coef":[10],"multiplier":1},"2":{"coef":[20],"multiplier":1},"3":{"coef":[20],"multiplier":1},"4":{"coef":[40],"multiplier":1},"5":{"coef":[40],"multiplier":1},"6":{"coef":[80],"multiplier":1},"7":{"coef":[80],"multiplier":1},"8":{"coef":[4000],"multiplier":1},"9":{"coef":[2],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[6,1,1,1,6,4,4,4,1,9,1,2,2,2,0,7,0,5,2,8,2,5,0,2,2,2,5,1,1,1],[7,6,5,0,7,2,9,2,3,3,3,7,5,1,1,1,4,4,4,2,2,2,0,8,6,0,2,2,2,0],[4,4,4,2,2,2,0,7,5,6,9,4,4,0,6,0,3,8,3,5,1,1,1,5,7,2,2,2,5,0],[2,2,2,0,4,4,4,7,9,5,7,0,5,2,2,2,5,0,5,0,1,1,1,0,8,3,3,3,7,0],[2,2,2,3,3,3,4,4,4,0,7,5,0,9,0,1,1,1,8,1,1,1,2,2,2,6,3,3,3,0]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('JuggleFruitsEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('JuggleFruitsEGTBonusWin', 0);
                                $slotSettings->SetGameData('JuggleFruitsEGTFreeGames', 0);
                                $slotSettings->SetGameData('JuggleFruitsEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('JuggleFruitsEGTTotalWin', 0);
                                $slotSettings->SetGameData('JuggleFruitsEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('JuggleFruitsEGTCurrentFreeGame') < $slotSettings->GetGameData('JuggleFruitsEGTFreeGames') && $slotSettings->GetGameData('JuggleFruitsEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('JuggleFruitsEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('JuggleFruitsEGTTotalWin', 0);
                                    $slotSettings->SetGameData('JuggleFruitsEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('JuggleFruitsEGTCurrentFreeGame', $slotSettings->GetGameData('JuggleFruitsEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('JuggleFruitsEGTBonusWin', $slotSettings->GetGameData('JuggleFruitsEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('JuggleFruitsEGTTotalWin', $slotSettings->GetGameData('JuggleFruitsEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('JuggleFruitsEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('JuggleFruitsEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('JuggleFruitsEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('JuggleFruitsEGTFreeGames', $slotSettings->GetGameData('JuggleFruitsEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('JuggleFruitsEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('JuggleFruitsEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('JuggleFruitsEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('JuggleFruitsEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('JuggleFruitsEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('JuggleFruitsEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('JuggleFruitsEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('JuggleFruitsEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('JuggleFruitsEGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(0, 6));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('JuggleFruitsEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('JuggleFruitsEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('JuggleFruitsEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('JuggleFruitsEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('JuggleFruitsEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('JuggleFruitsEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('JuggleFruitsEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('JuggleFruitsEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('JuggleFruitsEGTGambleStep', $slotSettings->GetGameData('JuggleFruitsEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('JuggleFruitsEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
