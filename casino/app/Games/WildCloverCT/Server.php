            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'create_session':
                                $slotSettings->SetGameData('WildCloverCTCards', [
                                    19, 
                                    44, 
                                    0, 
                                    37, 
                            ...
                            case 'init_game':
                                ...
                                $slotSettings->SetGameData('WildCloverCTBonusWin', 0);
                                $slotSettings->SetGameData('WildCloverCTFreeGames', 0);
                                $slotSettings->SetGameData('WildCloverCTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('WildCloverCTCurrentFreeGame0', 0);
                                $slotSettings->SetGameData('WildCloverCTTotalWin', 0);
                                $slotSettings->SetGameData('WildCloverCTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    if( $slotSettings->GetGameData('WildCloverCTCurrentFreeGame') < $slotSettings->GetGameData('WildCloverCTFreeGames') && $slotSettings->GetGameData('WildCloverCTFreeGames') > 0 ) 
                                    {
                            ...
                            case 'poll':
                                ...
                                {
                                    $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance() - $slotSettings->GetGameData('WildCloverCTTotalWin')) * 100;
                                }
                            ...
                            case 'game_finished':
                                $slotSettings->SetGameData('WildCloverCTBonusWin', 0);
                                $slotSettings->SetGameData('WildCloverCTTotalWin', 0);
                                $responseData['game_finished'][0]['freeround_play'] = $slotSettings->GetGameData('WildCloverCTCurrentFreeGame0');
                            ...
                            case 'subgame_finished':
                                if( $slotSettings->GetGameData('WildCloverCTCurrentFreeGame0') < $slotSettings->GetGameData('WildCloverCTFreeGames') && $slotSettings->GetGameData('WildCloverCTFreeGames') > 0 ) 
                                {
                                    ...
                                    $response = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"result":' . json_encode($result) . ',"slotDenom":' . $slotSettings->GetGameData($slotSettings->slotId . 'Denom') . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('WildCloverCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('WildCloverCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData('WildCloverCTFreeBalance') . ',"afterBalance":' . $slotSettings->GetGameData('WildCloverCTFreeBalance') . ',"bonusWin":' . $slotSettings->GetGameData('WildCloverCTBonusWin') . ',"totalWin":' . $reportWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Reels')) . '}}';
                                    if( $slotSettings->GetGameData('WildCloverCTFreeGames') <= $slotSettings->GetGameData('WildCloverCTCurrentFreeGame0') ) 
                                    {
                                        if( $reportWin > 0 ) 
                                        {
                                    ...
                                    $balanceInCents = $slotSettings->GetGameData('WildCloverCTFreeBalance');
                                    $responseData['subgame_finished'][0]['account_status']['credit'] = round($balanceInCents);
                                    $responseData['subgame_finished'][0]['freeround_play'] = $slotSettings->GetGameData('WildCloverCTCurrentFreeGame0');
                                    $responseData['subgame_finished'][1]['freeround_play'] = $slotSettings->GetGameData('WildCloverCTCurrentFreeGame0');
                            ...
                            case 'play':
                                ...
                                for( $spinLoop = 0; $spinLoop < $spinLoopLimit; $spinLoop++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] != 'freespin' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('WildCloverCTCurrentFreeGame0', 0);
                                        $slotSettings->SetGameData('WildCloverCTTotalWin', 0);
                                        $slotSettings->SetGameData('WildCloverCTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    ...
                                    {
                                        $slotSettings->SetGameData('WildCloverCTCurrentFreeGame', $slotSettings->GetGameData('WildCloverCTCurrentFreeGame') + 1);
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $slotSettings->SetGameData('WildCloverCTBonusWin', $slotSettings->GetGameData('WildCloverCTBonusWin') + $totalWin);
                                        $slotSettings->SetGameData('WildCloverCTTotalWin', $slotSettings->GetGameData('WildCloverCTTotalWin') + $totalWin);
                                        $balanceInCents = $slotSettings->GetGameData('WildCloverCTFreeBalance');
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('WildCloverCTTotalWin', $totalWin);
                                    }
                                    $fs = 0;
                                    if( $scattersCount >= 3 ) 
                                    {
                                    ...
                                    $winstring = '';
                                    $slotSettings->SetGameData('WildCloverCTGambleStep', 5);
                                    $hist = $slotSettings->GetGameData('WildCloverCTCards');
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $state = 'freespin';
                                        ...
                                        if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('WildCloverCTBonusWin') > 0 ) 
                                        {
                                            $responseData['play'][0]['result'][$spinLoop]['is_free'] = true;
                                    ...
                                    $responseData['play'][0]['result'][$spinLoop]['total_win'] = $slotSettings->GetGameData('WildCloverCTTotalWin') / $denom;
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"result":' . json_encode($responseData['play'][0]['result']) . ',"slotDenom":' . $denom . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('WildCloverCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('WildCloverCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('WildCloverCTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'gamble':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('WildCloverCTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                ...
                                $hist = $slotSettings->GetGameData('WildCloverCTCards');
                                $slotSettings->SetGameData('WildCloverCTGambleStep', $slotSettings->GetGameData('WildCloverCTGambleStep') - 1);
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isGambleWin = 0;
                                }
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('WildCloverCTGambleStep', 0);
                                    if( $postData['continue_params']['double_key'] != 'red' && $postData['continue_params']['double_key'] != 'black' ) 
                                    {
                                        if( $postData['continue_params']['double_key'] == 'diamond' ) 
                                        {
                                ...
                                $slotSettings->SetGameData('WildCloverCTCards', $hist);
