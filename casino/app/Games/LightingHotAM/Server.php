                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', 0);
                                $slotSettings->SetGameData($slotSettings->slotId . 'FreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData($slotSettings->slotId . 'FreeGames'));
                                $cFree = dechex($slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') - $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') * $floatBet);
                            ...
                            {
                                $slotSettings->SetGameData($slotSettings->slotId . 'Lines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData($slotSettings->slotId . 'Lines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            break;
                        case 'A/u2531':
                            $newWin = $slotSettings->GetGameData($slotSettings->slotId . 'TotalWin');
                            $oldWin = $slotSettings->GetGameData($slotSettings->slotId . 'TotalWin');
                            $balanceFormated = $slotSettings->GetGameData($slotSettings->slotId . 'balanceFormated');
                            $bank = $slotSettings->GetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''));
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                $reelsOld = $slotSettings->GetGameData($slotSettings->slotId . 'Reels');
                                $reelsNew = $slotSettings->GetReelStrips('', 'bet');
                                $mpl = 0;
                                for( $r = 1; $r <= 5; $r++ ) 
                                {
                                    for( $p = 0; $p <= 2; $p++ ) 
                                    {
                                        if( $reelsOld['reel' . $r][$p] == $reelsNew['reel' . $r][$p] ) 
                                        {
                                            $mpl++;
                                        }
                                    }
                            ...
                            $reelSate = $slotSettings->HexFormat($reelsNew['rp'][0]) . $slotSettings->HexFormat($reelsNew['rp'][1]) . $slotSettings->HexFormat($reelsNew['rp'][2]) . $slotSettings->HexFormat($reelsNew['rp'][3]) . $slotSettings->HexFormat($reelsNew['rp'][4]);
                            $newWin0 = $newWin * $mpl;
                            $newWin1 = $newWin0 - $newWin;
                            if( $mpl > 1 ) 
                            {
                                $slotSettings->SetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''), -1 * $newWin1);
                                $slotSettings->SetBalance($newWin1);
                                $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', $newWin0);
                                $newWin = $newWin0;
                            }
                            $response = '120010' . $balanceFormated . $slotSettings->HexFormat($newWin0 * 100) . $reelSate . $slotSettings->GetGameData($slotSettings->slotId . 'BetStr') . '1010101' . dechex($mpl) . '101010101010100b' . $slotSettings->GetGameData($slotSettings->slotId . 'winLinesFormated') . '0000000000000000' . $slotSettings->GetGameData($slotSettings->slotId . 'reelSate') . $slotSettings->GetGameData($slotSettings->slotId . 'LockedReels');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . $slotSettings->HexFormat($newWin0 * 100) . $reelSate . $slotSettings->GetGameData($slotSettings->slotId . 'BetStr') . '10101010101010101010100b' . $slotSettings->GetGameData($slotSettings->slotId . 'winLinesFormated') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards')) . '#101010';
                            $slotSettings->SetGameData($slotSettings->slotId . 'Collect', $response_collect);
                            $response_log = '{"responseEvent":"gambleResult","serverResponse":{"totalWin":' . $newWin1 . '}}';
                            $slotSettings->SaveLogReport($response_log, 0, 0, $newWin1, 'MPL');
                            break;
                        case 'A/u251':
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') + $totalWin);
                            }
                            ...
                            if( $totalWin > 0 ) 
                            {
                                $response = '11e010' . $balanceFormated . $slotSettings->HexFormat(round($totalWin * 100)) . $reelSate . $bet_h . $ln_h . '10101011101010101010100b' . $winLinesFormated . '0000000000000000' . $reelSate . '13' . implode('', $reels['reel1']) . '13' . implode('', $reels['reel2']) . '13' . implode('', $reels['reel3']) . '13' . implode('', $reels['reel4']) . '13' . implode('', $reels['reel5']) . '';
                                $slotSettings->SetGameData($slotSettings->slotId . 'reelSate', $reelSate);
                                $slotSettings->SetGameData($slotSettings->slotId . 'Reels', $reels);
                                $slotSettings->SetGameData($slotSettings->slotId . 'BetStr', $bet_h . $ln_h);
                                $slotSettings->SetGameData($slotSettings->slotId . 'LockedReels', '13' . implode('', $reels['reel1']) . '13' . implode('', $reels['reel2']) . '13' . implode('', $reels['reel3']) . '13' . implode('', $reels['reel4']) . '13' . implode('', $reels['reel5']));
                                $slotSettings->SetGameData($slotSettings->slotId . 'winLinesFormated', $winLinesFormated);
                                $slotSettings->SetGameData($slotSettings->slotId . 'balanceFormated', $balanceFormated);
                            }
                            else
                            {
                            ...
                            $slotSettings->SetGameData($slotSettings->slotId . 'DoubleAnswer', $reelSate . $bet_h . $ln_h . '1010101010101010101010' . $fixedLinesFormated0 . $winLinesFormated);
                            $slotSettings->SetGameData($slotSettings->slotId . 'DoubleBalance', $balanceFormated);
                            ...
                            $slotSettings->SetGameData($slotSettings->slotId . 'CollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                        ...
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData($slotSettings->slotId . 'Collect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData($slotSettings->slotId . 'DoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData($slotSettings->slotId . 'DoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData($slotSettings->slotId . 'DoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData($slotSettings->slotId . 'DoubleWin', $winall);
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData($slotSettings->slotId . 'CollectP0');
                            $slotSettings->SetGameData($slotSettings->slotId . 'Collect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData($slotSettings->slotId . 'DoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData($slotSettings->slotId . 'DoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData($slotSettings->slotId . 'DoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
