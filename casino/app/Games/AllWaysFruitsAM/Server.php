                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('AllWaysFruitsAMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('AllWaysFruitsAMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('AllWaysFruitsAMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('AllWaysFruitsAMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('AllWaysFruitsAMTotalWin', 0);
                                $slotSettings->SetGameData('AllWaysFruitsAMFreeBalance', 0);
                                $slotSettings->SetGameData('AllWaysFruitsAMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('AllWaysFruitsAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('AllWaysFruitsAMFreeGames') - $slotSettings->GetGameData('AllWaysFruitsAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('AllWaysFruitsAMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('AllWaysFruitsAMCurrentFreeGame') < $slotSettings->GetGameData('AllWaysFruitsAMFreeGames') && $slotSettings->GetGameData('AllWaysFruitsAMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('AllWaysFruitsAMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('AllWaysFruitsAMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('AllWaysFruitsAMBonusWin', 0);
                                $slotSettings->SetGameData('AllWaysFruitsAMFreeGames', 0);
                                $slotSettings->SetGameData('AllWaysFruitsAMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('AllWaysFruitsAMTotalWin', 0);
                                $slotSettings->SetGameData('AllWaysFruitsAMFreeBalance', 0);
                                $slotSettings->SetGameData('AllWaysFruitsAMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('AllWaysFruitsAMCurrentFreeGame', $slotSettings->GetGameData('AllWaysFruitsAMCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('AllWaysFruitsAMFreeGames') <= $slotSettings->GetGameData('AllWaysFruitsAMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('AllWaysFruitsAMBonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('AllWaysFruitsAMBonusWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('AllWaysFruitsAMBonusWin', $slotSettings->GetGameData('AllWaysFruitsAMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('AllWaysFruitsAMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('AllWaysFruitsAMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('AllWaysFruitsAMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('AllWaysFruitsAMFreeGames', $slotSettings->GetGameData('AllWaysFruitsAMFreeGames') + $slotSettings->slotFreeCount[$scattersCount]);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AllWaysFruitsAMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('AllWaysFruitsAMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('AllWaysFruitsAMFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"symDouble":' . $symDouble . ',"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('AllWaysFruitsAMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AllWaysFruitsAMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('AllWaysFruitsAMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('AllWaysFruitsAMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('AllWaysFruitsAMFreeGames') - $slotSettings->GetGameData('AllWaysFruitsAMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('AllWaysFruitsAMFreeGames') <= $slotSettings->GetGameData('AllWaysFruitsAMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('AllWaysFruitsAMBonusWin');
                                $bonusMpl = $slotSettings->slotFreeMpl;
                            }
                            else
                            {
                            ...
                            $slotSettings->SetGameData('AllWaysFruitsAMDoubleAnswer', $reelSate . $bet_h . $ln_h . $freeInfo . $freeWinState . $slotSettings->HexFormat($bonusMpl) . '1010' . $reelSate . '0' . $symCountAll . $winLinesFormated);
                            $slotSettings->SetGameData('AllWaysFruitsAMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('AllWaysFruitsAMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('AllWaysFruitsAMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('AllWaysFruitsAMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('AllWaysFruitsAMCollect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('AllWaysFruitsAMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('AllWaysFruitsAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('AllWaysFruitsAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('AllWaysFruitsAMDoubleWin', $winall);
                            $slotSettings->SetGameData('AllWaysFruitsAMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('AllWaysFruitsAMCollectP0');
                            $slotSettings->SetGameData('AllWaysFruitsAMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('AllWaysFruitsAMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('AllWaysFruitsAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('AllWaysFruitsAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
