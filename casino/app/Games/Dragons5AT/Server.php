            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            $response = '{"status":"success","microtime":0.0026731491088867,"dateTime":"2020-02-13 13:23:32","error":"","content":{"cmd":"sessionInfo","serverMathematics":"\/game\/Dragons5AT\/server","serverResources":"","sessionId":"54594109_4133a64673d3883d42cd003ee905ba3e","exitUlt":"\/","exitUrl":"\/","id":"341","name":"GameName","currency":"ALL","language":"en","type":"aristocrat","systemName":"5_dragons","version":"2","mobile":"1"}}';
                        }
                        ...
                        {
                            $balanceInCents = round(sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                            if( $_POST['pick'] == 'yellow' ) 
                            {
                                $slotSettings->SetGameData('Dragons5ATMpl', 15);
                                $slotSettings->SetGameData('Dragons5ATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Dragons5ATFreeGames', 5);
                            }
                            else if( $_POST['pick'] == 'blue' ) 
                            {
                                $_POST['pick'] = 1;
                                $slotSettings->SetGameData('Dragons5ATMpl', 10);
                                $slotSettings->SetGameData('Dragons5ATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Dragons5ATFreeGames', 8);
                            }
                            else if( $_POST['pick'] == 'black' ) 
                            {
                                $slotSettings->SetGameData('Dragons5ATMpl', 5);
                                $slotSettings->SetGameData('Dragons5ATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Dragons5ATFreeGames', 10);
                            }
                            else if( $_POST['pick'] == 'red' ) 
                            {
                                $slotSettings->SetGameData('Dragons5ATMpl', 3);
                                $slotSettings->SetGameData('Dragons5ATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Dragons5ATFreeGames', 15);
                            }
                            else if( $_POST['pick'] == 'white' ) 
                            {
                                $slotSettings->SetGameData('Dragons5ATMpl', 2);
                                $slotSettings->SetGameData('Dragons5ATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Dragons5ATFreeGames', 20);
                            }
                            $slotSettings->SetGameData('Dragons5ATPick', $_POST['pick']);
                            $response = '{"status":"success","microtime":0.0093810558319092,"dateTime":"2020-02-15 14:26:21","error":"","content":{"session":"54990161_9709e13bca7763b46396cda1868d2e7e","cmd":"gamePick","balance":' . $balanceInCents . ',"win":0,"symbols":{"1":{"1":"symbol_6","2":"symbol_9","3":"symbol_13"},"2":{"1":"symbol_10","2":"symbol_5","3":"symbol_8"},"3":{"1":"symbol_10","2":"symbol_13","3":"symbol_11"},"4":{"1":"symbol_13","2":"symbol_10","3":"symbol_6"},"5":{"1":"symbol_3","2":"symbol_8","3":"symbol_1"}},"winLines":[],"freeSpin":{"id":"54990161_20200215132548","current":0,"add":' . $slotSettings->GetGameData('Dragons5ATFreeGames') . ',"total":' . $slotSettings->GetGameData('Dragons5ATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('Dragons5ATBonusWin') * 100) . ',"multiplayer":' . $slotSettings->GetGameData('Dragons5ATMpl') . ',"pick":"' . $_POST['pick'] . '"},"actionId":"54990161_10_60","hash":"1ace145628d930d28bdf38789a28a9d7"}}';
                        }
                        else if( $postData['slotEvent'] == 'slotGamble' ) 
                        {
                            ...
                            $dealerCard = '';
                            $totalWin = $slotSettings->GetGameData('Dragons5ATTotalWin');
                            $gambleWin = 0;
                            $statBet = $totalWin;
                            ...
                            $slotSettings->SetGameData('Dragons5ATTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('Dragons5ATBonusWin', 0);
                                $slotSettings->SetGameData('Dragons5ATFreeGames', 0);
                                $slotSettings->SetGameData('Dragons5ATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Dragons5ATTotalWin', 0);
                                $slotSettings->SetGameData('Dragons5ATFreeBalance', 0);
                                $slotSettings->SetGameData('Dragons5ATMpl', 1);
                            }
                            else
                            {
                                $slotSettings->SetGameData('Dragons5ATCurrentFreeGame', $slotSettings->GetGameData('Dragons5ATCurrentFreeGame') + 1);
                                $bonusMpl = $slotSettings->GetGameData('Dragons5ATMpl');
                            }
                            $Balance = $slotSettings->GetBalance();
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('Dragons5ATBonusWin', $slotSettings->GetGameData('Dragons5ATBonusWin') + $totalWin);
                                $slotSettings->SetGameData('Dragons5ATTotalWin', $slotSettings->GetGameData('Dragons5ATTotalWin') + $totalWin);
                                $freeStateStr = '"id":"54594109_20200214110301","bagCount":0,"gameType":"extra","pick":"' . $slotSettings->GetGameData('Dragons5ATPick') . '","current":' . $slotSettings->GetGameData('Dragons5ATCurrentFreeGame') . ',"xWin":' . $slotSettings->GetGameData('Dragons5ATMpl') . ',"add":0,"total":' . $slotSettings->GetGameData('Dragons5ATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('Dragons5ATBonusWin') * 100) . '';
                            }
                            else
                            {
                                $slotSettings->SetGameData('Dragons5ATTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('Dragons5ATFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('Dragons5ATFreeGames', $slotSettings->GetGameData('Dragons5ATFreeGames') + $slotSettings->slotFreeCount[$scattersCount]);
                                    $freeStateStr = '"id":"54594109_20200214110301","bagCount":0,"gameType":"extra","current":' . $slotSettings->GetGameData('Dragons5ATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('Dragons5ATMpl') . ',"add":' . $slotSettings->slotFreeCount[$scattersCount] . ',"total":' . $slotSettings->GetGameData('Dragons5ATFreeGames') . ',"xWin":[' . implode(',', $wildsMplArr) . '],"totalWin":' . ($slotSettings->GetGameData('Dragons5ATBonusWin') * 100) . '';
                                }
                                else
                                {
                                    $slotSettings->SetGameData('Dragons5ATFreeBalance', $Balance);
                                    $slotSettings->SetGameData('Dragons5ATBonusWin', $totalWin);
                                    $slotSettings->SetGameData('Dragons5ATFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                    $freeStateStr = '"id":"54594109_20200214110301","bagCount":0,"gameType":"extra","current":' . $slotSettings->GetGameData('Dragons5ATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('Dragons5ATMpl') . ',"add":' . $slotSettings->slotFreeCount[$scattersCount] . ',"total":' . $slotSettings->GetGameData('Dragons5ATFreeGames') . ',"xWin":[' . implode(',', $wildsMplArr) . '],"totalWin":' . ($slotSettings->GetGameData('Dragons5ATBonusWin') * 100) . '';
                                }
                            }
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('Dragons5ATFreeGames') . ',"Mpl":' . $slotSettings->GetGameData('Dragons5ATMpl') . ',"currentFreeGames":' . $slotSettings->GetGameData('Dragons5ATCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":{},"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            $reelStrArr = [];
                            for( $i = 1; $i <= 5; $i++ ) 
                            {
