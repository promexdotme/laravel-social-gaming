                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'SELGAME':
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('LadysKissDeluxeVSCards', $hist);
                                ...
                                $slotSettings->SetGameData('LadysKissDeluxeVSBonusWin', 0);
                                $slotSettings->SetGameData('LadysKissDeluxeVSFreeGames', 0);
                                $slotSettings->SetGameData('LadysKissDeluxeVSCurrentFreeGame', 0);
                                $slotSettings->SetGameData('LadysKissDeluxeVSTotalWin', 0);
                                $slotSettings->SetGameData('LadysKissDeluxeVSFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('LadysKissDeluxeVSCurrentFreeGame') < $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') && $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'GA_SPIN':
                                ...
                                if( $slotSettings->GetGameData('LadysKissDeluxeVSCurrentFreeGame') < $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') && $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') > 0 ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('LadysKissDeluxeVSCurrentFreeGame', $slotSettings->GetGameData('LadysKissDeluxeVSCurrentFreeGame') + 1);
                                }
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('LadysKissDeluxeVSCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('LadysKissDeluxeVSBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                ...
                                $hist = $slotSettings->GetGameData('LadysKissDeluxeVSCards');
                                $gtypeaft = 1;
                                $slotSettings->SetGameData('LadysKissDeluxeVSBonusStart', 0);
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('LadysKissDeluxeVSFreeGames', $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('LadysKissDeluxeVSBonusWin', 0);
                                        $slotSettings->SetGameData('LadysKissDeluxeVSBonusStart', 1);
                                        $slotSettings->SetGameData('LadysKissDeluxeVSFreeGames', $slotSettings->slotFreeCount);
                                        $gtypeaft = 2;
                                    }
                                }
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('LadysKissDeluxeVSTotalWin', $totalWin);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('LadysKissDeluxeVSTotalWin', $totalWin);
                            ...
                            case 'GA_TAKE':
                                ...
                                {
                                    $gtype = 2;
                                    $slotSettings->SetGameData('LadysKissDeluxeVSBonusWin', $slotSettings->GetGameData('LadysKissDeluxeVSBonusWin') + $slotSettings->GetGameData('LadysKissDeluxeVSTotalWin'));
                                }
                                $slotSettings->SetGameData('LadysKissDeluxeVSTotalWin', 0);
                                $result_tmp[] = '{"status":"OK","authtoken":"1/90317550919705477001","for":"GA_TAKE","totsg":' . $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') . ',"plsg":' . $slotSettings->GetGameData('LadysKissDeluxeVSCurrentFreeGame') . ',"inscatwin":' . ($slotSettings->GetGameData('LadysKissDeluxeVSBonusWin') * 100) . ',"gtype":' . $gtype . ',"credit":{"tot":' . $balanceInCents . '}}';
                                break;
                            case 'QUITGAME':
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            case 'GA_GAMBLE':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('LadysKissDeluxeVSTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                ...
                                if( $slotSettings->GetGameData('LadysKissDeluxeVSBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"status":"OK","gtypeaft":' . $gtype . ',"gtype":' . $gtype . ',"totsg":' . $slotSettings->GetGameData('LadysKissDeluxeVSFreeGames') . ',"plsg":' . $slotSettings->GetGameData('LadysKissDeluxeVSCurrentFreeGame') . ',"inscatwin":' . ($slotSettings->GetGameData('LadysKissDeluxeVSBonusWin') * 100) . ',"authtoken":"1/90317550923628407358","for":"GA_GAMBLE","rno":29444,"state":3,"gmblwinf":2,"card":' . $dealerCard . ',"halfs":0,"snd":"' . $sndID . '","gmblamount":' . ($totalWin * 100) . ',"gmblmayhalf":2,"gmblhalfmin":100,"takesnd":"slidein","gmblhist":[' . implode(',', $hist) . '],"win":' . ($totalWin * 100) . ',"sgwin":0,"credit":{"tot":40885}}';
                                break;
                        }
