            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['action'] == 'risk' && $slotSettings->GetGameData('FruitCocktailIGWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['action'] ) 
                        {
                            case 'init':
                                ...
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $bet . ',"LastFuse":false,"LastLines":' . $lines . ',"MaxWin":675000,"RawCredit":1000,"Reels":[' . $reelStr . '],"Win":0,"slotViewState":"' . $slotSettings->slotViewState . '","slotKeyConfig":' . $slotSettings->slotKeyConfig . ',"args":{},"cmd":"init","time":"20181111T111512"}';
                                break;
                            case 'status':
                                if( !$slotSettings->HasGameData('FruitCocktailIGLines') || !$slotSettings->HasGameData('FruitCocktailIGBet') ) 
                                {
                                    $slotSettings->SetGameData('FruitCocktailIGLines', 1);
                                    $slotSettings->SetGameData('FruitCocktailIGBet', 1);
                                }
                                $balance = $slotSettings->GetBalance();
                                if( $slotSettings->GetGameData('FruitCocktailIGWin') > 0 ) 
                                {
                                    $balance = $balance - $slotSettings->GetGameData('FruitCocktailIGWin');
                                }
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $slotSettings->GetGameData('FruitCocktailIGBet') . ',"LastFuse":false,"LastLines":' . $slotSettings->GetGameData('FruitCocktailIGLines') . ',"MaxWin":675000,"RawCredit":"' . $balance . '","Reels":[[7,1,5,3,8],[5,3,6,7,7],[6,8,7,8,1]],"Win":0,"args":{},"cmd":"status","time":"20181111T111512"}';
                                break;
                            case 'start':
                                $balance = $slotSettings->GetBalance();
                                $bet = $postData['bet'];
                                ...
                                for( $i = 0; $i <= 1500; $i++ ) 
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $isBonus ) 
                                            {
                                                $bTmp = $slotSettings->Bonus($bet * $lines, $fuseBet, $bonusSym);
                                                $totalWin += $bTmp['win'];
                                                $infoBonus = $bTmp['info'];
                                            }
                                            else
                                ...
                                $slotSettings->SetGameData('FruitCocktailIGReels', $reelStr);
                                $slotSettings->SetGameData('FruitCocktailIGWin', $totalWin);
                                $slotSettings->SetGameData('FruitCocktailIGRisk', 1);
                                $slotSettings->SetGameData('FruitCocktailIGLines', $lines);
                                $slotSettings->SetGameData('FruitCocktailIGBet', $bet);
                                $slotSettings->SetGameData('FruitCocktailIGDealerCard', $card);
                                $rState = 'bet';
                                if( $isBonus ) 
                                {
                                    $rState = 'BG';
                            ...
                            case 'finish':
                                $balance = $slotSettings->GetBalance();
                                $slotSettings->SetGameData('FruitCocktailIGWin', 0);
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Credit":' . floor($balance) . ',"RawCredit":' . floor($balance) . ',"Reels":[' . $slotSettings->GetGameData('FruitCocktailIGReels') . '],"args":{},"cmd":"finish","time":"20181111T171830"}';
                                break;
                            case 'risk':
                                $onTabCards = [];
