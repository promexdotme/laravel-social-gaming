            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('SafariHeatDealerCard');
                        $totalWin = $slotSettings->GetGameData('SafariHeatTotalWin');
                        $gambleWin = 0;
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('SafariHeatDealerCard', $tmpDc);
                    ...
                    else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                    {
                        ...
                        if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('SafariHeatFreeGames') <= $slotSettings->GetGameData('SafariHeatCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('SafariHeatTotalWin') + $totalWin > 0 ) 
                        {
                            $slotSettings->SetBalance($slotSettings->GetGameData('SafariHeatTotalWin') + $totalWin);
                        }
                        ...
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            $slotSettings->SetGameData('SafariHeatTotalWin', $slotSettings->GetGameData('SafariHeatTotalWin') + $totalWin);
                            $totalWin = $slotSettings->GetGameData('SafariHeatBonusWin');
                            $Balance = $slotSettings->GetGameData('SafariHeatFreeBalance');
                        }
                        else
                        {
                            $slotSettings->SetGameData('SafariHeatTotalWin', $totalWin);
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('SafariHeatFreeBalance', $Balance);
                        ...
                        $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('SafariHeatFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SafariHeatCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
