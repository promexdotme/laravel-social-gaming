            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            $fsNum = $slotSettings->GetGameData('SinbadsGoldenVoyagePTMCurrentFreeGame');
                            $spins_ = $slotSettings->GetGameData('SinbadsGoldenVoyagePTMFreeSpins');
                            $logs_ = $slotSettings->GetGameData('SinbadsGoldenVoyagePTMFreeLogs');
                        ...
                        if( isset($postData['ID']) && ($postData['ID'] == '46583' || $postData['ID'] == '46302') ) 
                        {
                            ...
                            {
                                $postData['bet'] = $slotSettings->GetGameData('SinbadsGoldenVoyagePTMBet');
                            ...
                            else
                            {
                                ...
                                $lines = (int)$lines_[1];
                                $slotSettings->SetGameData('SinbadsGoldenVoyagePTMBet', $postData['bet']);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    for( $fp = 0; $fp < 7; $fp++ ) 
                                    {
                                        ...
                                        $responseLog[] = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"bonusMpl":1,"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":7,"currentFreeGames":' . ($fp + 1) . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('SinbadsGoldenVoyagePTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('SinbadsGoldenVoyagePTMFreeStartWin') . ',"totalWin":' . $currentWin . ',"winLines":[],"bonusInfo":"","Jackpots":"","reelsSymbols":' . $jsSpin . '}}';
                                    }
                                    if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                    {
                                    }
                                ...
                                $slotSettings->SetGameData('SinbadsGoldenVoyagePTMFreeSpins', $fspins0);
                                $slotSettings->SetGameData('SinbadsGoldenVoyagePTMFreeLogs', $responseLog);
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            $result_tmp[] = '3:::{"data":{"typeBalance":0,"currency":"' . $slotSettings->slotCurrency . '","balanceInCents":' . $balanceInCents . ',"deltaBalanceInCents":1},"ID":40085}';
                            if( $scattersCount >= 3 ) 
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '46090':
                                ...
                                {
                                    $fsReel = $slotSettings->GetGameData('SinbadsGoldenVoyagePTMFreeSpins');
