                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,20,80],"multiplier":1},"1":{"coef":[5,20,80],"multiplier":1},"2":{"coef":[5,25,100],"multiplier":1},"3":{"coef":[5,25,100],"multiplier":1},"4":{"coef":[15,30,150],"multiplier":1},"5":{"coef":[20,40,200],"multiplier":1},"6":{"coef":[25,50,250],"multiplier":1},"7":{"coef":[25,100,500],"multiplier":1},"8":{"coef":[30,200,1000],"multiplier":1},"10":{"coef":[5],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"freespinFakeReels":[[4,4,5,10,0,2,8,8,7,0,10,5,6,6,3,3,10,1,1,2],[9,9,4,7,7,0,5,10,3,1,8,9,2,6,10,6,6,5,8,8],[7,7,10,6,1,5,5,10,2,6,9,9,0,2,9,3,3,8,8,4],[5,5,3,2,9,9,6,3,3,9,4,0,7,7,4,8,1,1,4,4],[8,4,2,2,6,6,9,8,4,3,3,5,7,1,9,9,8,8,4,0]],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[1,1,5,10,2,2,8,8,7,0,10,5,4,4,3,3,10,6,6,6],[8,2,2,10,1,7,7,3,10,4,1,9,9,9,0,3,10,5,6,1],[7,7,9,8,1,1,4,10,2,6,9,9,6,2,3,3,10,5,5,0],[5,5,3,3,4,9,0,9,2,2,4,1,1,7,4,2,8,3,6,6],[8,4,2,3,0,6,2,8,4,6,6,5,7,7,9,9,8,3,1,1]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('SecretOfAlchemyEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('SecretOfAlchemyEGTBonusWin', 0);
                                $slotSettings->SetGameData('SecretOfAlchemyEGTFreeGames', 0);
                                $slotSettings->SetGameData('SecretOfAlchemyEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SecretOfAlchemyEGTTotalWin', 0);
                                $slotSettings->SetGameData('SecretOfAlchemyEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('SecretOfAlchemyEGTCurrentFreeGame') < $slotSettings->GetGameData('SecretOfAlchemyEGTFreeGames') && $slotSettings->GetGameData('SecretOfAlchemyEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTTotalWin', 0);
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTCurrentFreeGame', $slotSettings->GetGameData('SecretOfAlchemyEGTCurrentFreeGame') + 1);
                                ...
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                                if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') > 0 ) 
                                {
                                    $winType = 'none';
                                }
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTBonusWin', $slotSettings->GetGameData('SecretOfAlchemyEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTTotalWin', $slotSettings->GetGameData('SecretOfAlchemyEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('SecretOfAlchemyEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('SecretOfAlchemyEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('SecretOfAlchemyEGTFreeGames', $slotSettings->GetGameData('SecretOfAlchemyEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('SecretOfAlchemyEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('SecretOfAlchemyEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('SecretOfAlchemyEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('SecretOfAlchemyEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('SecretOfAlchemyEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SecretOfAlchemyEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('SecretOfAlchemyEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('SecretOfAlchemyEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('SecretOfAlchemyEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('SecretOfAlchemyEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"cascades":[],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"expand":[],"specialExpand":[],"gambles":5,"freespinScatters":[10],"freespins":' . $fs . ',"jackpot":' . $isJack . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('SecretOfAlchemyEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('SecretOfAlchemyEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('SecretOfAlchemyEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('SecretOfAlchemyEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('SecretOfAlchemyEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('SecretOfAlchemyEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('SecretOfAlchemyEGTGambleStep', $slotSettings->GetGameData('SecretOfAlchemyEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('SecretOfAlchemyEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('SecretOfAlchemyEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('SecretOfAlchemyEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
