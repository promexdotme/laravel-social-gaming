            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( $postData['act'] == '464529TBNS' && $slotSettings->GetGameData('MagicHotWDTotalWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['act'] ) 
                        {
                            ...
                            case 'WXMNPP7H8F':
                                ...
                                $slotSettings->SetGameData('MagicHotWDBonusWin', 0);
                                $slotSettings->SetGameData('MagicHotWDFreeGames', 0);
                                $slotSettings->SetGameData('MagicHotWDCurrentFreeGame', 0);
                                $slotSettings->SetGameData('MagicHotWDTotalWin', 0);
                                $slotSettings->SetGameData('MagicHotWDFreeBalance', 0);
                                $slotSettings->SetGameData('MagicHotWDFreeStartWin', 0);
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('MagicHotWDBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                    }
                                                }
                                            }
                                        }
                                ...
                                $reportWin = $totalWin;
                                $slotSettings->SetGameData('MagicHotWDTotalWin', $totalWin);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $winString = implode(',', $lineWins);
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('MagicHotWDFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('MagicHotWDCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('MagicHotWDBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('MagicHotWDFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case '464529TBNS':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('MagicHotWDTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $winState = 0;
