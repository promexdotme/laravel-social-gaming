                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('SakuraFruitsAMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('SakuraFruitsAMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('SakuraFruitsAMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('SakuraFruitsAMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('SakuraFruitsAMTotalWin', 0);
                                $slotSettings->SetGameData('SakuraFruitsAMFreeBalance', 0);
                                $slotSettings->SetGameData('SakuraFruitsAMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('SakuraFruitsAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('SakuraFruitsAMFreeGames') - $slotSettings->GetGameData('SakuraFruitsAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('SakuraFruitsAMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('SakuraFruitsAMCurrentFreeGame') < $slotSettings->GetGameData('SakuraFruitsAMFreeGames') && $slotSettings->GetGameData('SakuraFruitsAMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('SakuraFruitsAMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('SakuraFruitsAMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('SakuraFruitsAMBonusWin', 0);
                                $slotSettings->SetGameData('SakuraFruitsAMFreeGames', 0);
                                $slotSettings->SetGameData('SakuraFruitsAMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SakuraFruitsAMTotalWin', 0);
                                $slotSettings->SetGameData('SakuraFruitsAMFreeBalance', 0);
                                $slotSettings->SetGameData('SakuraFruitsAMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('SakuraFruitsAMCurrentFreeGame', $slotSettings->GetGameData('SakuraFruitsAMCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('SakuraFruitsAMFreeGames') <= $slotSettings->GetGameData('SakuraFruitsAMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('SakuraFruitsAMBonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('SakuraFruitsAMBonusWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('SakuraFruitsAMBonusWin', $slotSettings->GetGameData('SakuraFruitsAMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('SakuraFruitsAMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('SakuraFruitsAMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('SakuraFruitsAMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('SakuraFruitsAMFreeGames', $slotSettings->GetGameData('SakuraFruitsAMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SakuraFruitsAMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('SakuraFruitsAMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('SakuraFruitsAMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"symDouble":' . $symDouble . ',"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('SakuraFruitsAMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SakuraFruitsAMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('SakuraFruitsAMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('SakuraFruitsAMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('SakuraFruitsAMFreeGames') - $slotSettings->GetGameData('SakuraFruitsAMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('SakuraFruitsAMFreeGames') <= $slotSettings->GetGameData('SakuraFruitsAMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('SakuraFruitsAMBonusWin');
                                $bonusMpl = $slotSettings->slotFreeMpl;
                            }
                            else
                            {
                            ...
                            $slotSettings->SetGameData('SakuraFruitsAMDoubleAnswer', $reelSate . $bet_h . $ln_h . $freeInfo . $freeWinState . $slotSettings->HexFormat($bonusMpl) . '1010' . $reelSate . '0' . $symCountAll . $winLinesFormated);
                            $slotSettings->SetGameData('SakuraFruitsAMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('SakuraFruitsAMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('SakuraFruitsAMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('SakuraFruitsAMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('SakuraFruitsAMCollect');
                            $slotSettings->SetGameData('SakuraFruitsAMTotalWin', 0);
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('SakuraFruitsAMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('SakuraFruitsAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('SakuraFruitsAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('SakuraFruitsAMDoubleWin', $winall);
                            $slotSettings->SetGameData('SakuraFruitsAMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('SakuraFruitsAMCollectP0');
                            $slotSettings->SetGameData('SakuraFruitsAMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('SakuraFruitsAMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('SakuraFruitsAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('SakuraFruitsAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
