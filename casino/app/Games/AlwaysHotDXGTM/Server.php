                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('AlwaysHotDXGTMDealerCard');
                            $totalWin = $slotSettings->GetGameData('AlwaysHotDXGTMTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('AlwaysHotDXGTMTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('AlwaysHotDXGTMDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('AlwaysHotDXGTMBonusWin', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTMFreeGames', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTMTotalWin', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTMFreeBalance', 0);
                            }
                            else
                            {
                                $slotSettings->SetGameData('AlwaysHotDXGTMCurrentFreeGame', $slotSettings->GetGameData('AlwaysHotDXGTMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotDXGTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotDXGTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            $s[0] = $reels['reel1'][$linesId[$k][0] - 1];
                                            $s[1] = $reels['reel2'][$linesId[$k][1] - 1];
                                            ...
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotDXGTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('AlwaysHotDXGTMFreeGames') <= $slotSettings->GetGameData('AlwaysHotDXGTMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('AlwaysHotDXGTMTotalWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('AlwaysHotDXGTMTotalWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('AlwaysHotDXGTMBonusWin', $slotSettings->GetGameData('AlwaysHotDXGTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('AlwaysHotDXGTMTotalWin', $slotSettings->GetGameData('AlwaysHotDXGTMTotalWin') + $totalWin);
                                $totalWin = $slotSettings->GetGameData('AlwaysHotDXGTMBonusWin');
                                $Balance = $slotSettings->GetGameData('AlwaysHotDXGTMFreeBalance');
                            }
                            else
                            {
                                $slotSettings->SetGameData('AlwaysHotDXGTMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('AlwaysHotDXGTMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('AlwaysHotDXGTMFreeBalance', $Balance);
                                    $slotSettings->SetGameData('AlwaysHotDXGTMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('AlwaysHotDXGTMFreeGames', $slotSettings->GetGameData('AlwaysHotDXGTMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AlwaysHotDXGTMFreeBalance', $Balance);
                                    $slotSettings->SetGameData('AlwaysHotDXGTMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('AlwaysHotDXGTMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('AlwaysHotDXGTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AlwaysHotDXGTMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
