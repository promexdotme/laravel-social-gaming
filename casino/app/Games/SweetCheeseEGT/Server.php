                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,20,100],"multiplier":1},"1":{"coef":[5,20,100],"multiplier":1},"2":{"coef":[5,20,100],"multiplier":1},"3":{"coef":[10,30,150],"multiplier":1},"4":{"coef":[10,30,150],"multiplier":1},"5":{"coef":[10,50,200],"multiplier":1},"6":{"coef":[10,50,200],"multiplier":1},"7":{"coef":[20,100,400],"multiplier":1},"8":{"coef":[20,150,1000],"multiplier":1},"9":{"coef":[20,150,1000],"multiplier":1},"10":{"coef":[10,100,1000,2500],"multiplier":1},"11":{"coef":[5,20,100],"multiplier":1},"12":{"coef":[],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[8,1,4,10,5,2,6,1,5,11,3,6,1,4,2,10,5,6,7,2],[2,8,12,6,1,5,8,2,4,11,5,9,2,4,1,7,12,8,3,10],[4,0,11,1,8,10,6,4,8,1,3,2,9,5,10,7,0,3,6,2],[2,6,3,5,2,8,12,7,0,5,10,5,7,11,4,8,6,12,4,9],[6,1,7,10,5,0,11,4,5,10,2,9,5,4,8,7,5,10,3,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('SweetCheeseEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('SweetCheeseEGTBonusWin', 0);
                                $slotSettings->SetGameData('SweetCheeseEGTFreeGames', 0);
                                $slotSettings->SetGameData('SweetCheeseEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SweetCheeseEGTTotalWin', 0);
                                $slotSettings->SetGameData('SweetCheeseEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('SweetCheeseEGTCurrentFreeGame') < $slotSettings->GetGameData('SweetCheeseEGTFreeGames') && $slotSettings->GetGameData('SweetCheeseEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('SweetCheeseEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('SweetCheeseEGTTotalWin', 0);
                                    $slotSettings->SetGameData('SweetCheeseEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'ExtraWild', -1);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SweetCheeseEGTCurrentFreeGame', $slotSettings->GetGameData('SweetCheeseEGTCurrentFreeGame') + 1);
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $wild = [
                                            '10', 
                                            $slotSettings->GetGameData($slotSettings->slotId . 'ExtraWild') . ''
                                        ];
                                    }
                                    for( $k = 0; $k < $lines; $k++ ) 
                                    {
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('SweetCheeseEGTBonusWin', $slotSettings->GetGameData('SweetCheeseEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('SweetCheeseEGTTotalWin', $slotSettings->GetGameData('SweetCheeseEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('SweetCheeseEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SweetCheeseEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('SweetCheeseEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('SweetCheeseEGTFreeGames', $slotSettings->GetGameData('SweetCheeseEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData($slotSettings->slotId . 'ExtraWild', rand(0, 8));
                                        $slotSettings->SetGameData('SweetCheeseEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('SweetCheeseEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('SweetCheeseEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('SweetCheeseEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"ExtraWild":' . $slotSettings->GetGameData($slotSettings->slotId . 'ExtraWild') . ',"totalFreeGames":' . $slotSettings->GetGameData('SweetCheeseEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SweetCheeseEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('SweetCheeseEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('SweetCheeseEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('SweetCheeseEGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(0, 6));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('SweetCheeseEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('SweetCheeseEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"expand":[],"specialSymbols":[],"freeSpinsWildSymbol":' . $slotSettings->GetGameData($slotSettings->slotId . 'ExtraWild') . ',"specialExpand":[],"gambles":5,"freespins":' . $fs . ',"jackpot":' . $isJack . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('SweetCheeseEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('SweetCheeseEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('SweetCheeseEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('SweetCheeseEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('SweetCheeseEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('SweetCheeseEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('SweetCheeseEGTGambleStep', $slotSettings->GetGameData('SweetCheeseEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('SweetCheeseEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('SweetCheeseEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('SweetCheeseEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
