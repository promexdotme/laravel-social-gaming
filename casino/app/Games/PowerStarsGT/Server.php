                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('PowerStarsGTDealerCard');
                            $totalWin = $slotSettings->GetGameData('PowerStarsGTTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('PowerStarsGTTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('PowerStarsGTDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('PowerStarsGTBonusWin', 0);
                                $slotSettings->SetGameData('PowerStarsGTFreeGames', 0);
                                $slotSettings->SetGameData('PowerStarsGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PowerStarsGTTotalWin', 0);
                                $slotSettings->SetGameData('PowerStarsGTFreeBalance', 0);
                                for( $r = 1; $r <= 5; $r++ ) 
                                {
                                    $slotSettings->SetGameData('PowerStarsGTBonusReels.' . $r, -1);
                                }
                            }
                            else
                            {
                                $slotSettings->SetGameData('PowerStarsGTCurrentFreeGame', $slotSettings->GetGameData('PowerStarsGTCurrentFreeGame') + 1);
                            ...
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                ...
                                for( $r = 1; $r <= 5; $r++ ) 
                                {
                                    if( $slotSettings->GetGameData('PowerStarsGTBonusReels.' . $r) < 1 ) 
                                    {
                                        $slotSettings->SetGameData('PowerStarsGTBonusReels.' . $r, -1);
                                    }
                                    for( $p = 0; $p <= 3; $p++ ) 
                                    {
                                        if( $reels['reel' . $r][$p] == 'P_1' && $slotSettings->GetGameData('PowerStarsGTBonusReels.' . $r) < 1 ) 
                                        {
                                            $slotSettings->SetGameData('PowerStarsGTBonusReels.' . $r, 0);
                                        }
                                    }
                                }
                                for( $r = 1; $r <= 5; $r++ ) 
                                {
                                    if( $slotSettings->GetGameData('PowerStarsGTBonusReels.' . $r) >= 0 ) 
                                    {
                                        $reels['reel' . $r][0] = 'P_1';
                                        $reels['reel' . $r][1] = 'P_1';
                                        $reels['reel' . $r][2] = 'P_1';
                                ...
                                for( $r = 1; $r <= 5; $r++ ) 
                                {
                                    for( $p = 0; $p <= 3; $p++ ) 
                                    {
                                        if( $reels['reel' . $r][$p] == 'P_1' && $slotSettings->GetGameData('PowerStarsGTBonusReels.' . $r) <= 0 ) 
                                        {
                                            $scattersCount++;
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('PowerStarsGTFreeGames') <= $slotSettings->GetGameData('PowerStarsGTCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('PowerStarsGTTotalWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('PowerStarsGTTotalWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('PowerStarsGTBonusWin', $slotSettings->GetGameData('PowerStarsGTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('PowerStarsGTTotalWin', $totalWin);
                            }
                            else
                            {
                            ...
                            if( $scattersCount >= 1 ) 
                            {
                                if( $slotSettings->GetGameData('PowerStarsGTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('PowerStarsGTFreeBalance', $Balance);
                                    $slotSettings->SetGameData('PowerStarsGTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('PowerStarsGTFreeGames', $slotSettings->GetGameData('PowerStarsGTFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('PowerStarsGTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('PowerStarsGTFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"BonusReels":[-1,' . $slotSettings->GetGameData('PowerStarsGTBonusReels.1') . ',' . $slotSettings->GetGameData('PowerStarsGTBonusReels.2') . ',' . $slotSettings->GetGameData('PowerStarsGTBonusReels.3') . ',' . $slotSettings->GetGameData('PowerStarsGTBonusReels.4') . ',' . $slotSettings->GetGameData('PowerStarsGTBonusReels.5') . '],"totalFreeGames":' . $slotSettings->GetGameData('PowerStarsGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('PowerStarsGTCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('PowerStarsGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
