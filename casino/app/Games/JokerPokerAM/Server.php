                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $DealCards = '';
                                $slotSettings->SetGameData('JokerPokerAMBet', $lastEvent->serverResponse->Bet);
                                $slotSettings->SetGameData('JokerPokerAMCurrentCards', $lastEvent->serverResponse->CurrentCards);
                                $slotSettings->SetGameData('JokerPokerAMCards', $lastEvent->serverResponse->Cards);
                                $slotSettings->SetGameData('JokerPokerAMbalanceFormated', $balanceFormated);
                                ...
                                {
                                    if( $slotSettings->Bet[$b] == $slotSettings->GetGameData('JokerPokerAMBet') ) 
                                    {
                                        $betLevel = $b;
                                        break;
                                    }
                        ...
                        case 'A/u253':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $resultCards0 = $slotSettings->GetGameData('JokerPokerAMCurrentCards');
                            }
                            else
                            {
                                $resultCards_ = $slotSettings->GetGameData('JokerPokerAMCurrentCards');
                                $cards = $slotSettings->GetGameData('JokerPokerAMCards');
                            }
                            ...
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                ...
                                $totalWin = $cc[0] * $slotSettings->GetGameData('JokerPokerAMBet');
                                if( $totalWin <= $bank ) 
                                {
                                    break;
                                }
                            ...
                            $slotSettings->SetGameData('JokerPokerAMTotalWin', $totalWin);
                            $DealCards = '';
                            for( $i = 0; $i < 5; $i++ ) 
                            {
                                $DealCards .= ('2' . $resultCards[$i]->suit . dechex($resultCards[$i]->value));
                            ...
                            $slotSettings->SetGameData('JokerPokerAMCollect', $response_collect);
                            $slotSettings->SetGameData($slotSettings->slotId . 'BankReserved', 0);
                            $logArr = (object)[
                                'Bet' => $slotSettings->GetGameData('JokerPokerAMBet'), 
                        ...
                        case 'A/u257':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response_collect = $slotSettings->GetGameData('JokerPokerAMCollect');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                        ...
                        case 'A/u251':
                            ...
                            $slotSettings->SetGameData('JokerPokerAMBet', $allbet);
                            $slotSettings->SetGameData('JokerPokerAMCurrentCards', $resultCards);
                            $slotSettings->SetGameData('JokerPokerAMCards', $cardsArr);
                            $slotSettings->SetGameData('JokerPokerAMbalanceFormated', $balanceFormated);
                            break;
                        case 'A/u2514':
                            $balanceFormated = $slotSettings->GetGameData('JokerPokerAMbalanceFormated');
                            $cardsID = [];
                            $cnt = 0;
                            for( $i = 2; $i <= 14; $i++ ) 
                            {
                            ...
                            $dCard = rand(2, 14);
                            $totalWin = $slotSettings->GetGameData('JokerPokerAMTotalWin');
                            ...
                            $slotSettings->SetGameData('JokerPokerAMDealerCard', $dCard);
                            $slotSettings->SetGameData('JokerPokerAMDealerCardS', $dealerCard);
                            break;
                        case 'A/u2515':
                            $balanceFormated = $slotSettings->GetGameData('JokerPokerAMbalanceFormated');
                            $cardsID = [];
                        ...
                        case 'A/u2513':
                            ...
                            $cWin = $slotSettings->GetGameData('JokerPokerAMTotalWin');
                            $aWin = $cWin * 2;
                            ...
                            if( $isWin == 1 ) 
                            {
                                $pCard = rand($slotSettings->GetGameData('JokerPokerAMDealerCard'), 14);
                            }
                            else
                            {
                                $pCard = rand(2, $slotSettings->GetGameData('JokerPokerAMDealerCard'));
                            }
                            if( $slotSettings->GetGameData('JokerPokerAMDealerCard') == $pCard ) 
                            {
                                $totalWin = $cWin;
                            }
                            else if( $slotSettings->GetGameData('JokerPokerAMDealerCard') < $pCard ) 
                            {
                                $totalWin = $aWin;
                            }
                            else
                            ...
                            $response = '10d010' . $balanceFormated . $slotSettings->HexFormat(round($totalWin * 100)) . '10001a33e810' . $slotSettings->GetGameData('JokerPokerAMDealerCardS') . $pCardsStr . '0000001723c1d23b21b10101023b21b020100000' . $pSelect . $slotSettings->GetGameData('JokerPokerAMDealerCardS') . '#' . $pCard . '|' . $slotSettings->GetGameData('JokerPokerAMDealerCardS');
                            if( $totalWin <= 0 ) 
                            {
                                $totalWin = -1 * $cWin;
                            }
                            $response_log = '{"responseEvent":"gambleResult","serverResponse":{"totalWin":' . $totalWin . '}}';
                            $slotSettings->SaveLogReport($response_log, $slotSettings->GetGameData('JokerPokerAMBet'), 1, $totalWin, 'double');
                            break;
                    }
