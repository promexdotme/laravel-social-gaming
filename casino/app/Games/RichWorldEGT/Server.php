                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[10,40,100],"multiplier":0},"1":{"coef":[10,40,100],"multiplier":0},"2":{"coef":[10,50,200],"multiplier":0},"3":{"coef":[10,50,200],"multiplier":0},"4":{"coef":[12,60,450],"multiplier":0},"5":{"coef":[12,60,450],"multiplier":0},"6":{"coef":[15,150,450],"multiplier":0},"7":{"coef":[15,150,450],"multiplier":0},"8":{"coef":[25,200,850],"multiplier":0},"9":{"coef":[30,200,850],"multiplier":0},"10":{"coef":[50,850,5000],"multiplier":0},"11":{"coef":[],"multiplier":1},"12":{"coef":[2,5,20,200],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"freespinFakeReels":[[1,4,5,12,0,8,1,2,7,0,10,5,4,6,3,2,12,1,3,9],[11,11,11,5,7,0,4,10,3,8,1,9,2,6,12,3,8,5,6,1],[7,3,12,6,1,0,5,10,2,6,11,11,11,2,9,3,5,8,4,0],[5,6,3,2,11,11,11,9,3,2,10,0,1,7,4,8,12,3,4,2],[8,4,2,12,1,6,2,8,4,3,6,5,7,1,9,0,8,3,4,10]],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[1,4,5,12,0,8,1,2,7,0,10,5,4,6,3,2,12,1,3,9],[11,5,2,10,1,7,4,3,12,8,1,9,2,6,0,3,10,5,6,1],[7,3,11,0,1,8,5,10,2,6,11,1,6,2,9,3,12,5,4,0],[5,6,3,2,11,4,0,9,3,2,10,0,1,7,4,2,8,3,4,12],[8,4,2,12,1,6,2,8,4,3,6,5,7,1,9,0,8,3,4,10]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('RichWorldEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('RichWorldEGTBonusWin', 0);
                                $slotSettings->SetGameData('RichWorldEGTFreeGames', 0);
                                $slotSettings->SetGameData('RichWorldEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('RichWorldEGTTotalWin', 0);
                                $slotSettings->SetGameData('RichWorldEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('RichWorldEGTCurrentFreeGame') < $slotSettings->GetGameData('RichWorldEGTFreeGames') && $slotSettings->GetGameData('RichWorldEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('RichWorldEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('RichWorldEGTTotalWin', 0);
                                    $slotSettings->SetGameData('RichWorldEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('RichWorldEGTCurrentFreeGame', $slotSettings->GetGameData('RichWorldEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('RichWorldEGTBonusWin', $slotSettings->GetGameData('RichWorldEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('RichWorldEGTTotalWin', $slotSettings->GetGameData('RichWorldEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('RichWorldEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('RichWorldEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('RichWorldEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('RichWorldEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('RichWorldEGTFreeGames', $slotSettings->GetGameData('RichWorldEGTFreeGames') + $slotSettings->slotFreeCount[$scattersCount]);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('RichWorldEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('RichWorldEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('RichWorldEGTFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                    }
                                    $fs = $slotSettings->GetGameData('RichWorldEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('RichWorldEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('RichWorldEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('RichWorldEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('RichWorldEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('RichWorldEGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(0, 6));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('RichWorldEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('RichWorldEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('RichWorldEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('RichWorldEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('RichWorldEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('RichWorldEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('RichWorldEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('RichWorldEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('RichWorldEGTGambleStep', $slotSettings->GetGameData('RichWorldEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('RichWorldEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('RichWorldEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('RichWorldEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
