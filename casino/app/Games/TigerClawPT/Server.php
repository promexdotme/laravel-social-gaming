                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('TigerClawPTBonusWin', 0);
                                $slotSettings->SetGameData('TigerClawPTFreeGames', 0);
                                $slotSettings->SetGameData('TigerClawPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('TigerClawPTTotalWin', 0);
                                $slotSettings->SetGameData('TigerClawPTFreeBalance', 0);
                                $slotSettings->SetGameData('TigerClawPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('TigerClawPTCurrentFreeGame', $slotSettings->GetGameData('TigerClawPTCurrentFreeGame') + 1);
                                $tmpMpl = [
                                    2, 
                                    3, 
                                    5
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('TigerClawPTBonusWin', $slotSettings->GetGameData('TigerClawPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('TigerClawPTTotalWin', $totalWin);
                                $sID = 47752;
                                $rI = 5;
                            }
                            else
                            ...
                            if( $scattersCount >= 2 ) 
                            {
                                if( $slotSettings->GetGameData('TigerClawPTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('TigerClawPTFreeGames', $slotSettings->GetGameData('TigerClawPTFreeGames') + $slotSettings->slotFreeCountAdd[$scattersCount]);
                                }
                                else if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('TigerClawPTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('TigerClawPTBonusWin', 0);
                                    $slotSettings->SetGameData('TigerClawPTFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[4096],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('TigerClawPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('TigerClawPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('TigerClawPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('TigerClawPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
