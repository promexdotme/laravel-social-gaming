            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'bonuschoice':
                                if( $slotSettings->GetGameData('ForestTaleEGTBonusStart') != 1 ) 
                                {
                                    exit();
                                }
                                $allbet = $slotSettings->GetGameData('ForestTaleEGTBonusBet');
                                $choice = (int)$postData['bet']['choice'];
                                $curWin = $slotSettings->GetGameData('ForestTaleEGTChoiceWinAmount');
                                $scnt = $slotSettings->GetGameData('ForestTaleEGTChoiceCount');
                                $scWinsCoef = $curWin[2];
                                shuffle($scWinsCoef);
                                $scWins = [
                                    '{"coef":' . $scWinsCoef[0] . ',"bonus":false,"totalWin":' . ($scWinsCoef[0] * $allbet * 100) . '}', 
                                    '{"coef":' . $scWinsCoef[1] . ',"bonus":false,"totalWin":' . ($scWinsCoef[1] * $allbet * 100) . '}', 
                                    '{"coef":' . $scWinsCoef[2] . ',"bonus":false,"totalWin":' . ($scWinsCoef[2] * $allbet * 100) . '}'
                                ];
                                if( $scnt >= 4 ) 
                                {
                                    $scWins[] = '{"coef":' . $scWinsCoef[3] . ',"bonus":false,"totalWin":' . ($scWinsCoef[3] * $allbet * 100) . '}';
                                }
                                if( $scnt >= 5 ) 
                                {
                                    $scWins[] = '{"coef":' . $scWinsCoef[4] . ',"bonus":false,"totalWin":' . ($scWinsCoef[4] * $allbet * 100) . '}';
                                }
                                $scWins[$choice] = '{"coef":' . $curWin[0] . ',"bonus":true,"totalWin":' . ($curWin[0] * $allbet * 100) . '}';
                                $slotSettings->SetGameData('ForestTaleEGTBonusStart', 2);
                                $result_tmp[] = '{"complex":{"bonuschoiceObject":{"selectedIndex":' . $choice . ',"totalBet":' . ($allbet * 100) . ',"options":[' . implode(',', $scWins) . ']},"freespins":0,"multiplier":1,"choices":0,"gambles":0,"gameCommand":"bonuschoice"},"state":"choice","winAmount":' . (($slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') + $curWin[1]) * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":1800675758589,"sessionKey":"f03017b9a27ddd27b5214d6a063b9ab3","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":1569874127218}';
                                break;
                            case 'choice':
                                if( $slotSettings->GetGameData('ForestTaleEGTBonusStart') != 1 && $slotSettings->GetGameData('ForestTaleEGTBonusStart') != 2 ) 
                                {
                                    exit();
                                }
                                $ChoiceArr = $slotSettings->GetGameData('ForestTaleEGTChoiceArr');
                                $fs = $slotSettings->GetGameData('ForestTaleEGTFreeGames');
                                $fw = $slotSettings->GetGameData('ForestTaleEGTMultiplier');
                                $choice = (int)$postData['bet']['choice'];
                                $opt = [];
                                if( !is_array($ChoiceArr) ) 
                                {
                                    $ChoiceArr = [];
                                }
                                $ChoiceArr[] = $choice;
                                ...
                                if( count($ChoiceArr) >= 3 && $slotSettings->GetGameData('ForestTaleEGTFreeGames') > 0 ) 
                                {
                                    $isEnd = true;
                                    $pl_cur = 'true';
                                    $slotSettings->SetGameData('ForestTaleEGTBonusStart', 0);
                                }
                                $state = 'choice';
                                $closed = '';
                                if( $isEnd ) 
                                {
                                    ...
                                    for( $i = 0; $i < 12; $i++ ) 
                                    {
                                        ...
                                        if( !in_array($i, $ChoiceArr) ) 
                                        {
                                            $opt[$i] = '{"pos":' . $i . ',"multiplier":' . $cfr0 . ',"freespins":' . $cfr . ',"mult":' . $cfr0 . ',"fs":' . $cfr . ',"play":' . $pl[rand(0, 1)] . '}';
                                        }
                                    }
                                    $closed = implode(',', $opt);
                                }
                                $slotSettings->SetGameData('ForestTaleEGTChoiceArr', $ChoiceArr);
                                $slotSettings->SetGameData('ForestTaleEGTFreeGames', $fs + $cfr_cur);
                                $slotSettings->SetGameData('ForestTaleEGTMultiplier', $fw + $cfr0_cur);
                                $result_tmp[] = '{"complex":{"choice":{"pos":' . $choice . ',"multiplier":' . $slotSettings->GetGameData('ForestTaleEGTMultiplier') . ',"freespins":' . $slotSettings->GetGameData('ForestTaleEGTFreeGames') . ',"mult":' . $cfr0_cur . ',"fs":' . $cfr_cur . ',"play":' . $pl_cur . ',"closed":[' . $closed . ']},"freespins":' . $slotSettings->GetGameData('ForestTaleEGTFreeGames') . ',"multiplier":1,"gameCommand":"choice"},"state":"' . $state . '","winAmount":' . ($slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":1789706666325,"sessionKey":"46e640104224cef8245c4329c25ef561","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":1567196976582}';
                                break;
                            case 'login':
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,20,100],"multiplier":2},"1":{"coef":[5,20,100],"multiplier":2},"2":{"coef":[5,20,100],"multiplier":2},"3":{"coef":[5,20,100],"multiplier":2},"4":{"coef":[10,50,200],"multiplier":2},"5":{"coef":[10,50,200],"multiplier":2},"6":{"coef":[10,50,200],"multiplier":2},"7":{"coef":[2,20,100,500],"multiplier":2},"8":{"coef":[2,20,100,500],"multiplier":2},"9":{"coef":[10,100,1000,10000],"multiplier":1},"10":{"coef":[2,10,10,50,50,150],"multiplier":1},"11":{"coef":[0,0,0],"multiplier":1}},"scatterCoef":{"3":{"withoutBonus":[2,3,4,6,10]},"4":{"withoutBonus":[50,20,10,30,15]},"5":{"withoutBonus":[50,100,80,150,120]}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[6,4,6,1,1,0,8,0,3,3,4,6,4,9,8,2,1,5,5,7],[6,2,0,0,6,0,3,8,3,1,1,7,4,4,8,2,2,5,8,5],[3,6,9,2,2,8,3,3,7,4,4,10,6,0,0,5,8,5,1,1],[6,5,5,3,3,8,7,5,6,9,4,4,1,8,1,2,2,0,0,0],[6,1,5,7,5,8,0,0,3,6,3,4,8,2,6,2,3,9,1,1]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('ForestTaleEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('ForestTaleEGTBonusWin', 0);
                                $slotSettings->SetGameData('ForestTaleEGTFreeGames', 0);
                                $slotSettings->SetGameData('ForestTaleEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('ForestTaleEGTTotalWin', 0);
                                $slotSettings->SetGameData('ForestTaleEGTFreeBalance', 0);
                                $slotSettings->SetGameData('ForestTaleEGTMultiplier', 1);
                                $slotSettings->SetGameData('ForestTaleEGTFreeWild', -1);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('ForestTaleEGTCurrentFreeGame') < $slotSettings->GetGameData('ForestTaleEGTFreeGames') && $slotSettings->GetGameData('ForestTaleEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('ForestTaleEGTBonusStart', 0);
                                    $slotSettings->SetGameData('ForestTaleEGTBonusBet', $allbet);
                                    $slotSettings->SetGameData('ForestTaleEGTBonusWin', 0);
                                    $slotSettings->SetGameData('ForestTaleEGTFreeGames', 0);
                                    $slotSettings->SetGameData('ForestTaleEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('ForestTaleEGTTotalWin', 0);
                                    $slotSettings->SetGameData('ForestTaleEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData('ForestTaleEGTMultiplier', 1);
                                    $slotSettings->SetGameData('ForestTaleEGTFreeWild', -1);
                                    $bonusMpl = 1;
                                }
                                ...
                                {
                                    $slotSettings->SetGameData('ForestTaleEGTCurrentFreeGame', $slotSettings->GetGameData('ForestTaleEGTCurrentFreeGame') + 1);
                                    $bonusMpl = $slotSettings->GetGameData('ForestTaleEGTMultiplier');
                                }
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    $scattersWin3 = 0;
                                    if( $totalWin <= $spinWinLimit && $scattersCount2 >= 3 ) 
                                    {
                                        if( $scattersCount2 == 3 ) 
                                        {
                                            $scf = [
                                        ...
                                        shuffle($scf);
                                        $scattersWin3 = $allbet * $scf[0];
                                        $slotSettings->SetGameData('ForestTaleEGTChoiceWinAmount', [
                                            $scf[0], 
                                            $scattersWin3, 
                                            $scf
                                        ]);
                                        $slotSettings->SetGameData('ForestTaleEGTChoiceCount', $scattersCount2);
                                    }
                                    if( $i > 1000 ) 
                                    {
                                        $winType = 'none';
                                    ...
                                    else
                                    {
                                        ...
                                        else if( $totalWin + $scattersWin3 <= $spinWinLimit && $winType == 'bonus' ) 
                                        {
                                        ...
                                        else if( $totalWin > 0 && $totalWin + $scattersWin3 <= $spinWinLimit && $winType == 'win' ) 
                                        {
                                            $cBank = $slotSettings->GetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''));
                                            if( $cBank < $spinWinLimit ) 
                                            {
                                        ...
                                        else if( $totalWin + $scattersWin3 == 0 && $winType == 'none' ) 
                                        {
                                            break;
                                        }
                                    }
                                ...
                                if( $totalWin > 0 ) 
                                {
                                    $slotSettings->SetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''), -1 * $totalWin + $scattersWin3);
                                    $slotSettings->SetBalance($totalWin + $scattersWin3);
                                }
                                $reportWin = $totalWin + $scattersWin3;
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('ForestTaleEGTBonusWin', $slotSettings->GetGameData('ForestTaleEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('ForestTaleEGTTotalWin', $slotSettings->GetGameData('ForestTaleEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('ForestTaleEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('ForestTaleEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 && $postData['slotEvent'] != 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('ForestTaleEGTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('ForestTaleEGTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('ForestTaleEGTFreeGames', 0);
                                    $slotSettings->SetGameData('ForestTaleEGTMultiplier', 1);
                                    $slotSettings->SetGameData('ForestTaleEGTFreeWild', rand(5, 7));
                                    $slotSettings->SetGameData('ForestTaleEGTChoiceArr', []);
                                    $fs = $slotSettings->GetGameData('ForestTaleEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"Multiplier":' . $slotSettings->GetGameData('ForestTaleEGTMultiplier') . ',"FreeWild":' . $slotSettings->GetGameData('ForestTaleEGTFreeWild') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('ForestTaleEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('ForestTaleEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('ForestTaleEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('ForestTaleEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('ForestTaleEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('ForestTaleEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('ForestTaleEGTBonusWin');
                                    $state = 'freespin';
                                    if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('ForestTaleEGTBonusWin') > 0 ) 
                                    {
                                        $state = 'gamble';
                                    }
                                }
                                ...
                                if( $scattersCount2 >= 3 ) 
                                {
                                    $state = 'bonuschoice';
                                    $slotSettings->SetGameData('ForestTaleEGTTotalWin', $totalWin);
                                    $slotSettings->SetGameData('ForestTaleEGTBonusStart', 1);
                                    $slotSettings->SetGameData('ForestTaleEGTBonusCnt', $scattersCount2);
                                    $bstate = ',"bonuschoiceObject": {"bonusScatterIndex": 10}';
                                }
                                if( $scattersCount >= 3 ) 
                                {
                                    $state = 'choice';
                                    $slotSettings->SetGameData('ForestTaleEGTBonusStart', 1);
                                    $bstate = ',"choices": 1,"freespinScatters":[11]';
                                }
                                if( $scattersCount >= 3 && $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $state = 'freespin';
                                    $sgwin = $slotSettings->GetGameData('ForestTaleEGTFreeGames');
                                    $slotSettings->SetGameData('ForestTaleEGTFreeGames', $slotSettings->GetGameData('ForestTaleEGTFreeGames') + 1);
                                }
                                if( !isset($sgwin) ) 
                                {
                                    $fs = 0;
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"choices": 0,"expand":[]' . $bstate . ',"multiplier":' . $slotSettings->GetGameData('ForestTaleEGTMultiplier') . ',"wildcard":' . $slotSettings->GetGameData('ForestTaleEGTFreeWild') . ',"specialExpand":[],"gambles":5,"freespins":' . $fs . ',"jackpot":' . $isJack . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('ForestTaleEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('ForestTaleEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('ForestTaleEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('ForestTaleEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('ForestTaleEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('ForestTaleEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('ForestTaleEGTGambleStep', $slotSettings->GetGameData('ForestTaleEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('ForestTaleEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
