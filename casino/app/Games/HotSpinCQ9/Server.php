                        ...
                        switch( $aid ) 
                        {
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('HotSpinCQ9BonusStart', 0);
                                    $slotSettings->SetGameData('HotSpinCQ9BonusBet', $allbet);
                                    $slotSettings->SetGameData('HotSpinCQ9BonusWin', 0);
                                    $slotSettings->SetGameData('HotSpinCQ9FreeGames', 0);
                                    $slotSettings->SetGameData('HotSpinCQ9CurrentFreeGame', 0);
                                    $slotSettings->SetGameData('HotSpinCQ9TotalWin', 0);
                                    $slotSettings->SetGameData('HotSpinCQ9FreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData('HotSpinCQ9Multiplier', 1);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('HotSpinCQ9CurrentFreeGame', $slotSettings->GetGameData('HotSpinCQ9CurrentFreeGame') + 1);
                                    $bonusMpl = $slotSettings->GetGameData('HotSpinCQ9Multiplier');
                                }
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $scattersCount >= 3 ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('HotSpinCQ9scatPayMpl', $scatPayArr[0]);
                                        $slotSettings->SetGameData('HotSpinCQ9scatPayWin', $scatPayWin);
                                    }
                                ...
                                $reportWin = $totalWin;
                                $slotSettings->SetGameData('HotSpinCQ9TotalWin', $totalWin);
                                $curReels = '';
                                ...
                                if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('HotSpinCQ9BonusTrigger', 1);
                                    $WinType = 4;
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"Multiplier":' . $slotSettings->GetGameData('HotSpinCQ9Multiplier') . ',"FreeWild":' . $slotSettings->GetGameData('HotSpinCQ9FreeWild') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('HotSpinCQ9FreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('HotSpinCQ9CurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('HotSpinCQ9BonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'spin_accept':
                                ...
                                break;
                            case 'bonus_step':
                                if( $slotSettings->GetGameData('HotSpinCQ9BonusTrigger') == 1 ) 
                                {
                                    $scatPayMpl = $slotSettings->GetGameData('HotSpinCQ9scatPayMpl');
                                    $scatPayWin = $slotSettings->GetGameData('HotSpinCQ9scatPayWin');
                                    $eResp = '{"err":200,"res":2,"vals":[1,{"Type":3,"ID":145,"Version":0,"ErrorCode":0,"PlayerBet":50,"AccumlateWinAmt":' . round($scatPayWin * 100) . ',"ScatterPayFromBaseGame":' . round($scatPayWin * 100) . ',"GameComplete":true,"udcDataSet":{"SelExtraData":[],"SelMultiplier":[],"SelSpinTimes":[],"SelWin":[],"PlayerSelected":[0]}}],"msg":null}';
                                    $slotSettings->SetGameData('HotSpinCQ9BonusTrigger', 0);
                                    $result_tmp[] = '~m~' . (strlen($eResp) + 3) . '~m~~j~' . $eResp;
                                }
                                break;
                            case 'bonus_accept':
