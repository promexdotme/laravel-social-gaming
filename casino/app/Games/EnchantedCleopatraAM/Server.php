                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('EnchantedCleopatraAMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('EnchantedCleopatraAMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('EnchantedCleopatraAMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('EnchantedCleopatraAMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('EnchantedCleopatraAMTotalWin', 0);
                                $slotSettings->SetGameData('EnchantedCleopatraAMFreeBalance', 0);
                                $slotSettings->SetGameData('EnchantedCleopatraAMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('EnchantedCleopatraAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') - $slotSettings->GetGameData('EnchantedCleopatraAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('EnchantedCleopatraAMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('EnchantedCleopatraAMCurrentFreeGame') < $slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') && $slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('EnchantedCleopatraAMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('EnchantedCleopatraAMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('EnchantedCleopatraAMBonusWin', 0);
                                $slotSettings->SetGameData('EnchantedCleopatraAMFreeGames', 0);
                                $slotSettings->SetGameData('EnchantedCleopatraAMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('EnchantedCleopatraAMTotalWin', 0);
                                $slotSettings->SetGameData('EnchantedCleopatraAMFreeBalance', 0);
                                $slotSettings->SetGameData('EnchantedCleopatraAMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('EnchantedCleopatraAMCurrentFreeGame', $slotSettings->GetGameData('EnchantedCleopatraAMCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') <= $slotSettings->GetGameData('EnchantedCleopatraAMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('EnchantedCleopatraAMBonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('EnchantedCleopatraAMBonusWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('EnchantedCleopatraAMBonusWin', $slotSettings->GetGameData('EnchantedCleopatraAMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('EnchantedCleopatraAMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('EnchantedCleopatraAMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('EnchantedCleopatraAMFreeGames', $slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') + $slotSettings->slotFreeCount[$scattersCount]);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('EnchantedCleopatraAMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('EnchantedCleopatraAMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('EnchantedCleopatraAMFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"symDouble":' . $symDouble . ',"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('EnchantedCleopatraAMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('EnchantedCleopatraAMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('EnchantedCleopatraAMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') - $slotSettings->GetGameData('EnchantedCleopatraAMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('EnchantedCleopatraAMFreeGames') <= $slotSettings->GetGameData('EnchantedCleopatraAMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('EnchantedCleopatraAMBonusWin');
                                $bonusMpl = $slotSettings->slotFreeMpl;
                            }
                            else
                            {
                            ...
                            $slotSettings->SetGameData('EnchantedCleopatraAMDoubleAnswer', $reelSate . $bet_h . $ln_h . $freeInfo . $freeWinState . $slotSettings->HexFormat($bonusMpl) . '1010' . $reelSate . '0' . $symCountAll . $winLinesFormated);
                            $slotSettings->SetGameData('EnchantedCleopatraAMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('EnchantedCleopatraAMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('EnchantedCleopatraAMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('EnchantedCleopatraAMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('EnchantedCleopatraAMCollect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('EnchantedCleopatraAMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('EnchantedCleopatraAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('EnchantedCleopatraAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('EnchantedCleopatraAMDoubleWin', $winall);
                            $slotSettings->SetGameData('EnchantedCleopatraAMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('EnchantedCleopatraAMCollectP0');
                            $slotSettings->SetGameData('EnchantedCleopatraAMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('EnchantedCleopatraAMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('EnchantedCleopatraAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('EnchantedCleopatraAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
