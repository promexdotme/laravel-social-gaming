                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'subscribe':
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('GraceOfCleopatraEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('GraceOfCleopatraEGTBonusWin', 0);
                                $slotSettings->SetGameData('GraceOfCleopatraEGTFreeGames', 0);
                                $slotSettings->SetGameData('GraceOfCleopatraEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('GraceOfCleopatraEGTTotalWin', 0);
                                $slotSettings->SetGameData('GraceOfCleopatraEGTFreeBalance', 0);
                                $slotSettings->SetGameData('GraceOfCleopatraEGTExpandSymbol', -1);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('GraceOfCleopatraEGTCurrentFreeGame') < $slotSettings->GetGameData('GraceOfCleopatraEGTFreeGames') && $slotSettings->GetGameData('GraceOfCleopatraEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTTotalWin', 0);
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTExpandSymbol', -1);
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', '');
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTCurrentFreeGame', $slotSettings->GetGameData('GraceOfCleopatraEGTCurrentFreeGame') + 1);
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        for( $r = 1; $r <= 5; $r++ ) 
                                        {
                                            for( $p = 0; $p <= 2; $p++ ) 
                                            {
                                                if( $reelsEx['reel' . $r][$p] == $slotSettings->GetGameData('GraceOfCleopatraEGTExpandSymbol') ) 
                                                {
                                                    $reelsEx['reel' . $r][0] = $slotSettings->GetGameData('GraceOfCleopatraEGTExpandSymbol');
                                                    $reelsEx['reel' . $r][1] = $slotSettings->GetGameData('GraceOfCleopatraEGTExpandSymbol');
                                                    $reelsEx['reel' . $r][2] = $slotSettings->GetGameData('GraceOfCleopatraEGTExpandSymbol');
                                                    break;
                                                }
                                            }
                                        }
                                    ...
                                    for( $k = 0; $k < $lines; $k++ ) 
                                    {
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if( $postData['slotEvent'] == 'freespin' && $csym == $slotSettings->GetGameData('GraceOfCleopatraEGTExpandSymbol') ) 
                                                {
                                                    $cLineExp = [];
                                                    $exCount = 0;
                                                    for( $ex = 0; $ex < 5; $ex++ ) 
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTBonusWin', $slotSettings->GetGameData('GraceOfCleopatraEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTTotalWin', $slotSettings->GetGameData('GraceOfCleopatraEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('GraceOfCleopatraEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                $firstSpinLog = '';
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                ...
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('GraceOfCleopatraEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('GraceOfCleopatraEGTFreeGames', $slotSettings->GetGameData('GraceOfCleopatraEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('GraceOfCleopatraEGTExpandSymbol', rand(0, 8));
                                        $slotSettings->SetGameData('GraceOfCleopatraEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('GraceOfCleopatraEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('GraceOfCleopatraEGTFreeGames', $slotSettings->slotFreeCount);
                                        $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', ',"firstSpin":{"expand":[],"reels":[' . $curReels . '],"lines":[],"expand":[],"scatters":[' . $scattersStr . ']}');
                                    }
                                    $fs = $slotSettings->GetGameData('GraceOfCleopatraEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $winStringExpand = implode(',', $lineWins2);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('GraceOfCleopatraEGTFreeGames') . ',"freeSpinsExpandLines": [' . $winStringExpand . '],"freeSpinsExpandSymbol":' . $slotSettings->GetGameData('GraceOfCleopatraEGTExpandSymbol') . ',"freeSpinsExpandWinAmount": ' . $expandWin . ',"currentFreeGames":' . $slotSettings->GetGameData('GraceOfCleopatraEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('GraceOfCleopatraEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('GraceOfCleopatraEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('GraceOfCleopatraEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('GraceOfCleopatraEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"expand":[],"gambles":5,"freespinScatters":[9],"freespins":' . $fs . ',"jackpot":' . $isJack . ',"freeSpinsExpandSymbol":' . $slotSettings->GetGameData('GraceOfCleopatraEGTExpandSymbol') . ',"freeSpinsExpandLines":[' . $winStringExpand . '],"freeSpinsExpandWinAmount":' . $expandWin . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":1786348900601,"balance":' . $balanceInCents . ',"sessionKey":"894b17d797618b2d99d0c94df1d88e72","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('GraceOfCleopatraEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('GraceOfCleopatraEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('GraceOfCleopatraEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('GraceOfCleopatraEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('GraceOfCleopatraEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('GraceOfCleopatraEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('GraceOfCleopatraEGTGambleStep', $slotSettings->GetGameData('GraceOfCleopatraEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('GraceOfCleopatraEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('GraceOfCleopatraEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('GraceOfCleopatraEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
