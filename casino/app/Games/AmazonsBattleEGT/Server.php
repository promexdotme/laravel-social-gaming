                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[2,8,15],"multiplier":1},"1":{"coef":[2,8,15],"multiplier":1},"2":{"coef":[4,10,20],"multiplier":1},"3":{"coef":[4,10,20],"multiplier":1},"4":{"coef":[6,15,25],"multiplier":1},"5":{"coef":[8,15,30],"multiplier":1},"6":{"coef":[8,15,40],"multiplier":1},"7":{"coef":[4,10,20,45],"multiplier":1},"8":{"coef":[5,10,20,50],"multiplier":1},"9":{"coef":[10,50,250,1000],"multiplier":1},"10":{"coef":[2,2,2],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"freespinFakeReels":[[0,0,0,1,1,1,2,2,2,3,3,3,4,4,4,5,5,5,6,6,6,7,7,7,8,8,8,9,9,9,0,0,0],[4,4,4,6,6,6,2,2,2,8,8,8,3,3,3,0,0,0,1,1,1,8,8,8,9,9,9,7,7,7,5,5,5],[2,2,2,1,1,1,0,0,0,8,8,8,4,4,4,5,5,5,9,9,9,7,7,7,3,3,3,6,6,6,1,1,1],[9,9,9,7,7,7,2,2,2,8,8,8,0,0,0,5,5,5,6,6,6,1,1,1,3,3,3,0,0,0,4,4,4],[6,6,6,9,9,9,2,2,2,8,8,8,4,4,4,5,5,5,0,0,0,7,7,7,3,3,3,1,1,1,4,4,4]],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[0,0,0,1,1,1,2,2,2,3,3,3,4,4,4,5,5,5,6,6,6,7,7,7,8,8,8,9,9,9,0,0,0],[4,4,4,6,6,6,2,2,2,8,8,8,3,3,3,0,0,0,1,1,1,10,10,10,9,9,9,7,7,7,5,5,5],[2,2,2,1,1,1,0,0,0,8,8,8,4,4,4,5,5,5,9,9,9,7,7,7,3,3,3,6,6,6,10,10,10],[9,9,9,7,7,7,2,2,2,8,8,8,10,10,10,5,5,5,6,6,6,1,1,1,3,3,3,0,0,0,4,4,4],[6,6,6,9,9,9,2,2,2,8,8,8,4,4,4,5,5,5,0,0,0,7,7,7,3,3,3,1,1,1,4,4,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('AmazonsBattleEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('AmazonsBattleEGTBonusWin', 0);
                                $slotSettings->SetGameData('AmazonsBattleEGTFreeGames', 0);
                                $slotSettings->SetGameData('AmazonsBattleEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('AmazonsBattleEGTTotalWin', 0);
                                $slotSettings->SetGameData('AmazonsBattleEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('AmazonsBattleEGTCurrentFreeGame') < $slotSettings->GetGameData('AmazonsBattleEGTFreeGames') && $slotSettings->GetGameData('AmazonsBattleEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('AmazonsBattleEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('AmazonsBattleEGTTotalWin', 0);
                                    $slotSettings->SetGameData('AmazonsBattleEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AmazonsBattleEGTCurrentFreeGame', $slotSettings->GetGameData('AmazonsBattleEGTCurrentFreeGame') + 1);
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $scattersCount >= 7 ) 
                                    {
                                        ...
                                        var_dump($scPos);
                                        $scattersStr = '{"scatterName":10,"cells":[' . implode(',', $scPos) . '],"winAmount":' . ($scattersWin * 100) . ',"freespins":' . $sgwin . '}';
                                    }
                                    ...
                                    else
                                    {
                                        ...
                                        else if( $scattersCount >= 7 && $winType != 'bonus' ) 
                                        {
                                        }
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('AmazonsBattleEGTBonusWin', $slotSettings->GetGameData('AmazonsBattleEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('AmazonsBattleEGTTotalWin', $slotSettings->GetGameData('AmazonsBattleEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('AmazonsBattleEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AmazonsBattleEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 7 ) 
                                {
                                    if( $slotSettings->GetGameData('AmazonsBattleEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('AmazonsBattleEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('AmazonsBattleEGTFreeGames', $slotSettings->GetGameData('AmazonsBattleEGTFreeGames') + $slotSettings->slotFreeCount[$scattersCount]);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('AmazonsBattleEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('AmazonsBattleEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('AmazonsBattleEGTFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                    }
                                    $fs = $slotSettings->GetGameData('AmazonsBattleEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('AmazonsBattleEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AmazonsBattleEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('AmazonsBattleEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('AmazonsBattleEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('AmazonsBattleEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('AmazonsBattleEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('AmazonsBattleEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('AmazonsBattleEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('AmazonsBattleEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('AmazonsBattleEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('AmazonsBattleEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('AmazonsBattleEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('AmazonsBattleEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('AmazonsBattleEGTGambleStep', $slotSettings->GetGameData('AmazonsBattleEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('AmazonsBattleEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('AmazonsBattleEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('AmazonsBattleEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
