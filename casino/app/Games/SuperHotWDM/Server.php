            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( $postData['act'] == '464529TBNS' && $slotSettings->GetGameData('SuperHotWDMTotalWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['act'] ) 
                        {
                            ...
                            case 'D3425KPHW1':
                                ...
                                break;
                            case 'PTUDQQMKZB':
                                $result_tmp[] = '{"a": "CRc5ya6I", "cid": "' . $reqCid . '", "id":  ' . $reqId . ', "res": {"limit_count": "0"}, "s": "ok"}';
                                break;
                            case 'ping':
                            ...
                            case 'close':
                                ...
                                break;
                            case 'pause':
                                $result_tmp[] = '{"a": "VfgEano2", "cid": "' . $reqCid . '", "id": ' . $reqId . ', "s": "ok"}';
                                break;
                            case 'resume':
                            ...
                            case 'WXMNPP7H8F':
                                ...
                                $slotSettings->SetGameData('SuperHotWDMBonusWin', 0);
                                $slotSettings->SetGameData('SuperHotWDMFreeGames', 0);
                                $slotSettings->SetGameData('SuperHotWDMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SuperHotWDMTotalWin', 0);
                                $slotSettings->SetGameData('SuperHotWDMFreeBalance', 0);
                                $slotSettings->SetGameData('SuperHotWDMFreeStartWin', 0);
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SuperHotWDMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                    }
                                                }
                                            }
                                        }
                                ...
                                $reportWin = $totalWin;
                                $slotSettings->SetGameData('SuperHotWDMTotalWin', $totalWin);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $winString = implode(',', $lineWins);
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('SuperHotWDMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SuperHotWDMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('SuperHotWDMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('SuperHotWDMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                ...
                                break;
                            case '9MHKS5E2Z2':
                                $result_tmp[] = '{"a": "8PIOHLo2", "cid": "' . $reqCid . '", "id": ' . $reqId . ', "res": {"bank": "0", "credit": "' . round($slotSettings->GetBalance() * 100) . '", "flags": "0"}, "s": "ok"}';
                                break;
                            case 'VKYHTCZVTN':
                                $result_tmp[] = '{"a": "8PIOHLo2", "cid": "' . $reqCid . '", "id": ' . $reqId . ', "res": {"bank": "0", "credit": "' . round($slotSettings->GetBalance() * 100) . '", "flags": "0"}, "s": "ok"}';
                                break;
                            case 'KZHGWHDL9T':
                                $result_tmp[] = '{"a": "rWJBqTiq", "cid": "' . $reqCid . '", "id": ' . $reqId . ', "s": "ok"}';
                                break;
                            case '464529TBNS':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('SuperHotWDMTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $winState = 0;
                                ...
                                $result_tmp[] = '{"a": "JtxxRtC8", "cid": "' . $reqCid . '", "id": ' . $reqId . ', "res": {"bank": "0", "cprize": "0", "credit": "' . round($afterBalance * 100) . '", "flags": "' . $winFlag . '", "prize": "' . round($totalWin * 100) . '", "win": "' . $winState . '", "x": "0"}, "s": "ok"}';
                                break;
                            case 'W42THTUT96':
                                $result_tmp[] = '{"a": "aTjH2yIT", "cid": "' . $reqCid . '", "id": ' . $reqId . ', "res": {"flags": "0"}, "s": "ok"}';
                                break;
                        }
