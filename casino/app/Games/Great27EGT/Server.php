                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[20],"multiplier":1},"1":{"coef":[20],"multiplier":1},"2":{"coef":[20],"multiplier":1},"3":{"coef":[20],"multiplier":1},"4":{"coef":[160],"multiplier":1},"5":{"coef":[160],"multiplier":1},"6":{"coef":[200],"multiplier":1},"7":{"coef":[400],"multiplier":1},"8":{"coef":[600],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[6,1,1,1,4,0,0,0,4,5,8,2,2,2,3,3,3,5,2,2,2,5,8,2,2,2,7,1,1,1],[7,0,0,0,8,2,2,2,3,3,3,7,5,1,1,1,0,0,0,2,2,2,4,8,6,5,2,2,2,5],[0,0,0,2,2,2,1,1,1,6,4,0,0,0,6,7,3,3,3,5,1,1,1,5,7,2,2,2,5,8]],"lines":[10],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('Great27EGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('Great27EGTBonusWin', 0);
                                $slotSettings->SetGameData('Great27EGTFreeGames', 0);
                                $slotSettings->SetGameData('Great27EGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Great27EGTTotalWin', 0);
                                $slotSettings->SetGameData('Great27EGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('Great27EGTCurrentFreeGame') < $slotSettings->GetGameData('Great27EGTFreeGames') && $slotSettings->GetGameData('Great27EGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('Great27EGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('Great27EGTTotalWin', 0);
                                    $slotSettings->SetGameData('Great27EGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('Great27EGTCurrentFreeGame', $slotSettings->GetGameData('Great27EGTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $wsym[0] > 0 && $wsym[1] > 0 && $wsym[2] > 0 ) 
                                            {
                                                ...
                                                {
                                                    $cWin = $tWin;
                                                    $tmpStringWin = '{"card":' . $csym . ',"len":3,"count":' . $sMpl . ',"winPerCount":' . ($tWin0 * 100) . ',"multiplier":1,"winAmount":' . ($cWin / ($sMpl * $wildsymmpl) * 100) . ',"cells":[' . implode(',', $cells) . ']}';
                                                }
                                            }
                                            $totalWin += $cWin;
                                            $cWins[$j] = $cWin;
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('Great27EGTBonusWin', $slotSettings->GetGameData('Great27EGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('Great27EGTTotalWin', $slotSettings->GetGameData('Great27EGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('Great27EGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('Great27EGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('Great27EGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('Great27EGTFreeGames', $slotSettings->GetGameData('Great27EGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('Great27EGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('Great27EGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('Great27EGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('Great27EGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('Great27EGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('Great27EGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('Great27EGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('Great27EGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('Great27EGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('Great27EGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('Great27EGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('Great27EGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('Great27EGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('Great27EGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('Great27EGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('Great27EGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('Great27EGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('Great27EGTGambleStep', $slotSettings->GetGameData('Great27EGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('Great27EGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
