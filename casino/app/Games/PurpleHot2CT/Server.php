            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'create_session':
                                $slotSettings->SetGameData('PurpleHot2CTCards', [
                                    19, 
                                    44, 
                                    0, 
                                    37, 
                            ...
                            case 'init_game':
                                ...
                                $slotSettings->SetGameData('PurpleHot2CTBonusWin', 0);
                                $slotSettings->SetGameData('PurpleHot2CTFreeGames', 0);
                                $slotSettings->SetGameData('PurpleHot2CTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PurpleHot2CTCurrentFreeGame0', 0);
                                $slotSettings->SetGameData('PurpleHot2CTTotalWin', 0);
                                $slotSettings->SetGameData('PurpleHot2CTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    if( $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame') < $slotSettings->GetGameData('PurpleHot2CTFreeGames') && $slotSettings->GetGameData('PurpleHot2CTFreeGames') > 0 ) 
                                    {
                            ...
                            case 'poll':
                                ...
                                {
                                    $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance() - $slotSettings->GetGameData('PurpleHot2CTTotalWin')) * 100;
                                }
                            ...
                            case 'game_finished':
                                $slotSettings->SetGameData('PurpleHot2CTBonusWin', 0);
                                $slotSettings->SetGameData('PurpleHot2CTTotalWin', 0);
                                $responseData['game_finished'][0]['freeround_play'] = $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame0');
                            ...
                            case 'subgame_finished':
                                if( $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame0') < $slotSettings->GetGameData('PurpleHot2CTFreeGames') && $slotSettings->GetGameData('PurpleHot2CTFreeGames') > 0 ) 
                                {
                                    ...
                                    $response = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"result":' . json_encode($result) . ',"slotDenom":' . $slotSettings->GetGameData($slotSettings->slotId . 'Denom') . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('PurpleHot2CTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData('PurpleHot2CTFreeBalance') . ',"afterBalance":' . $slotSettings->GetGameData('PurpleHot2CTFreeBalance') . ',"bonusWin":' . $slotSettings->GetGameData('PurpleHot2CTBonusWin') . ',"totalWin":' . $reportWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Reels')) . '}}';
                                    if( $slotSettings->GetGameData('PurpleHot2CTFreeGames') <= $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame0') ) 
                                    {
                                        if( $reportWin > 0 ) 
                                        {
                                    ...
                                    $balanceInCents = $slotSettings->GetGameData('PurpleHot2CTFreeBalance');
                                    $responseData['subgame_finished'][0]['account_status']['credit'] = round($balanceInCents);
                                    $responseData['subgame_finished'][0]['freeround_play'] = $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame0');
                                    $responseData['subgame_finished'][1]['freeround_play'] = $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame0');
                            ...
                            case 'play':
                                ...
                                for( $spinLoop = 0; $spinLoop < $spinLoopLimit; $spinLoop++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] != 'freespin' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('PurpleHot2CTCurrentFreeGame0', 0);
                                        $slotSettings->SetGameData('PurpleHot2CTTotalWin', 0);
                                        $slotSettings->SetGameData('PurpleHot2CTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    ...
                                    {
                                        $slotSettings->SetGameData('PurpleHot2CTCurrentFreeGame', $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame') + 1);
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $slotSettings->SetGameData('PurpleHot2CTBonusWin', $slotSettings->GetGameData('PurpleHot2CTBonusWin') + $totalWin);
                                        $slotSettings->SetGameData('PurpleHot2CTTotalWin', $slotSettings->GetGameData('PurpleHot2CTTotalWin') + $totalWin);
                                        $balanceInCents = $slotSettings->GetGameData('PurpleHot2CTFreeBalance');
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('PurpleHot2CTTotalWin', $totalWin);
                                    }
                                    $fs = 0;
                                    if( $scattersCount >= 3 ) 
                                    {
                                    ...
                                    $winstring = '';
                                    $slotSettings->SetGameData('PurpleHot2CTGambleStep', 5);
                                    $hist = $slotSettings->GetGameData('PurpleHot2CTCards');
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $state = 'freespin';
                                        ...
                                        if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('PurpleHot2CTBonusWin') > 0 ) 
                                        {
                                            $responseData['play'][0]['result'][$spinLoop]['is_free'] = true;
                                    ...
                                    $responseData['play'][0]['result'][$spinLoop]['total_win'] = $slotSettings->GetGameData('PurpleHot2CTTotalWin') / $denom;
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"result":' . json_encode($responseData['play'][0]['result']) . ',"slotDenom":' . $denom . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('PurpleHot2CTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('PurpleHot2CTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('PurpleHot2CTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'gamble':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('PurpleHot2CTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                ...
                                $hist = $slotSettings->GetGameData('PurpleHot2CTCards');
                                $slotSettings->SetGameData('PurpleHot2CTGambleStep', $slotSettings->GetGameData('PurpleHot2CTGambleStep') - 1);
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isGambleWin = 0;
                                }
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('PurpleHot2CTGambleStep', 0);
                                    if( $postData['continue_params']['double_key'] != 'red' && $postData['continue_params']['double_key'] != 'black' ) 
                                    {
                                        if( $postData['continue_params']['double_key'] == 'diamond' ) 
                                        {
                                ...
                                $slotSettings->SetGameData('PurpleHot2CTCards', $hist);
