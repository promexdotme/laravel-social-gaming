                ...
                try
                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        case 'A/u350':
                            ...
                            break;
                        case 'A/u2517':
                            if( $slotSettings->GetGameData('HotChoiceAMFreeSelect') == 1 ) 
                            {
                                $slotSettings->SetGameData('HotChoiceAMFreeSelect', 0);
                                if( $tmpPar[1] == 0 ) 
                                {
                                    $slotSettings->SetGameData('HotChoiceAMFreeGames', 15);
                                    $slotSettings->SetGameData('HotChoiceAMFreeGamesMpl', 3);
                                    $fInfo = '';
                                }
                                else if( $tmpPar[1] == 1 ) 
                                {
                                    $slotSettings->SetGameData('HotChoiceAMFreeGames', 10);
                                    $slotSettings->SetGameData('HotChoiceAMFreeGamesMpl', 5);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('HotChoiceAMFreeGames', 6);
                                    $slotSettings->SetGameData('HotChoiceAMFreeGamesMpl', 9);
                                }
                                $tFree = dechex($slotSettings->GetGameData('HotChoiceAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('HotChoiceAMFreeGames') - $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $response = '105' . $slotSettings->GetGameData('HotChoiceAMChoice0') . $freeInfo . $slotSettings->GetGameData('HotChoiceAMChoice2') . $slotSettings->HexFormat($slotSettings->GetGameData('HotChoiceAMFreeGamesMpl')) . $slotSettings->GetGameData('HotChoiceAMChoice1');
                            }
                            break;
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('HotChoiceAMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('HotChoiceAMFreeGamesMpl', $lastEvent->serverResponse->FreeGamesMpl);
                                $slotSettings->SetGameData('HotChoiceAMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('HotChoiceAMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('HotChoiceAMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('HotChoiceAMTotalWin', 0);
                                $slotSettings->SetGameData('HotChoiceAMFreeBalance', 0);
                                $slotSettings->SetGameData('HotChoiceAMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('HotChoiceAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('HotChoiceAMFreeGames') - $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('HotChoiceAMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame') < $slotSettings->GetGameData('HotChoiceAMFreeGames') && $slotSettings->GetGameData('HotChoiceAMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('HotChoiceAMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('HotChoiceAMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('HotChoiceAMBonusWin', 0);
                                $slotSettings->SetGameData('HotChoiceAMFreeGames', 0);
                                $slotSettings->SetGameData('HotChoiceAMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('HotChoiceAMTotalWin', 0);
                                $slotSettings->SetGameData('HotChoiceAMFreeBalance', 0);
                                $slotSettings->SetGameData('HotChoiceAMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('HotChoiceAMCurrentFreeGame', $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame') + 1);
                                $bonusMpl = $slotSettings->GetGameData('HotChoiceAMFreeGamesMpl');
                            }
                            $linesId[0] = [
                                2, 
                                2, 
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('HotChoiceAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('HotChoiceAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('HotChoiceAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":4,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('HotChoiceAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":5,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('HotChoiceAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":[' . ($linesId[$k][4] - 1) . ',"' . $s[4] . '"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('HotChoiceAMFreeGames') <= $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('HotChoiceAMBonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('HotChoiceAMBonusWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('HotChoiceAMBonusWin', $slotSettings->GetGameData('HotChoiceAMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('HotChoiceAMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('HotChoiceAMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('HotChoiceAMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('HotChoiceAMFreeGames', $slotSettings->GetGameData('HotChoiceAMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('HotChoiceAMFreeGamesMpl', 3);
                                    $slotSettings->SetGameData('HotChoiceAMFreeSelect', 1);
                                    $slotSettings->SetGameData('HotChoiceAMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('HotChoiceAMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('HotChoiceAMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"FreeGamesMpl":' . $slotSettings->GetGameData('HotChoiceAMFreeGamesMpl') . ',"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('HotChoiceAMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('HotChoiceAMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('HotChoiceAMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('HotChoiceAMFreeGames') - $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('HotChoiceAMFreeGames') <= $slotSettings->GetGameData('HotChoiceAMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('HotChoiceAMBonusWin');
                                $bonusMpl = 0;
                            }
                            else
                            {
                            ...
                            $slotSettings->SetGameData('HotChoiceAMChoice0', '010' . $balanceFormated . $slotSettings->HexFormat(round($totalWin * 100)) . $reelSate . $bet_h . $ln_h);
                            $slotSettings->SetGameData('HotChoiceAMChoice1', $freeWinState);
                            $slotSettings->SetGameData('HotChoiceAMChoice2', '1010' . $reelSate . $fixedLinesFormated0 . $winLinesFormated . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards')) . '#');
                            ...
                            $slotSettings->SetGameData('HotChoiceAMDoubleAnswer', $reelSate . $bet_h . $ln_h . '1010101010101010101010' . $fixedLinesFormated0 . $winLinesFormated);
                            $slotSettings->SetGameData('HotChoiceAMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('HotChoiceAMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('HotChoiceAMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('HotChoiceAMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('HotChoiceAMCollect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('HotChoiceAMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('HotChoiceAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('HotChoiceAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('HotChoiceAMDoubleWin', $winall);
                            $slotSettings->SetGameData('HotChoiceAMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('HotChoiceAMCollectP0');
                            $slotSettings->SetGameData('HotChoiceAMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('HotChoiceAMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('HotChoiceAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('HotChoiceAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
