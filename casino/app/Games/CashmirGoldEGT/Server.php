                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,25,125],"multiplier":1},"1":{"coef":[5,25,125],"multiplier":1},"2":{"coef":[5,25,125],"multiplier":1},"3":{"coef":[10,50,250],"multiplier":1},"4":{"coef":[10,50,250],"multiplier":1},"5":{"coef":[10,50,250],"multiplier":1},"6":{"coef":[5,25,125,500],"multiplier":1},"7":{"coef":[10,50,250,1000],"multiplier":1},"8":{"coef":[10,50,250,1000],"multiplier":1},"9":{"coef":[25,125,1000,5000],"multiplier":1},"10":{"coef":[5,25,250],"multiplier":1},"11":{"coef":[],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[1,4,0,7,5,2,1,3,0,9,7,5,2,1,4,1,0,7,8,6],[4,8,0,2,1,1,6,7,2,1,0,7,1,4,1,6,7,9,2,3],[8,3,4,1,2,1,9,1,5,8,6,1,2,7,1,3,9,6,5,7],[9,2,3,8,4,1,1,4,1,7,5,9,0,6,1,7,8,3,1,5],[1,2,6,9,7,4,8,3,5,1,7,0,6,1,9,1,3,1,7,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('CashmirGoldEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('CashmirGoldEGTBonusWin', 0);
                                $slotSettings->SetGameData('CashmirGoldEGTFreeGames', 0);
                                $slotSettings->SetGameData('CashmirGoldEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('CashmirGoldEGTTotalWin', 0);
                                $slotSettings->SetGameData('CashmirGoldEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('CashmirGoldEGTCurrentFreeGame') < $slotSettings->GetGameData('CashmirGoldEGTFreeGames') && $slotSettings->GetGameData('CashmirGoldEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('CashmirGoldEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('CashmirGoldEGTTotalWin', 0);
                                    $slotSettings->SetGameData('CashmirGoldEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('CashmirGoldEGTCurrentFreeGame', $slotSettings->GetGameData('CashmirGoldEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('CashmirGoldEGTBonusWin', $slotSettings->GetGameData('CashmirGoldEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('CashmirGoldEGTTotalWin', $slotSettings->GetGameData('CashmirGoldEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('CashmirGoldEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('CashmirGoldEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('CashmirGoldEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('CashmirGoldEGTFreeGames', $slotSettings->GetGameData('CashmirGoldEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('CashmirGoldEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('CashmirGoldEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('CashmirGoldEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('CashmirGoldEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('CashmirGoldEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('CashmirGoldEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('CashmirGoldEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('CashmirGoldEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('CashmirGoldEGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(0, 6));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('CashmirGoldEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('CashmirGoldEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('CashmirGoldEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('CashmirGoldEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('CashmirGoldEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('CashmirGoldEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('CashmirGoldEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('CashmirGoldEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('CashmirGoldEGTGambleStep', $slotSettings->GetGameData('CashmirGoldEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('CashmirGoldEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
