            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( $postData['act'] == '464529TBNS' && $slotSettings->GetGameData('MagicFruits27WDTotalWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['act'] ) 
                        {
                            ...
                            case 'WXMNPP7H8F':
                                ...
                                $slotSettings->SetGameData('MagicFruits27WDBonusWin', 0);
                                $slotSettings->SetGameData('MagicFruits27WDFreeGames', 0);
                                $slotSettings->SetGameData('MagicFruits27WDCurrentFreeGame', 0);
                                $slotSettings->SetGameData('MagicFruits27WDTotalWin', 0);
                                $slotSettings->SetGameData('MagicFruits27WDFreeBalance', 0);
                                $slotSettings->SetGameData('MagicFruits27WDFreeStartWin', 0);
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('MagicFruits27WDBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                    }
                                                }
                                            }
                                        }
                                ...
                                $reportWin = $totalWin;
                                $slotSettings->SetGameData('MagicFruits27WDTotalWin', $totalWin);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $winString = implode(',', $lineWins);
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('MagicFruits27WDFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('MagicFruits27WDCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('MagicFruits27WDBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('MagicFruits27WDFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case '464529TBNS':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('MagicFruits27WDTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $winState = 0;
