            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( isset($postData['ID']) && $postData['ID'] == '46123' ) 
                        {
                            $result_tmp = [];
                            $umid = '0';
                            $postData['slotEvent'] = 'bet';
                            $bonusMpl = 1;
                            $slotSettings->SetGameData('JiXiang8PTMBonusWin', 0);
                            $slotSettings->SetGameData('JiXiang8PTMFreeGames', 0);
                            $slotSettings->SetGameData('JiXiang8PTMCurrentFreeGame', 0);
                            $slotSettings->SetGameData('JiXiang8PTMTotalWin', 0);
                            $slotSettings->SetGameData('JiXiang8PTMFreeBalance', 0);
                            $slotSettings->SetGameData('JiXiang8PTMFreeStartWin', 0);
                            $linesId[0] = [
                                2, 
                                2, 
                                2, 
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('JiXiang8PTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('JiXiang8PTMBonusWin', $slotSettings->GetGameData('JiXiang8PTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('JiXiang8PTMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('JiXiang8PTMTotalWin', $totalWin);
                            ...
                            $symStr = $reels['reel1'][0] . ',' . $reels['reel2'][0] . ',' . $reels['reel3'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel2'][1] . ',' . $reels['reel3'][1] . ',' . $reels['reel1'][2] . ',' . $reels['reel2'][2] . ',' . $reels['reel3'][2];
                            $posStr = $reels['rp'][0] . ',' . $reels['rp'][3] . ',' . $reels['rp'][6] . ',' . $reels['rp'][1] . ',' . $reels['rp'][4] . ',' . $reels['rp'][7] . ',' . $reels['rp'][2] . ',' . $reels['rp'][5] . ',' . $reels['rp'][8];
                            $result_tmp[] = '3:::{"data":{"placeBetsResponse":{"gameId":2476842377,"newBalance":' . $balanceInCents . '},"loadResultsResponse":{"gameId":2476842377,"drawState":{"drawId":0,"state":"settling","wCapMaxWin":1000000.0,"winLines":{"display":"' . $symStr . '","reelset":0,"runningTotal":0.0,"stops":"' . $posStr . '"},"replayInfo":{"foItems":"' . $posStr . '"},"bet":{"pick":"' . $postData['numLines'] . '","seq":0,"stake":' . $postData['bet'] . ',"type":"line","won":"' . $won . '"}}}},"ID":47382,"umid":37}';
                            $result_tmp[] = '3:::{"data":{"typeBalance":0,"currency":"' . $slotSettings->slotCurrency . '","balanceInCents":' . ($balanceInCents * 100) . ',"deltaBalanceInCents":1},"ID":40085}';
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('JiXiang8PTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('JiXiang8PTMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('JiXiang8PTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('JiXiang8PTMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '40066':
                                ...
                                $result_tmp[] = '3:::{"data":{"funNoticeGames":0,"funNoticePayouts":0,"gameGroup":"gtsjxb","minBet":0,"maxBet":0,"minPosBet":0,"maxPosBet":50000,"coinSizes":[' . implode(',', $gameBets) . ']},"ID":40025,"umid":21}';
                                break;
                            case '40036':
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    {
                                        $slotSettings->SetGameData($slotSettings->slotId . 'brokenGames', 'gtsjxb');
                                    }
                                }
                            ...
                            case '40030':
                                ...
                                break;
                            case '46090':
                                $result_tmp[] = '3:::{"data":{"gameId":2476582349,"drawState":{"drawId":0}},"ID":47380,"umid":30}';
                                break;
                            case '46124':
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance());
                                $result_tmp[] = '3:::{"data":{"settleBetsResponse":{"gameId":2476842377,"newBalance":' . $balanceInCents . '},"closeGameResponse":{},"openGameResponse":{"gameId":2476843970,"drawState":{"drawId":0}}},"ID":47383,"umid":39}';
                                break;
                            case '48300':
