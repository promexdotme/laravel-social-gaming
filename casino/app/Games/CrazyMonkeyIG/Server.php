            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['action'] == 'risk' && $slotSettings->GetGameData('CrazyMonkeyIGWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['action'] ) 
                        {
                            case 'init':
                                ...
                                break;
                            case 'status':
                                if( !$slotSettings->HasGameData('CrazyMonkeyIGLines') || !$slotSettings->HasGameData('CrazyMonkeyIGBet') ) 
                                {
                                    $slotSettings->SetGameData('CrazyMonkeyIGLines', 1);
                                    $slotSettings->SetGameData('CrazyMonkeyIGBet', 1);
                                }
                                $balance = $slotSettings->GetBalance();
                                if( $slotSettings->GetGameData('CrazyMonkeyIGWin') > 0 ) 
                                {
                                    $balance = $balance - $slotSettings->GetGameData('CrazyMonkeyIGWin');
                                }
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $slotSettings->GetGameData('CrazyMonkeyIGBet') . ',"LastFuse":false,"LastLines":' . $slotSettings->GetGameData('CrazyMonkeyIGLines') . ',"MaxWin":675000,"RawCredit":"' . $balance . '","Reels":[[7,1,5,3,8],[5,3,6,7,7],[6,8,7,8,1]],"Win":0,"args":{},"cmd":"status","time":"20181111T111512"}';
                                break;
                            case 'start':
                                $balance = $slotSettings->GetBalance();
                                $bet = $postData['bet'];
                                ...
                                $slotSettings->SetGameData('CrazyMonkeyIGReels', $reelStr);
                                $slotSettings->SetGameData('CrazyMonkeyIGWin', $totalWin);
                                $slotSettings->SetGameData('CrazyMonkeyIGRisk', 1);
                                $slotSettings->SetGameData('CrazyMonkeyIGLines', $lines);
                                $slotSettings->SetGameData('CrazyMonkeyIGBet', $bet);
                                $slotSettings->SetGameData('CrazyMonkeyIGDealerCard', $card);
                                $rState = 'bet';
                                if( $isBonus ) 
                                {
                                    $rState = 'BG';
                            ...
                            case 'finish':
                                $balance = $slotSettings->GetBalance();
                                $slotSettings->SetGameData('CrazyMonkeyIGWin', 0);
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Credit":' . floor($balance) . ',"RawCredit":' . floor($balance) . ',"Reels":[' . $slotSettings->GetGameData('CrazyMonkeyIGReels') . '],"args":{},"cmd":"finish","time":"20181111T171830"}';
                                break;
                            case 'risk':
                                $onTabCards = [];
