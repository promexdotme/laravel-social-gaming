                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,25,125],"multiplier":1},"1":{"coef":[5,25,125],"multiplier":1},"2":{"coef":[5,25,125],"multiplier":1},"3":{"coef":[10,50,250],"multiplier":1},"4":{"coef":[10,50,250],"multiplier":1},"5":{"coef":[10,50,250],"multiplier":1},"6":{"coef":[25,75,500],"multiplier":1},"7":{"coef":[25,75,500],"multiplier":1},"8":{"coef":[2,25,125,750],"multiplier":1},"9":{"coef":[5,50,250,1000],"multiplier":1},"10":{"coef":[5,50,250,1000],"multiplier":1},"11":{"coef":[10,125,1000,5000],"multiplier":1},"12":{"coef":[2,5,25,250],"multiplier":1},"13":{"coef":[],"multiplier":1},"14":{"coef":[],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[2,4,10,3,2,7,0,6,1,11,9,0,5,12,1,2,10,4,5,8],[7,8,0,11,3,2,4,6,0,5,7,0,9,1,5,0,1,2,12,1],[0,3,9,12,1,4,6,2,0,8,3,0,6,1,11,10,2,7,4,5],[3,6,11,1,7,0,4,8,0,2,6,12,5,7,3,6,1,4,10,2],[2,10,3,0,1,7,0,12,6,2,9,4,11,3,0,6,5,1,8,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('SummerBlissEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('SummerBlissEGTBonusWin', 0);
                                $slotSettings->SetGameData('SummerBlissEGTFreeGames', 0);
                                $slotSettings->SetGameData('SummerBlissEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SummerBlissEGTTotalWin', 0);
                                $slotSettings->SetGameData('SummerBlissEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('SummerBlissEGTCurrentFreeGame') < $slotSettings->GetGameData('SummerBlissEGTFreeGames') && $slotSettings->GetGameData('SummerBlissEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('SummerBlissEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('SummerBlissEGTTotalWin', 0);
                                    $slotSettings->SetGameData('SummerBlissEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SummerBlissEGTCurrentFreeGame', $slotSettings->GetGameData('SummerBlissEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('SummerBlissEGTBonusWin', $slotSettings->GetGameData('SummerBlissEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('SummerBlissEGTTotalWin', $slotSettings->GetGameData('SummerBlissEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('SummerBlissEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SummerBlissEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('SummerBlissEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('SummerBlissEGTFreeGames', $slotSettings->GetGameData('SummerBlissEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('SummerBlissEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('SummerBlissEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('SummerBlissEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('SummerBlissEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('SummerBlissEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SummerBlissEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('SummerBlissEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('SummerBlissEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('SummerBlissEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('SummerBlissEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('SummerBlissEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('SummerBlissEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('SummerBlissEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('SummerBlissEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('SummerBlissEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('SummerBlissEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('SummerBlissEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('SummerBlissEGTGambleStep', $slotSettings->GetGameData('SummerBlissEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('SummerBlissEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('SummerBlissEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('SummerBlissEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
