            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            $result_tmp = [];
                            if( $slotSettings->GetGameData('SilentSamuraiPTBonusStep') == 0 ) 
                            {
                                $WinArr = [
                                    2500, 
                                    2200, 
                                ...
                                shuffle($WinArr);
                                $slotSettings->SetGameData('SilentSamuraiPTBonusOpt', $WinArr);
                                $curWin = $WinArr[$postData['index']];
                                if( $curWin > 10 ) 
                                {
                                    $slotSettings->SetGameData('SilentSamuraiPTFreeGames', $slotSettings->GetGameData('SilentSamuraiPTFreeGames') + ($curWin / 100));
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SilentSamuraiPTFreeMpl', $slotSettings->GetGameData('SilentSamuraiPTFreeMpl') + $curWin);
                                }
                                $result_tmp[] = '3:::{"data":{"prizes":[' . $curWin . '],"windowId":"2Hjr3k"},"ID":40241,"umid":474}';
                                $slotSettings->SetGameData('SilentSamuraiPTBonusStep', 1);
                            }
                            else if( $slotSettings->GetGameData('SilentSamuraiPTBonusStep') == 1 ) 
                            {
                                $WinArr = $slotSettings->GetGameData('SilentSamuraiPTBonusOpt');
                                $curWin = $WinArr[$postData['index']];
                                if( $curWin > 10 ) 
                                {
                                    $slotSettings->SetGameData('SilentSamuraiPTFreeGames', $slotSettings->GetGameData('SilentSamuraiPTFreeGames') + ($curWin / 100));
                                ...
                                $slotSettings->SetGameData('SilentSamuraiPTBonusStep', 2);
                            }
                            $umid = 0;
                        }
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('SilentSamuraiPTBonusWin', 0);
                                $slotSettings->SetGameData('SilentSamuraiPTFreeGames', 0);
                                $slotSettings->SetGameData('SilentSamuraiPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SilentSamuraiPTTotalWin', 0);
                                $slotSettings->SetGameData('SilentSamuraiPTFreeBalance', 0);
                                $slotSettings->SetGameData('SilentSamuraiPTFreeStartWin', 0);
                                $slotSettings->SetGameData('SilentSamuraiPTFreeMpl', $slotSettings->slotFreeMpl);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('SilentSamuraiPTCurrentFreeGame', $slotSettings->GetGameData('SilentSamuraiPTCurrentFreeGame') + 1);
                                $bonusMpl = $slotSettings->GetGameData('SilentSamuraiPTFreeMpl');
                            }
                            $linesId = [];
                            $linesId[0] = [
                                2, 
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SilentSamuraiPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SilentSamuraiPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SilentSamuraiPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":4,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SilentSamuraiPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":5,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SilentSamuraiPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":[' . ($linesId[$k][4] - 1) . ',"' . $s[4] . '"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('SilentSamuraiPTBonusWin', $slotSettings->GetGameData('SilentSamuraiPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('SilentSamuraiPTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('SilentSamuraiPTTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 2 ) 
                            {
                                $slotSettings->SetGameData('SilentSamuraiPTBonusStep', 0);
                                $slotSettings->SetGameData('SilentSamuraiPTFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('SilentSamuraiPTBonusWin', 0);
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('SilentSamuraiPTFreeGames', $slotSettings->GetGameData('SilentSamuraiPTFreeGames') + 9);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SilentSamuraiPTFreeGames', 1);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('SilentSamuraiPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SilentSamuraiPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('SilentSamuraiPTBonusWin') . ',"FreeMpl":' . $slotSettings->GetGameData('SilentSamuraiPTFreeMpl') . ',"freeStartWin":' . $slotSettings->GetGameData('SilentSamuraiPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            case '40743':
                                ...
                                shuffle($wins);
                                $curWin = $wins[0];
                                if( $curWin == 0 && $slotSettings->GetGameData('SilentSamuraiPTFreeGames') == 0 ) 
                                {
                                    $wins[0] = 100;
                                    $curWin = $wins[0];
                                }
                                ...
                                if( $curWin == 0 || $slotSettings->GetGameData('SilentSamuraiPTFreeGames') >= 28 || $slotSettings->GetGameData('SilentSamuraiPTFreeMpl') >= 8 ) 
                                {
                                    $slotSettings->SetGameData('SilentSamuraiPTBonusStep', 2);
                                    for( $i = 0; $i < 21; $i++ ) 
                                    {
