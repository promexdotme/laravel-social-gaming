                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('BillysGameAMLines', $curLines);
                                $freeMpl = '11';
                                ...
                                $tFree = dechex($slotSettings->GetGameData('BillysGameAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('BillysGameAMFreeGames') - $slotSettings->GetGameData('BillysGameAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('BillysGameAMBonusWin') * $floatBet);
                            ...
                            {
                                $slotSettings->SetGameData('BillysGameAMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('BillysGameAMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $wsym[0] > 0 && $wsym[1] > 0 && $wsym[2] > 0 ) 
                                            {
                                                ...
                                                $tWin = $slotSettings->Paytable['SYM_' . $csym][3] * $betLine * $sMpl * $doubleMpl;
                                                if( $cWin < $tWin ) 
                                                {
                                                    $cWin = $tWin;
                                                }
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('BillysGameAMFreeGames') <= $slotSettings->GetGameData('BillysGameAMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('BillysGameAMBonusWin') + $totalWin > 0 ) 
                            {
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"symDouble":' . $symDouble . ',"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('BillysGameAMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BillysGameAMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('BillysGameAMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('BillysGameAMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            for( $i = 0; $i < 9; $i++ ) 
                            {
                                $cWins[$i] = $cWins[$i] / $betLine / $bonusMpl / $doubleMpl * 10;
                            ...
                            $response = '1' . $gameState . '010' . $balanceFormated . $slotSettings->HexFormat(round($totalWin * 100)) . $reelSate . $bet_h . '09101010101010101010101009' . $winLinesFormated . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards')) . '#101010';
                            $response .= ('_' . json_encode($reels));
                            $slotSettings->SetGameData('BillysGameAMDoubleAnswer', $reelSate . $bet_h . '09101010101010101010101009' . $winLinesFormated);
                            ...
                            $slotSettings->SetGameData('BillysGameAMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('BillysGameAMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                        ...
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('BillysGameAMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('BillysGameAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('BillysGameAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('BillysGameAMDoubleWin', $winall);
                            $slotSettings->SetGameData('BillysGameAMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('BillysGameAMCollectP0');
                            $slotSettings->SetGameData('BillysGameAMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('BillysGameAMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('BillysGameAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('BillysGameAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
