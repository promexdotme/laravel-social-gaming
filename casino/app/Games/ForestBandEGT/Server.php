                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[10,20,50],"multiplier":1},"1":{"coef":[10,20,50],"multiplier":1},"2":{"coef":[10,20,50],"multiplier":1},"3":{"coef":[10,40,100],"multiplier":1},"4":{"coef":[20,50,150],"multiplier":1},"5":{"coef":[20,50,150],"multiplier":1},"6":{"coef":[50,125,400],"multiplier":1},"7":{"coef":[50,125,400],"multiplier":1},"8":{"coef":[10,75,200,750],"multiplier":1},"9":{"coef":[10,75,400,1000],"multiplier":1},"11":{"coef":[5],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[2,4,1,3,2,7,0,6,11,4,9,0,5,6,1,2,11,4,5,8],[7,8,0,1,3,10,4,6,0,5,7,0,9,1,5,0,1,2,5,1],[0,3,9,7,1,0,6,11,0,8,3,0,6,1,4,10,2,7,4,5],[3,6,0,10,7,0,4,8,0,2,6,8,5,7,3,6,9,4,1,2],[2,10,3,0,11,7,0,5,6,2,9,4,7,3,1,6,5,1,8,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('ForestBandEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('ForestBandEGTBonusWin', 0);
                                $slotSettings->SetGameData('ForestBandEGTFreeGames', 0);
                                $slotSettings->SetGameData('ForestBandEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('ForestBandEGTTotalWin', 0);
                                $slotSettings->SetGameData('ForestBandEGTFreeBalance', 0);
                                $slotSettings->SetGameData('ForestBandEGTHolds', []);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('ForestBandEGTCurrentSeq', $lastEvent->serverResponse->curSeq);
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('ForestBandEGTCurrentFreeGame') < $slotSettings->GetGameData('ForestBandEGTFreeGames') && $slotSettings->GetGameData('ForestBandEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('ForestBandEGTTotalWin', 0);
                                    $slotSettings->SetGameData('ForestBandEGTHolds', []);
                                    $slotSettings->SetGameData('ForestBandEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', '');
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('ForestBandEGTCurrentFreeGame', $slotSettings->GetGameData('ForestBandEGTCurrentFreeGame') + 1);
                                ...
                                if( $winType == 'bonus' ) 
                                {
                                    ...
                                    if( $wlb != 'NULL' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('ForestBandEGTCurrentSeq', $curSeq);
                                        if( $curSeqWin > 0 ) 
                                        {
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    {
                                        $curSeq = (array)$slotSettings->GetGameData('ForestBandEGTCurrentSeq');
                                        $curSeq['result'] = (array)$curSeq['result'];
                                        $curSeq['result'][$slotSettings->GetGameData('ForestBandEGTCurrentFreeGame')] = (array)$curSeq['result'][$slotSettings->GetGameData('ForestBandEGTCurrentFreeGame')];
                                        $curSeq['result'][$slotSettings->GetGameData('ForestBandEGTCurrentFreeGame')]['reels'] = (array)$curSeq['result'][$slotSettings->GetGameData('ForestBandEGTCurrentFreeGame')]['reels'];
                                        $reels = $curSeq['result'][$slotSettings->GetGameData('ForestBandEGTCurrentFreeGame')]['reels'];
                                    }
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        ...
                                        {
                                            $t = explode(',', $holds[$r]);
                                            $reels['reel' . ((int)$t[0] + 1)][(int)$t[1]] = '10';
                                        }
                                    }
                                    for( $k = 0; $k < $lines; $k++ ) 
                                    {
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('ForestBandEGTBonusWin', $slotSettings->GetGameData('ForestBandEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('ForestBandEGTTotalWin', $slotSettings->GetGameData('ForestBandEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('ForestBandEGTFreeBalance');
                                    $slotSettings->SetGameData($slotSettings->slotId . 'Holds', $holdsNew);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('ForestBandEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                ...
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('ForestBandEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('ForestBandEGTFreeGames', $slotSettings->GetGameData('ForestBandEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('ForestBandEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('ForestBandEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('ForestBandEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', ',"firstSpin":{"expand":[],"reels":[' . $curReels . '],"lines":[],"scatters":[' . $scattersStr . ']}');
                                    $fs = $slotSettings->GetGameData('ForestBandEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"curSeq":' . json_encode($curSeq) . ',"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"holdPositions": ' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Holds')) . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('ForestBandEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('ForestBandEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('ForestBandEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('ForestBandEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('ForestBandEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('ForestBandEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('ForestBandEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('ForestBandEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('ForestBandEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('ForestBandEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('ForestBandEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('ForestBandEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('ForestBandEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('ForestBandEGTGambleStep', $slotSettings->GetGameData('ForestBandEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('ForestBandEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('ForestBandEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('ForestBandEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
