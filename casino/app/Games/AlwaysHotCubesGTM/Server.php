                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('AlwaysHotCubesGTMDealerCard');
                            $totalWin = $slotSettings->GetGameData('AlwaysHotCubesGTMTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('AlwaysHotCubesGTMTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('AlwaysHotCubesGTMDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('AlwaysHotCubesGTMBonusWin', 0);
                                $slotSettings->SetGameData('AlwaysHotCubesGTMFreeGames', 0);
                                $slotSettings->SetGameData('AlwaysHotCubesGTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('AlwaysHotCubesGTMTotalWin', 0);
                                $slotSettings->SetGameData('AlwaysHotCubesGTMFreeBalance', 0);
                            }
                            else
                            {
                                $slotSettings->SetGameData('AlwaysHotCubesGTMCurrentFreeGame', $slotSettings->GetGameData('AlwaysHotCubesGTMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotCubesGTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotCubesGTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"]}';
                                                }
                                            }
                                            $s[0] = $reels['reel1'][$linesId[$k][0] - 1];
                                            $s[1] = $reels['reel2'][$linesId[$k][1] - 1];
                                            ...
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotCubesGTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('AlwaysHotCubesGTMFreeGames') <= $slotSettings->GetGameData('AlwaysHotCubesGTMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('AlwaysHotCubesGTMTotalWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('AlwaysHotCubesGTMTotalWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('AlwaysHotCubesGTMBonusWin', $slotSettings->GetGameData('AlwaysHotCubesGTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('AlwaysHotCubesGTMTotalWin', $slotSettings->GetGameData('AlwaysHotCubesGTMTotalWin') + $totalWin);
                                $totalWin = $slotSettings->GetGameData('AlwaysHotCubesGTMBonusWin');
                                $Balance = $slotSettings->GetGameData('AlwaysHotCubesGTMFreeBalance');
                            }
                            else
                            {
                                $slotSettings->SetGameData('AlwaysHotCubesGTMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('AlwaysHotCubesGTMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('AlwaysHotCubesGTMFreeBalance', $Balance);
                                    $slotSettings->SetGameData('AlwaysHotCubesGTMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('AlwaysHotCubesGTMFreeGames', $slotSettings->GetGameData('AlwaysHotCubesGTMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AlwaysHotCubesGTMFreeBalance', $Balance);
                                    $slotSettings->SetGameData('AlwaysHotCubesGTMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('AlwaysHotCubesGTMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('AlwaysHotCubesGTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AlwaysHotCubesGTMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
