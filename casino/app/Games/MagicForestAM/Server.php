                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('MagicForestAMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('MagicForestAMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('MagicForestAMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('MagicForestAMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('MagicForestAMTotalWin', 0);
                                $slotSettings->SetGameData('MagicForestAMFreeBalance', 0);
                                $slotSettings->SetGameData('MagicForestAMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('MagicForestAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('MagicForestAMFreeGames') - $slotSettings->GetGameData('MagicForestAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('MagicForestAMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('MagicForestAMCurrentFreeGame') < $slotSettings->GetGameData('MagicForestAMFreeGames') && $slotSettings->GetGameData('MagicForestAMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('MagicForestAMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('MagicForestAMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('MagicForestAMBonusWin', 0);
                                $slotSettings->SetGameData('MagicForestAMFreeGames', 0);
                                $slotSettings->SetGameData('MagicForestAMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('MagicForestAMTotalWin', 0);
                                $slotSettings->SetGameData('MagicForestAMFreeBalance', 0);
                                $slotSettings->SetGameData('MagicForestAMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('MagicForestAMCurrentFreeGame', $slotSettings->GetGameData('MagicForestAMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('MagicForestAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('MagicForestAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('MagicForestAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":4,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('MagicForestAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":5,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('MagicForestAMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":[' . ($linesId[$k][4] - 1) . ',"' . $s[4] . '"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('MagicForestAMFreeGames') <= $slotSettings->GetGameData('MagicForestAMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('MagicForestAMBonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('MagicForestAMBonusWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('MagicForestAMBonusWin', $slotSettings->GetGameData('MagicForestAMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('MagicForestAMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('MagicForestAMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('MagicForestAMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('MagicForestAMFreeGames', $slotSettings->GetGameData('MagicForestAMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('MagicForestAMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('MagicForestAMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('MagicForestAMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('MagicForestAMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('MagicForestAMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('MagicForestAMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('MagicForestAMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('MagicForestAMFreeGames') - $slotSettings->GetGameData('MagicForestAMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('MagicForestAMFreeGames') <= $slotSettings->GetGameData('MagicForestAMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('MagicForestAMBonusWin');
                                $bonusMpl = $slotSettings->slotFreeMpl;
                            }
                            else
                            {
                            ...
                            $slotSettings->SetGameData('MagicForestAMDoubleAnswer', $reelSate . $bet_h . $ln_h . '1010101010101010101010' . $fixedLinesFormated0 . $winLinesFormated);
                            $slotSettings->SetGameData('MagicForestAMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('MagicForestAMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('MagicForestAMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('MagicForestAMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('MagicForestAMCollect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('MagicForestAMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('MagicForestAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('MagicForestAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('MagicForestAMDoubleWin', $winall);
                            $slotSettings->SetGameData('MagicForestAMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('MagicForestAMCollectP0');
                            $slotSettings->SetGameData('MagicForestAMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('MagicForestAMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('MagicForestAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('MagicForestAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
