                    ...
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('FlameOfOlympusATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('FlameOfOlympusATMpl') . ',"add":0,"total":' . $slotSettings->GetGameData('FlameOfOlympusATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('FlameOfOlympusATBonusWin') * 100) . '';
                            }
                            else
                            {
                                $slotSettings->SetGameData('FlameOfOlympusATTotalWin', $totalWin);
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('FlameOfOlympusATFreeGames') . ',"Mpl":' . $slotSettings->GetGameData('FlameOfOlympusATMpl') . ',"currentFreeGames":' . $slotSettings->GetGameData('FlameOfOlympusATCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":{},"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            $reelStrArr = [];
                            for( $i = 1; $i <= 5; $i++ ) 
                            {
