            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    if( $postData['slotEvent'] == 'doInit' ) 
                    {
                        ...
                        $fsInfo = '';
                        $slotSettings->SetGameData('PekingLuckPMBonusWin', 0);
                        $slotSettings->SetGameData('PekingLuckPMFreeGames', 0);
                        $slotSettings->SetGameData('PekingLuckPMCurrentFreeGame', 0);
                        $slotSettings->SetGameData('PekingLuckPMTotalWin', 0);
                        $slotSettings->SetGameData($slotSettings->slotId . 'FreeBalance', $slotSettings->GetBalance());
                        $slotSettings->SetGameData('PekingLuckPMBonusState', 0);
                        $slotSettings->SetGameData('PekingLuckPMBonusMpl', 0);
                        if( $lastEvent != 'NULL' ) 
                        {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('PekingLuckPMDealerCard');
                        $totalWin = $slotSettings->GetGameData('PekingLuckPMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('PekingLuckPMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('PekingLuckPMDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('PekingLuckPMBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('PekingLuckPMFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('PekingLuckPMBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('PekingLuckPMBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('PekingLuckPMFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('PekingLuckPMBonusState', $bstate);
                    }
                    ...
                    {
                        if( $slotSettings->GetGameData('PekingLuckPMCurrentFreeGame') < $slotSettings->GetGameData('PekingLuckPMFreeGames') && $slotSettings->GetGameData('PekingLuckPMFreeGames') > 0 ) 
                        {
                        ...
                        {
                            $slotSettings->SetGameData('PekingLuckPMCurrentFreeGame', $slotSettings->GetGameData('PekingLuckPMCurrentFreeGame') + 1);
                            $bonusMpl = $slotSettings->GetGameData('PekingLuckPMBonusMpl');
                        }
                        $Balance = $slotSettings->GetBalance();
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('PekingLuckPMFreeGames') == 0 ) 
                            {
                                $slotSettings->SetGameData('PekingLuckPMBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('PekingLuckPMFreeGames', rand(10, 38));
                                $spinType = 'b&bw=1&bgid=0&bgt=28';
                            }
                            else
                            {
                                $slotSettings->SetGameData('PekingLuckPMFreeGames', $slotSettings->GetGameData('PekingLuckPMFreeGames') + $slotSettings->slotFreeCount);
                            }
                        }
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('PekingLuckPMBonusWin', $slotSettings->GetGameData('PekingLuckPMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('PekingLuckPMTotalWin', $slotSettings->GetGameData('PekingLuckPMTotalWin') + $totalWin);
                            $spinType = 's';
                            $Balance = $slotSettings->GetGameData('PekingLuckPMFreeBalance');
                            if( $slotSettings->GetGameData('PekingLuckPMFreeGames') <= $slotSettings->GetGameData('PekingLuckPMCurrentFreeGame') && $slotSettings->GetGameData('PekingLuckPMFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('PekingLuckPMFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('PekingLuckPMBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('PekingLuckPMBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('PekingLuckPMBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('PekingLuckPMBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('PekingLuckPMFreeGames') . '&fswin=' . $slotSettings->GetGameData('PekingLuckPMTotalWin') . '&fs=' . $slotSettings->GetGameData('PekingLuckPMCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('PekingLuckPMBonusWin'));
                            }
                        ...
                        else
                        {
                            ...
                            $slotSettings->SetGameData('PekingLuckPMTotalWin', $totalWin);
                            $slotSettings->SetGameData('PekingLuckPMBonusWin', $totalWin);
                        }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('PekingLuckPMBonusWin') . '&bstate=' . $slotSettings->GetGameData('PekingLuckPMBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('PekingLuckPMFreeGames') <= $slotSettings->GetGameData('PekingLuckPMCurrentFreeGame') && $slotSettings->GetGameData('PekingLuckPMFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('PekingLuckPMTotalWin', 0);
                            $slotSettings->SetGameData('PekingLuckPMBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('PekingLuckPMBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('PekingLuckPMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('PekingLuckPMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('PekingLuckPMBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('PekingLuckPMFreeBalance', $Balance);
                                $slotSettings->SetGameData('PekingLuckPMTotalWin', 0);
                                $slotSettings->SetGameData('PekingLuckPMBonusState', 0);
                                $slotSettings->SetGameData('PekingLuckPMBonusWin', $totalWin);
                            }
