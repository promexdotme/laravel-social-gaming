            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'create_session':
                                $slotSettings->SetGameData('TreasureHillCTCards', [
                                    19, 
                                    44, 
                                    0, 
                                    37, 
                            ...
                            case 'init_game':
                                ...
                                $slotSettings->SetGameData('TreasureHillCTBonusWin', 0);
                                $slotSettings->SetGameData('TreasureHillCTFreeGames', 0);
                                $slotSettings->SetGameData('TreasureHillCTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('TreasureHillCTCurrentFreeGame0', 0);
                                $slotSettings->SetGameData('TreasureHillCTTotalWin', 0);
                                $slotSettings->SetGameData('TreasureHillCTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    if( $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame') < $slotSettings->GetGameData('TreasureHillCTFreeGames') && $slotSettings->GetGameData('TreasureHillCTFreeGames') > 0 ) 
                                    {
                            ...
                            case 'poll':
                                ...
                                {
                                    $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance() - $slotSettings->GetGameData('TreasureHillCTTotalWin')) * 100;
                                }
                            ...
                            case 'game_finished':
                                $slotSettings->SetGameData('TreasureHillCTBonusWin', 0);
                                $slotSettings->SetGameData('TreasureHillCTTotalWin', 0);
                                $responseData['game_finished'][0]['freeround_play'] = $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame0');
                            ...
                            case 'subgame_finished':
                                if( $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame0') < $slotSettings->GetGameData('TreasureHillCTFreeGames') && $slotSettings->GetGameData('TreasureHillCTFreeGames') > 0 ) 
                                {
                                    ...
                                    $response = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"result":' . json_encode($result) . ',"slotDenom":' . $slotSettings->GetGameData($slotSettings->slotId . 'Denom') . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('TreasureHillCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData('TreasureHillCTFreeBalance') . ',"afterBalance":' . $slotSettings->GetGameData('TreasureHillCTFreeBalance') . ',"bonusWin":' . $slotSettings->GetGameData('TreasureHillCTBonusWin') . ',"totalWin":' . $reportWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Reels')) . '}}';
                                    if( $slotSettings->GetGameData('TreasureHillCTFreeGames') <= $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame0') ) 
                                    {
                                        if( $reportWin > 0 ) 
                                        {
                                    ...
                                    $balanceInCents = $slotSettings->GetGameData('TreasureHillCTFreeBalance');
                                    $responseData['subgame_finished'][0]['account_status']['credit'] = round($balanceInCents);
                                    $responseData['subgame_finished'][0]['freeround_play'] = $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame0');
                                    $responseData['subgame_finished'][1]['freeround_play'] = $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame0');
                            ...
                            case 'play':
                                ...
                                for( $spinLoop = 0; $spinLoop < $spinLoopLimit; $spinLoop++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] != 'freespin' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('TreasureHillCTCurrentFreeGame0', 0);
                                        $slotSettings->SetGameData('TreasureHillCTTotalWin', 0);
                                        $slotSettings->SetGameData('TreasureHillCTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    ...
                                    {
                                        $slotSettings->SetGameData('TreasureHillCTCurrentFreeGame', $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame') + 1);
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $slotSettings->SetGameData('TreasureHillCTBonusWin', $slotSettings->GetGameData('TreasureHillCTBonusWin') + $totalWin);
                                        $slotSettings->SetGameData('TreasureHillCTTotalWin', $slotSettings->GetGameData('TreasureHillCTTotalWin') + $totalWin);
                                        $balanceInCents = $slotSettings->GetGameData('TreasureHillCTFreeBalance');
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('TreasureHillCTTotalWin', $totalWin);
                                    }
                                    $fs = 0;
                                    if( $scattersCount >= 3 ) 
                                    {
                                        if( $slotSettings->GetGameData('TreasureHillCTFreeGames') > 0 ) 
                                        {
                                            $slotSettings->SetGameData('TreasureHillCTFreeGames', $slotSettings->GetGameData('TreasureHillCTFreeGames') + $slotSettings->slotFreeCount);
                                        }
                                        else
                                        {
                                            $slotSettings->SetGameData('TreasureHillCTFreeStartWin', $totalWin);
                                            $slotSettings->SetGameData('TreasureHillCTBonusWin', $totalWin);
                                            $slotSettings->SetGameData('TreasureHillCTFreeGames', $slotSettings->slotFreeCount);
                                        }
                                        $fs = $slotSettings->GetGameData('TreasureHillCTFreeGames');
                                    ...
                                    $winstring = '';
                                    $slotSettings->SetGameData('TreasureHillCTGambleStep', 5);
                                    $hist = $slotSettings->GetGameData('TreasureHillCTCards');
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $state = 'freespin';
                                        ...
                                        if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('TreasureHillCTBonusWin') > 0 ) 
                                        {
                                            $responseData['play'][0]['result'][$spinLoop]['is_free'] = true;
                                    ...
                                    $responseData['play'][0]['result'][$spinLoop]['total_win'] = $slotSettings->GetGameData('TreasureHillCTTotalWin') / $denom;
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"result":' . json_encode($responseData['play'][0]['result']) . ',"slotDenom":' . $denom . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('TreasureHillCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('TreasureHillCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeBalance') . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('TreasureHillCTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'gamble':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('TreasureHillCTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                ...
                                $hist = $slotSettings->GetGameData('TreasureHillCTCards');
                                $slotSettings->SetGameData('TreasureHillCTGambleStep', $slotSettings->GetGameData('TreasureHillCTGambleStep') - 1);
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isGambleWin = 0;
                                }
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('TreasureHillCTGambleStep', 0);
                                    if( $postData['continue_params']['double_key'] != 'red' && $postData['continue_params']['double_key'] != 'black' ) 
                                    {
                                        if( $postData['continue_params']['double_key'] == 'diamond' ) 
                                        {
                                ...
                                $slotSettings->SetGameData('TreasureHillCTCards', $hist);
