                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('FunkyMonkeyPTMMBonusWin', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTMMFreeGames', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTMMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTMMTotalWin', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTMMFreeBalance', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTMMFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('FunkyMonkeyPTMMCurrentFreeGame', $slotSettings->GetGameData('FunkyMonkeyPTMMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('FunkyMonkeyPTMMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( in_array($s[0], [
                                                7, 
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('FunkyMonkeyPTMMBonusWin', $slotSettings->GetGameData('FunkyMonkeyPTMMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('FunkyMonkeyPTMMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('FunkyMonkeyPTMMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('FunkyMonkeyPTMMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('FunkyMonkeyPTMMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyPTMMFreeGames', $slotSettings->GetGameData('FunkyMonkeyPTMMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('FunkyMonkeyPTMMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyPTMMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyPTMMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('FunkyMonkeyPTMMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('FunkyMonkeyPTMMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('FunkyMonkeyPTMMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('FunkyMonkeyPTMMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
