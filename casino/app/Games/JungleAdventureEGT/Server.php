                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,20,100],"multiplier":1},"1":{"coef":[5,20,100],"multiplier":1},"2":{"coef":[5,20,100],"multiplier":1},"3":{"coef":[10,50,200],"multiplier":1},"4":{"coef":[10,50,200],"multiplier":1},"5":{"coef":[5,20,75,250],"multiplier":1},"6":{"coef":[5,25,100,500],"multiplier":1},"7":{"coef":[5,50,200,1000],"multiplier":1},"8":{"coef":[10,100,1000,5000],"multiplier":1},"9":{"coef":[2,15,50],"multiplier":1},"10":{"coef":[0],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[3,2,8,1,4,0,6,1,2,7,0,1,4,9,2,3,5,0,1,6],[7,2,5,0,1,9,2,0,4,3,9,4,3,2,6,1,4,0,5,2],[1,8,2,5,4,6,0,10,1,5,2,6,4,7,0,4,5,9,3,6],[3,9,4,3,0,8,1,5,2,6,0,3,1,8,2,5,0,6,1,2],[6,3,5,0,6,4,1,3,2,6,0,7,9,3,4,1,8,4,2,7]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('JungleAdventureEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('JungleAdventureEGTBonusWin', 0);
                                $slotSettings->SetGameData('JungleAdventureEGTFreeGames', 0);
                                $slotSettings->SetGameData('JungleAdventureEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('JungleAdventureEGTTotalWin', 0);
                                $slotSettings->SetGameData('JungleAdventureEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('JungleAdventureEGTCurrentFreeGame') < $slotSettings->GetGameData('JungleAdventureEGTFreeGames') && $slotSettings->GetGameData('JungleAdventureEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('JungleAdventureEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('JungleAdventureEGTTotalWin', 0);
                                    $slotSettings->SetGameData('JungleAdventureEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('JungleAdventureEGTCurrentFreeGame', $slotSettings->GetGameData('JungleAdventureEGTCurrentFreeGame') + 1);
                                            ...
                                            for( $r = 1; $r <= 5; $r++ ) 
                                            {
                                                for( $p = 0; $p <= 2; $p++ ) 
                                                {
                                                    if( $reels['reel' . $r][$p] == '10' ) 
                                                    {
                                                        ...
                                                        $spec = '{"specialSymbol": 10, "cells": [2, ' . $p . '], "winAmount": 0, "totalWinMultiplier": ' . $bonusMpl0 . '}';
                                                        $totalWin = $slotSettings->GetGameData('JungleAdventureEGTBonusWin');
                                                    }
                                                }
                                            }
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('JungleAdventureEGTBonusWin', $slotSettings->GetGameData('JungleAdventureEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('JungleAdventureEGTTotalWin', $slotSettings->GetGameData('JungleAdventureEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('JungleAdventureEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('JungleAdventureEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('JungleAdventureEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('JungleAdventureEGTFreeGames', $slotSettings->GetGameData('JungleAdventureEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('JungleAdventureEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('JungleAdventureEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('JungleAdventureEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('JungleAdventureEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('JungleAdventureEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('JungleAdventureEGTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('JungleAdventureEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('JungleAdventureEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('JungleAdventureEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('JungleAdventureEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('JungleAdventureEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('JungleAdventureEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('JungleAdventureEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('JungleAdventureEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('JungleAdventureEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('JungleAdventureEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('JungleAdventureEGTGambleStep', $slotSettings->GetGameData('JungleAdventureEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('JungleAdventureEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('JungleAdventureEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('JungleAdventureEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
