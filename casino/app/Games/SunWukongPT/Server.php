                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( isset($postData['ID']) && $postData['ID'] == '46108' ) 
                        {
                            ...
                            $lines = $slotSettings->GetGameData('SunWukongPTLines');
                            $betLine = $slotSettings->GetGameData('SunWukongPTBet') / $lines;
                            $allWin = 0;
                            $spins = [];
                            $responseLog = [];
                            for( $s = 1; $s <= $fCount; $s++ ) 
                            {
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    $winTypeTmp = $slotSettings->GetSpinSettings('freespin', $slotSettings->GetGameData('SunWukongPTBet'));
                                ...
                                $slotSettings->SetGameData('SunWukongPTBonusWin', $slotSettings->GetGameData('SunWukongPTBonusWin') + $totalWin);
                                $spins[] = '{"drawId":' . ($s + 1) . ',"winLines":{"currentWin":' . $totalWin . ',"currentSpins":' . $fCount . ',"display":"Q,SA,M,K,M;PE,M,J,A,N;A,PE,K,Q,A","reelset":0,"runningTotal":' . ($slotSettings->GetGameData('SunWukongPTFreeStartWin') + $slotSettings->GetGameData('SunWukongPTBonusWin')) . ',"spins":' . $fCount . ',"stops":"' . implode(',', $reels['rp']) . '"},"replayInfo":{"foItems":"' . implode(',', $reels['rp']) . '"}}';
                                $jsSpin = '' . json_encode($reels) . '';
                                $responseLog[] = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"bonusMpl":' . $bonusMpl . ',"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $fCount . ',"currentFreeGames":' . $s . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('SunWukongPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('SunWukongPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"bonusInfo":"","Jackpots":"","reelsSymbols":' . $jsSpin . '}}';
                            }
                            $slotSettings->SetGameData('SunWukongPTFreeSpins', $spins);
                            $slotSettings->SetGameData('SunWukongPTFreeLogs', $responseLog);
                            $slotSettings->SetGameData('SunWukongPTFreeGames', $fCount);
                            $slotSettings->SetGameData('SunWukongPTCurrentFreeGame', 0);
                            $fSpinList = implode(',', $spins);
                            $result_tmp[] = '3:::{"data":{"gameId":2543168908,"drawStates":[{"drawId":1,"feature":{"name":"bonus","details":{"bonusPayout":' . $slotSettings->GetGameData('SunWukongPTFreeStartWin') . ',"initialSpins":' . $fCount . ',"multiplier":' . $bonusMpl . ',"pick":' . $postData['pick'] . ',"spins":' . $fCount . ',"totalPayout":' . $allWin . '}}},' . $fSpinList . ']},"ID":47342,"umid":1951}';
                        }
                        ...
                        {
                            $fsNum = $slotSettings->GetGameData('SunWukongPTCurrentFreeGame');
                            $spins_ = $slotSettings->GetGameData('SunWukongPTFreeSpins');
                            $logs_ = $slotSettings->GetGameData('SunWukongPTFreeLogs');
                            ...
                            $slotSettings->SetGameData('SunWukongPTCurrentFreeGame', $slotSettings->GetGameData('SunWukongPTCurrentFreeGame') + 1);
                        }
                        if( isset($postData['ID']) && ($postData['ID'] == '46123' || $postData['ID'] == '46302') ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('SunWukongPTBonusWin', 0);
                                $slotSettings->SetGameData('SunWukongPTFreeGames', 0);
                                $slotSettings->SetGameData('SunWukongPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SunWukongPTTotalWin', 0);
                                $slotSettings->SetGameData('SunWukongPTFreeBalance', 0);
                                $slotSettings->SetGameData('SunWukongPTFreeStartWin', 0);
                            }
                            ...
                            {
                                $postData['bet'] = $slotSettings->GetGameData('SunWukongPTBet');
                                $lines = $slotSettings->GetGameData('SunWukongPTLines');
                            }
                            else
                            {
                                ...
                                $lines = (int)$lines_[1];
                                $slotSettings->SetGameData('SunWukongPTBet', $postData['bet']);
                                $slotSettings->SetGameData('SunWukongPTLines', $lines);
                            }
                            ...
                            {
                                $slotSettings->SetGameData('SunWukongPTBonusWin', $slotSettings->GetGameData('SunWukongPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('SunWukongPTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('SunWukongPTTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('SunWukongPTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('SunWukongPTFreeGames', $slotSettings->GetGameData('SunWukongPTFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SunWukongPTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('SunWukongPTBonusWin', 0);
                                    $slotSettings->SetGameData('SunWukongPTFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('SunWukongPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SunWukongPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('SunWukongPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('SunWukongPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '46090':
                                ...
                                {
                                    $fsReel = $slotSettings->GetGameData('SunWukongPTFreeSpins');
                            ...
                            case '40036':
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    {
                                        $slotSettings->SetGameData('SunWukongPTFreeSpins', $lastEvent->serverResponse->freeSeq);
                                        $slotSettings->SetGameData('SunWukongPTFreeLogs', $lastEvent->serverResponse->freeLogSeq);
                                        $slotSettings->SetGameData('SunWukongPTbonusMpl', $lastEvent->serverResponse->bonusMpl);
                                    }
