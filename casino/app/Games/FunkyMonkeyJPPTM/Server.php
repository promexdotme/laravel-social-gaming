                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( isset($postData['umid']) ) 
                        {
                            ...
                            {
                                $result_tmp[] = '3:::{"data":{"jackpotUpdates":{"fmjp":[{"coinSize":' . ($slotSettings->Bet[count($slotSettings->Bet) - 1] * 100) . ',"jackpot":' . ($slotSettings->slotJackpot[0] * 100) . '}]}},"ID":40042,"umid":10}';
                            }
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMBonusWin', 0);
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMFreeGames', 0);
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMTotalWin', 0);
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMFreeBalance', 0);
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMCurrentFreeGame', $slotSettings->GetGameData('FunkyMonkeyJPPTMMCurrentFreeGame') + 1);
                            ...
                            if( $gameBets[count($gameBets) - 1] == $betLine ) 
                            {
                                $jackID = 0;
                            }
                            ...
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                ...
                                if( $jackState['isJackPay'] ) 
                                {
                                    ...
                                    for( $jl = 1; $jl <= 3; $jl++ ) 
                                    {
                                        $jreel = $slotSettings->PutBonusToLine($jl, $linesId[$rline][$jl - 1], '10');
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('FunkyMonkeyJPPTMMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( in_array($s[0], [
                                                7, 
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym && $csym == '10' && $slotSettings->Bet[count($slotSettings->Bet) - 1] <= $betLine ) 
                                            {
                                                $cWins[$k] = 0;
                                                $isJackPay = true;
                                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMBonusWin', $slotSettings->GetGameData('FunkyMonkeyJPPTMMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('FunkyMonkeyJPPTMMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('FunkyMonkeyJPPTMMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('FunkyMonkeyJPPTMMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyJPPTMMFreeGames', $slotSettings->GetGameData('FunkyMonkeyJPPTMMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('FunkyMonkeyJPPTMMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyJPPTMMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyJPPTMMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            $jpWin = (500000 * $betLine - ($slotSettings->slotJackpot[0] * 100)) * -1;
                            $result_tmp[] = '3:::{"data":{"credit":' . $balanceInCents . ',"coinsize":1,"jpWin":' . $jpWin . ', "jackpot":235,"results":[' . implode(',', $reels['rp']) . '],"windowId":"Adbmao"},"ID":49079,"umid":35}';
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('FunkyMonkeyJPPTMMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('FunkyMonkeyJPPTMMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('FunkyMonkeyJPPTMMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('FunkyMonkeyJPPTMMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            if( $isJackPay ) 
                            {
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '40066':
                                ...
                                $result_tmp[] = '3:::{"data":{"funNoticeGames":0,"funNoticePayouts":0,"gameGroup":"pmn","minBet":0,"maxBet":0,"minPosBet":0,"maxPosBet":10,"coinSizes":[' . implode(',', $gameBets) . ']},"ID":40025,"umid":21}';
                                break;
                            case '40036':
