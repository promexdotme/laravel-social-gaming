            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('WolfGoldDealerCard');
                        $totalWin = $slotSettings->GetGameData('WolfGoldTotalWin');
                        $gambleWin = 0;
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('WolfGoldDealerCard', $tmpDc);
                    ...
                    {
                        $bstate = $slotSettings->GetGameData('WolfGoldBonusState');
                        $Balance = $slotSettings->GetBalance();
                        if( $bstate == 0 ) 
                        {
                            $andFree = [
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('WolfGoldFreeGames');
                            $stat[$postData['ind']] = 1;
                        ...
                        else if( $bstate == 1 ) 
                        {
                            ...
                            $andFree[$postData['ind']] = $slotSettings->GetGameData('WolfGoldBonusMpl');
                            $stat[$postData['ind']] = 2;
                            $response = 'fsmul=' . $slotSettings->GetGameData('WolfGoldBonusMpl') . '&bgid=0&balance=' . $Balance . '&wins=' . implode(',', $andFree) . '&fsmax=' . $slotSettings->GetGameData('WolfGoldFreeGames') . '&level=2&index=149&balance_cash=' . $Balance . '&balance_bonus=0.00&na=s&fswin=0.00&status=' . implode(',', $stat) . '&stime=' . floor(microtime(true) * 1000) . '&fs=1&bgt=28&wins_mask=m,m,m,m,m,m&end=1&fsres=0.00&sver=5&n_reel_set=1&counter=298';
                            $bstate++;
                        }
                        $slotSettings->SetGameData('WolfGoldBonusState', $bstate);
                    }
                    ...
                    else if( $postData['slotEvent'] == 'doSpin' ) 
                    {
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            ...
                            {
                                $slotSettings->SetGameData('WolfGoldBonusMpl', rand(1, 18));
                                $slotSettings->SetGameData('WolfGoldFreeGames', rand(10, 38));
                        ...
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $slotSettings->GetGameData('WolfGoldFreeGames') <= $slotSettings->GetGameData('WolfGoldCurrentFreeGame') && $slotSettings->GetGameData('WolfGoldFreeGames') > 0 ) 
                            {
                                $spinType = 'c';
                                $winString .= ('&fs_total=' . $slotSettings->GetGameData('WolfGoldFreeGames') . '&fsmul_total=' . $slotSettings->GetGameData('WolfGoldBonusMpl') . '&fswin_total=' . $slotSettings->GetGameData('WolfGoldBonusWin') . '&fsres_total=' . $slotSettings->GetGameData('WolfGoldBonusWin') . '');
                            }
                            else
                            {
                                $winString .= ('&fsmul=' . $slotSettings->GetGameData('WolfGoldBonusMpl') . '&fsmax=' . $slotSettings->GetGameData('WolfGoldFreeGames') . '&fswin=' . $slotSettings->GetGameData('WolfGoldTotalWin') . '&fs=' . $slotSettings->GetGameData('WolfGoldCurrentFreeGame') . '&fsres=' . $slotSettings->GetGameData('WolfGoldBonusWin'));
                            }
                        ...
                        $response = 'tw=' . $slotSettings->GetGameData('WolfGoldBonusWin') . '&bstate=' . $slotSettings->GetGameData('WolfGoldBonusState') . '&balance=' . $Balance . '&index=' . $postData['index'] . '&balance_cash=' . $Balance . '&balance_bonus=0.00' . $winString . '&na=' . $spinType . '&stime=' . floor(microtime(true) * 1000) . '&sa=' . $reels['reel1'][3] . ',' . $reels['reel2'][3] . ',' . $reels['reel3'][3] . ',' . $reels['reel4'][3] . ',' . $reels['reel5'][3] . '&sb=' . $reels['reel1'][-1] . ',' . $reels['reel2'][-1] . ',' . $reels['reel3'][-1] . ',' . $reels['reel4'][-1] . ',' . $reels['reel5'][-1] . '&sh=3&c=0.01&sver=5&n_reel_set=0&counter=' . ((int)$postData['counter'] + 1) . '&l=25&s=' . $s . '&w=' . $totalWinRaw . '';
                        if( $slotSettings->GetGameData('WolfGoldFreeGames') <= $slotSettings->GetGameData('WolfGoldCurrentFreeGame') && $slotSettings->GetGameData('WolfGoldFreeGames') > 0 ) 
                        {
                            $slotSettings->SetGameData('WolfGoldTotalWin', 0);
                            $slotSettings->SetGameData('WolfGoldBonusWin', 0);
                        }
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BonusMpl":' . $slotSettings->GetGameData('WolfGoldBonusMpl') . ',"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('WolfGoldFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('WolfGoldCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' . $slotSettings->GetGameData('WolfGoldBonusWin') . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
