            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( isset($postData['ID']) && ($postData['ID'] == '46311' || $postData['ID'] == '46302') ) 
                        {
                            if( $postData['ID'] == '46302' ) 
                            {
                                $postData['savedStates'] = [];
                                $postData['savedStates'][0] = [];
                                $postData['savedStates'][0]['attr21'] = 1;
                            }
                            if( $slotSettings->GetGameData('HotGemsPTCurrentFreeGame') <= $slotSettings->GetGameData('HotGemsPTFreeGames') && $slotSettings->GetGameData('HotGemsPTFreeGames') > 0 && $postData['savedStates'][0]['attr21'] > 0 ) 
                            {
                                $fsNum = $slotSettings->GetGameData('HotGemsPTCurrentFreeGame');
                                $spins_ = $slotSettings->GetGameData('HotGemsPTFreeSpins');
                                $logs_ = $slotSettings->GetGameData('HotGemsPTFreeLogs');
                                ...
                                if( $postData['ID'] == '46302' ) 
                                {
                                    $result_tmp[] = '3:::' . $spins_[$slotSettings->GetGameData('HotGemsPTCurrentFreeGame')];
                                }
