                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $scattersCount >= 2 ) 
                            {
                                ...
                                {
                                    $slotSettings->SetGameData('WaysOfPhoenixPTFreeGames', $slotSettings->GetGameData('WaysOfPhoenixPTFreeGames') + $slotSettings->slotFreeCountAdd[$scattersCount]);
                                }
                                else if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('WaysOfPhoenixPTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('WaysOfPhoenixPTBonusWin', 0);
                                    $slotSettings->SetGameData('WaysOfPhoenixPTFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $rI = $cReelIndex;
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"gridState":' . json_encode($gridState) . ',"linesArr":[4096],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('WaysOfPhoenixPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('WaysOfPhoenixPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('WaysOfPhoenixPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('WaysOfPhoenixPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            case '31031':
                                ...
                                break;
                            case '40029':
                                $result_tmp[] = '3:::{"data":{"numFgWon":' . $slotSettings->GetGameData('WaysOfPhoenixPTFreeGames') . ',"windowId":"JB2We4"},"ID":49373,"umid":63}';
                                break;
                            case '10001':
                            ...
                            case '40030':
                                ...
                                {
                                    $bonusOpt = '';
                                    $gr = $slotSettings->GetGameData('WaysOfPhoenixPTGridState');
