                    ...
                    try
                    {
                        ...
                        if( $postData['slotEvent'] == 'gameInit' ) 
                        {
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $lastEvent->serverResponse->currentFreeGames . ',"add":0,"total":' . $lastEvent->serverResponse->totalFreeGames . ',"totalWin":' . $lastEvent->serverResponse->totalWin . '';
                        ...
                        {
                            $response = '{"status":"success","microtime":0.0026731491088867,"dateTime":"2020-02-13 13:23:32","error":"","content":{"cmd":"sessionInfo","serverMathematics":"\/game\/DoubleHappinessAT\/server","serverResources":"","sessionId":"54594109_4133a64673d3883d42cd003ee905ba3e","exitUlt":"\/","exitUrl":"\/","id":"341","name":"GameName","currency":"ALL","language":"en","type":"aristocrat","systemName":"double_happiness","version":"2","mobile":"1"}}';
                        }
                        ...
                        else if( $postData['slotEvent'] == 'gamePick' ) 
                        {
                            ...
                            if( $_POST['pick'] == 0 ) 
                            {
                                $slotSettings->SetGameData('DoubleHappinessATMpl', 10);
                                $slotSettings->SetGameData('DoubleHappinessATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DoubleHappinessATFreeGames', 5);
                            }
                            else if( $_POST['pick'] == 1 ) 
                            {
                                $slotSettings->SetGameData('DoubleHappinessATMpl', 5);
                                $slotSettings->SetGameData('DoubleHappinessATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DoubleHappinessATFreeGames', 10);
                            }
                            else if( $_POST['pick'] == 2 ) 
                            {
                                $slotSettings->SetGameData('DoubleHappinessATMpl', 3);
                                $slotSettings->SetGameData('DoubleHappinessATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DoubleHappinessATFreeGames', 15);
                            }
                            else if( $_POST['pick'] == 3 ) 
                            {
                                $slotSettings->SetGameData('DoubleHappinessATMpl', 2);
                                $slotSettings->SetGameData('DoubleHappinessATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DoubleHappinessATFreeGames', 20);
                            }
                            $response = '{"status":"success","microtime":0.0093810558319092,"dateTime":"2020-02-15 14:26:21","error":"","content":{"session":"54990161_9709e13bca7763b46396cda1868d2e7e","cmd":"gamePick","balance":' . $balanceInCents . ',"win":0,"symbols":{"1":{"1":"symbol_6","2":"symbol_9","3":"symbol_13"},"2":{"1":"symbol_10","2":"symbol_5","3":"symbol_8"},"3":{"1":"symbol_10","2":"symbol_13","3":"symbol_11"},"4":{"1":"symbol_13","2":"symbol_10","3":"symbol_6"},"5":{"1":"symbol_3","2":"symbol_8","3":"symbol_1"}},"winLines":[],"freeSpin":{"id":"54990161_20200215132548","current":0,"add":' . $slotSettings->GetGameData('DoubleHappinessATFreeGames') . ',"total":' . $slotSettings->GetGameData('DoubleHappinessATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('DoubleHappinessATBonusWin') * 100) . ',"multiplayer":' . $slotSettings->GetGameData('DoubleHappinessATMpl') . ',"pick":' . $_POST['pick'] . '},"actionId":"54990161_10_60","hash":"1ace145628d930d28bdf38789a28a9d7"}}';
                        }
                        else if( $postData['slotEvent'] == 'slotGamble' ) 
                        {
                            ...
                            $dealerCard = '';
                            $totalWin = $slotSettings->GetGameData('DoubleHappinessATTotalWin');
                            $gambleWin = 0;
                            $statBet = $totalWin;
                            ...
                            $slotSettings->SetGameData('DoubleHappinessATTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            $winTypeTmp = $slotSettings->GetSpinSettings($postData['slotEvent'], $postData['slotBet'] * 30, $postData['slotLines']);
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bankSum = ($postData['slotBet'] * 30) / 100 * $slotSettings->GetPercent();
                                $slotSettings->SetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''), $bankSum, $postData['slotEvent']);
                                $slotSettings->UpdateJackpots($postData['slotBet'] * 30);
                                $slotSettings->SetBalance(-1 * ($postData['slotBet'] * 30), $postData['slotEvent']);
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('DoubleHappinessATBonusWin', 0);
                                $slotSettings->SetGameData('DoubleHappinessATFreeGames', 0);
                                $slotSettings->SetGameData('DoubleHappinessATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DoubleHappinessATTotalWin', 0);
                                $slotSettings->SetGameData('DoubleHappinessATFreeBalance', 0);
                                $slotSettings->SetGameData('DoubleHappinessATMpl', 3);
                            }
                            else
                            {
                                $slotSettings->SetGameData('DoubleHappinessATCurrentFreeGame', $slotSettings->GetGameData('DoubleHappinessATCurrentFreeGame') + 1);
                                $bonusMpl = $slotSettings->GetGameData('DoubleHappinessATMpl');
                            }
                            $Balance = $slotSettings->GetBalance();
                            ...
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    ...
                                    $reels['reel' . $rw1][$rp1 - 1] = 'symbol_1';
                                    $reels['reel' . $rw2][$rp2 - 1] = 'symbol_1';
                                }
                                ...
                                $scattersWin = $slotSettings->Paytable[$scatter][$scattersCount] * $postData['slotBet'] * 30;
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('DoubleHappinessATBonusWin', $slotSettings->GetGameData('DoubleHappinessATBonusWin') + $totalWin);
                                $slotSettings->SetGameData('DoubleHappinessATTotalWin', $slotSettings->GetGameData('DoubleHappinessATTotalWin') + $totalWin);
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('DoubleHappinessATCurrentFreeGame') . ',"wilds":[[' . implode(',', $rwilds[0]) . '],[' . implode(',', $rwilds[1]) . ']],"multiplayer":' . $slotSettings->GetGameData('DoubleHappinessATMpl') . ',"add":0,"total":' . $slotSettings->GetGameData('DoubleHappinessATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('DoubleHappinessATBonusWin') * 100) . '';
                            }
                            else
                            {
                                $slotSettings->SetGameData('DoubleHappinessATTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('DoubleHappinessATFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('DoubleHappinessATFreeGames', $slotSettings->GetGameData('DoubleHappinessATFreeGames') + $slotSettings->slotFreeCount[$scattersCount]);
                                    $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('DoubleHappinessATCurrentFreeGame') . ',"wilds":[[' . implode(',', $rwilds[0]) . '],[' . implode(',', $rwilds[1]) . ']],"multiplayer":' . $slotSettings->GetGameData('DoubleHappinessATMpl') . ',"add":' . $slotSettings->slotFreeCount[$scattersCount] . ',"total":' . $slotSettings->GetGameData('DoubleHappinessATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('DoubleHappinessATBonusWin') * 100) . '';
                                }
                                else
                                {
                                    $slotSettings->SetGameData('DoubleHappinessATFreeBalance', $Balance);
                                    $slotSettings->SetGameData('DoubleHappinessATBonusWin', $totalWin);
                                    $slotSettings->SetGameData('DoubleHappinessATFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                    $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('DoubleHappinessATCurrentFreeGame') . ',"wilds":[[' . implode(',', $rwilds[0]) . '],[' . implode(',', $rwilds[1]) . ']],"multiplayer":' . $slotSettings->GetGameData('DoubleHappinessATMpl') . ',"add":' . $slotSettings->slotFreeCount[$scattersCount] . ',"total":' . $slotSettings->GetGameData('DoubleHappinessATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('DoubleHappinessATBonusWin') * 100) . '';
                                }
                            }
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('DoubleHappinessATFreeGames') . ',"Mpl":' . $slotSettings->GetGameData('DoubleHappinessATMpl') . ',"currentFreeGames":' . $slotSettings->GetGameData('DoubleHappinessATCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":{},"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            $reelStrArr = [];
                            for( $i = 1; $i <= 5; $i++ ) 
                            {
                            ...
                            $slotSettings->SaveLogReport($response_log, $postData['slotBet'], 30, $reportWin, $postData['slotEvent']);
                        }
