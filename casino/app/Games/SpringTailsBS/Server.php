            ...
            function get_($request, $game)
            {
                DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( $aid == 'PLACEBET' && (string)$_POST['BUY'] == 'TRUE' ) 
                        {
                            $aid = 'BUY';
                        }
                        switch( $aid ) 
                        {
                            case 'ENTER':
                                ...
                                if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                                {
                                    ...
                                    $result_tmp[0] = 'RESULT=OK&BALANCE=' . $balanceInCents . '&game_mode=free&server_type=AP&LASTHAND=YES&STATE=BUYFEATURE_FREESPIN&IBETVALUES=' . implode('', $slotSettings->Bet) . '&FREESPINS=' . $freeLeft . '&FREESPINSTOTAL=12&FREE_SPIN_ROUND_WIN=0&LASTSTOPREEL=4,1,4|2,4,3|5,5,2|10,0,5|2,5,5&COMPLEXWIN=' . $fwCoin . '&LASTBET=' . $bet . '|5|1&SCPAYOUT=0&FREESPINS_PAYOUT=' . $bwCoin . '&AUTOPLAY_VALUES=5|10|15|20|25|30|40|50|100&DEFCOIN=' . $bet . '&POSSIBLE_LINES=5&DEFAULTNUMLINES=5&DEFAULTBETPERLINE=1&IPAYOUT3=100|50|25|10|8|6|3|3|2|2&IPAYOUT4=250|150|75|30|25|20|12|12|8|8&IPAYOUT5=25000|600|300|100|75|60|40|40|30|30&SCATTER=1|3&FSCOUNT=12&DEFBETVALUE=5&BUY_COUNT=12&BUY_PRICE=290&BONUSBALANCE=0&GID=816&TIME=1615391038604&END=0';
                                }
                                else
                                {
                                    $result_tmp[0] = 'RESULT=OK&BALANCE=' . $balanceInCents . '&game_mode=free&server_type=AP&LASTHAND=NO&AUTOPLAY_VALUES=5|10|15|20|25|30|40|50|100&IBETVALUES=' . implode('', $slotSettings->Bet) . '&DEFCOIN=' . $bet . '&POSSIBLE_LINES=5&DEFAULTNUMLINES=5&DEFAULTBETPERLINE=1&IPAYOUT3=100|50|25|10|8|6|3|3|2|2&IPAYOUT4=250|150|75|30|25|20|12|12|8|8&IPAYOUT5=25000|600|300|100|75|60|40|40|30|30&SCATTER=1|3&FSCOUNT=12&DEFBETVALUE=5&BUY_COUNT=12&BUY_PRICE=290&BONUSBALANCE=0&GID=816&TIME=' . time() . '&END=0';
                                }
                                break;
                            case 'BUY':
                                ...
                                $bonusBuyBet = $betline * 58 * $lines;
                                if( $lines <= 0 || $betline <= 0.0001 ) 
                                {
                                    $response = '{"responseEvent":"error","responseType":"' . $postData['command'] . '","serverResponse":"invalid bet state"}';
                                    exit( $response );
                                }
                                if( $slotSettings->GetBalance() < $bonusBuyBet ) 
                                {
                                    $response = '{"responseEvent":"error","responseType":"' . $postData['command'] . '","serverResponse":"invalid balance"}';
                                    exit( $response );
                                }
                                $postData['slotEvent'] = 'bet';
                                $slotSettings->SetBalance(-1 * $bonusBuyBet, $postData['slotEvent']);
                                $bankSum = $bonusBuyBet / 100 * $slotSettings->GetPercent();
                                $slotSettings->SetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''), $bankSum, $postData['slotEvent'], true);
                                $jackState = $slotSettings->UpdateJackpots($bonusBuyBet);
                                ...
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance());
                                $result_tmp[0] = 'RESULT=OK&BALANCE=' . $balanceInCents . '&TOTALWIN=0&MULTIPLIER=1|1|1|1|1&FREESPINS=12&BASIC_PAYOUT=0|0|0|0|0&STOPREEL=9,6,8|13,6,1|6,13,2|4,9,13|7,0,5&FREESPINSTOTAL=12&COMPLEXWIN=0&PAYOUT=0|0|0|0|0&FREE_SPIN_ROUND_WIN=0&STATE=BUYFEATURE_FREESPIN&BASIC_WIN=0&FREESPINS_PAYOUT=0&BONUSBALANCE=0&GID=816&TIME=1615051635033&END=0';
                                $slotSettings->SaveLogReport('NULL', $bonusBuyBet, $lines, 0, 'BB');
                                break;
                            case 'DOBONUS':
                            case 'PLACEBET':
                                $linesId = [];
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $scattersWin > 0 ) 
                                    {
                                        $scattersStr = '&SCPAYOUT=' . round($scattersWin / $betline);
                                    }
                                ...
                                $fs = 0;
                                $freeDataStr = '&TOTALWIN=' . round($totalWin / $betline) . '&STATE=MAIN&BASIC_WIN=' . round(($totalWin - $scattersWin) / $betline) . '&COMPLEXWIN=' . round($totalWin / $betline);
                                if( $scattersCount >= 3 ) 
                                {
                                    ...
                                    $fwCoin = round($slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') / $betline);
                                    $freeLeft = $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') - $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame');
                                    $freeDataStr = '&FSADD=' . $slotSettings->slotFreeCount[$scattersCount] . '&FREESPINS=' . $freeLeft . '&SCATTERWIN=' . round($scattersWin / $allbet) . '&FREESPINS_PAYOUT=' . $bwCoin . '&STATE=FREESPIN';
                                ...
                                $curReels .= ('|' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2]);
                                $curReels .= ('|' . $reels['reel5'][0] . ',' . $reels['reel5'][1] . ',' . $reels['reel5'][2]);
                                $winString = '&MULTIPLIER=1|1|1|1|1&BASIC_PAYOUT=' . implode('|', $cWins2) . '&PAYOUT=' . implode('|', $cWins);
                                ...
                                if( $postData['slotEvent'] == 'freespin' && $winType != 'bonus' ) 
                                {
                                    ...
                                    {
                                        $freeDataStr = '&TOTALWIN=' . round($totalWin / $betline) . '&FREE_SPIN_ROUND_WIN=' . round($totalWin / $betline) . '&FREESPINS_PAYOUT=' . $bwCoin . '&STATE=MAIN&BASIC_WIN=' . round($totalWin / $betline) . '&FREESPINSTOTAL=' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . '&FSADD=0&COMPLEXWIN=' . $fwCoin . '&FREESPINS=0';
                                    }
                                    else
                                    {
                                        $freeDataStr = '&TOTALWIN=' . round($totalWin / $betline) . '&FREE_SPIN_ROUND_WIN=' . round($totalWin / $betline) . '&FREESPINS_PAYOUT=' . $bwCoin . '&STATE=FREESPIN&BASIC_WIN=' . round($totalWin / $betline) . '&FREESPINSTOTAL=' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . '&FSADD=0&COMPLEXWIN=' . $fwCoin . '&FREESPINS=' . $freeLeft;
                                    }
                                }
                                $result_tmp[0] = 'RESULT=OK&BALANCE=' . $balanceInCents . '&STOPREEL=' . $curReels . '&BONUSBALANCE=0&GID=816&TIME=' . time() . '&END=0' . $winString . $freeDataStr . $scattersStr;
                                break;
                        }
