                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,10,20],"multiplier":1},"1":{"coef":[5,10,20],"multiplier":1},"2":{"coef":[10,20,40],"multiplier":1},"3":{"coef":[10,20,40],"multiplier":1},"4":{"coef":[15,30,80],"multiplier":1},"5":{"coef":[15,30,80],"multiplier":1},"6":{"coef":[10,20,40,200],"multiplier":1},"7":{"coef":[15,40,80,500],"multiplier":1},"8":{"coef":[20,60,200,1200],"multiplier":1},"9":{"coef":[40,80,800,3000],"multiplier":1},"10":{"coef":[],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[6,1,1,8,8,0,5,4,4,9,9,6,2,2,2,5,7,3,3,3],[5,4,4,9,9,6,0,1,1,10,7,3,3,8,8,5,6,2,2,2],[6,1,1,9,9,0,5,4,4,8,8,2,2,2,10,7,3,3,3,3],[0,1,1,8,8,6,5,10,4,4,4,7,2,2,9,9,6,3,3,3],[7,1,1,10,5,0,7,8,8,4,4,6,2,2,9,9,0,3,3,3]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('VampireNightEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('VampireNightEGTBonusWin', 0);
                                $slotSettings->SetGameData('VampireNightEGTFreeGames', 0);
                                $slotSettings->SetGameData('VampireNightEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('VampireNightEGTTotalWin', 0);
                                $slotSettings->SetGameData('VampireNightEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('VampireNightEGTCurrentFreeGame') < $slotSettings->GetGameData('VampireNightEGTFreeGames') && $slotSettings->GetGameData('VampireNightEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('VampireNightEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('VampireNightEGTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('VampireNightEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('VampireNightEGTTotalWin', $totalWin);
                                $slotSettings->SetGameData('VampireNightEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('VampireNightEGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(0, 6));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('VampireNightEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('VampireNightEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('VampireNightEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('VampireNightEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('VampireNightEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('VampireNightEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('VampireNightEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('VampireNightEGTGambleStep', $slotSettings->GetGameData('VampireNightEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('VampireNightEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('VampireNightEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('VampireNightEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
