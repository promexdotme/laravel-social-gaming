                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('Hot27AMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('Hot27AMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('Hot27AMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('Hot27AMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('Hot27AMTotalWin', 0);
                                $slotSettings->SetGameData('Hot27AMFreeBalance', 0);
                                $slotSettings->SetGameData('Hot27AMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('Hot27AMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('Hot27AMFreeGames') - $slotSettings->GetGameData('Hot27AMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('Hot27AMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('Hot27AMCurrentFreeGame') < $slotSettings->GetGameData('Hot27AMFreeGames') && $slotSettings->GetGameData('Hot27AMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('Hot27AMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            $response = '05' . $slotSettings->FormatReelStrips('') . '5' . $slotSettings->FormatReelStrips('') . '000101010101010' . $balanceFormated . '1009' . $minBets . $maxBets . '1b101010101010191b1b1b110010101010100000000000000000' . $betsLength . $betString . '1c10101010101010101010101010101010101010101010101010101010#00101010|0';
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('Hot27AMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('Hot27AMBonusWin', 0);
                                $slotSettings->SetGameData('Hot27AMFreeGames', 0);
                                $slotSettings->SetGameData('Hot27AMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Hot27AMTotalWin', 0);
                                $slotSettings->SetGameData('Hot27AMFreeBalance', 0);
                                $slotSettings->SetGameData('Hot27AMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('Hot27AMCurrentFreeGame', $slotSettings->GetGameData('Hot27AMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('Hot27AMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('Hot27AMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('Hot27AMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('Hot27AMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('Hot27AMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            for( $i = 0; $i <= 27; $i++ ) 
                            {
                                $cWins[$i] = $cWins[$i] / $betLine / $bonusMpl * 1;
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('Hot27AMFreeGames') - $slotSettings->GetGameData('Hot27AMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('Hot27AMFreeGames') <= $slotSettings->GetGameData('Hot27AMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('Hot27AMBonusWin');
                                $bonusMpl = $slotSettings->slotFreeMpl;
                            }
                            else
                            {
                            ...
                            $response = '1' . $gameState . '010' . $balanceFormated . $slotSettings->HexFormat(round($totalWin * 100)) . $reelSate . $bet_h . '1b10101010101010101010101c' . $winLinesFormated . '2c1b010418173230#101010';
                            $response .= ('_' . json_encode($reels));
                            $slotSettings->SetGameData('Hot27AMDoubleAnswer', $reelSate . $bet_h . $ln_h . $freeInfo . $freeWinState . $slotSettings->HexFormat($bonusMpl) . '1010' . $reelSate . '0' . $symCountAll . $winLinesFormated);
                            $slotSettings->SetGameData('Hot27AMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('Hot27AMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('Hot27AMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('Hot27AMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('Hot27AMCollect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('Hot27AMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('Hot27AMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('Hot27AMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('Hot27AMDoubleWin', $winall);
                            $slotSettings->SetGameData('Hot27AMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('Hot27AMCollectP0');
                            $slotSettings->SetGameData('Hot27AMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('Hot27AMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('Hot27AMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('Hot27AMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
