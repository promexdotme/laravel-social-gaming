            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'create_session':
                                $slotSettings->SetGameData('DesertTalesCTCards', [
                                    19, 
                                    44, 
                                    0, 
                                    37, 
                            ...
                            case 'init_game':
                                ...
                                $slotSettings->SetGameData('DesertTalesCTBonusWin', 0);
                                $slotSettings->SetGameData('DesertTalesCTFreeGames', 0);
                                $slotSettings->SetGameData('DesertTalesCTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DesertTalesCTCurrentFreeGame0', 0);
                                $slotSettings->SetGameData('DesertTalesCTTotalWin', 0);
                                $slotSettings->SetGameData('DesertTalesCTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    if( $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame') < $slotSettings->GetGameData('DesertTalesCTFreeGames') && $slotSettings->GetGameData('DesertTalesCTFreeGames') > 0 ) 
                                    {
                            ...
                            case 'poll':
                                ...
                                {
                                    $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance() - $slotSettings->GetGameData('DesertTalesCTTotalWin')) * 100;
                                }
                            ...
                            case 'game_finished':
                                $slotSettings->SetGameData('DesertTalesCTBonusWin', 0);
                                $slotSettings->SetGameData('DesertTalesCTTotalWin', 0);
                                $responseData['game_finished'][0]['freeround_play'] = $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame0');
                            ...
                            case 'subgame_finished':
                                if( $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame0') < $slotSettings->GetGameData('DesertTalesCTFreeGames') && $slotSettings->GetGameData('DesertTalesCTFreeGames') > 0 ) 
                                {
                                    ...
                                    $response = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"result":' . json_encode($result) . ',"slotDenom":' . $slotSettings->GetGameData($slotSettings->slotId . 'Denom') . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('DesertTalesCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData('DesertTalesCTFreeBalance') . ',"afterBalance":' . $slotSettings->GetGameData('DesertTalesCTFreeBalance') . ',"bonusWin":' . $slotSettings->GetGameData('DesertTalesCTBonusWin') . ',"totalWin":' . $reportWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Reels')) . '}}';
                                    if( $slotSettings->GetGameData('DesertTalesCTFreeGames') <= $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame0') ) 
                                    {
                                        if( $reportWin > 0 ) 
                                        {
                                    ...
                                    $balanceInCents = $slotSettings->GetGameData('DesertTalesCTFreeBalance');
                                    $responseData['subgame_finished'][0]['account_status']['credit'] = round($balanceInCents);
                                    $responseData['subgame_finished'][0]['freeround_play'] = $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame0');
                                    $responseData['subgame_finished'][1]['freeround_play'] = $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame0');
                            ...
                            case 'play':
                                ...
                                for( $spinLoop = 0; $spinLoop < $spinLoopLimit; $spinLoop++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] != 'freespin' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('DesertTalesCTCurrentFreeGame0', 0);
                                        $slotSettings->SetGameData('DesertTalesCTTotalWin', 0);
                                        $slotSettings->SetGameData('DesertTalesCTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    ...
                                    {
                                        $slotSettings->SetGameData('DesertTalesCTCurrentFreeGame', $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame') + 1);
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $slotSettings->SetGameData('DesertTalesCTBonusWin', $slotSettings->GetGameData('DesertTalesCTBonusWin') + $totalWin);
                                        $slotSettings->SetGameData('DesertTalesCTTotalWin', $slotSettings->GetGameData('DesertTalesCTTotalWin') + $totalWin);
                                        $balanceInCents = $slotSettings->GetGameData('DesertTalesCTFreeBalance');
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('DesertTalesCTTotalWin', $totalWin);
                                    }
                                    $fs = 0;
                                    if( $scattersCount >= 3 ) 
                                    {
                                        if( $slotSettings->GetGameData('DesertTalesCTFreeGames') > 0 ) 
                                        {
                                            $slotSettings->SetGameData('DesertTalesCTFreeGames', $slotSettings->GetGameData('DesertTalesCTFreeGames') + $slotSettings->slotFreeCount);
                                        }
                                        else
                                        {
                                            $slotSettings->SetGameData('DesertTalesCTFreeStartWin', $totalWin);
                                            $slotSettings->SetGameData('DesertTalesCTBonusWin', $totalWin);
                                            $slotSettings->SetGameData('DesertTalesCTFreeGames', $slotSettings->slotFreeCount);
                                        }
                                        $fs = $slotSettings->GetGameData('DesertTalesCTFreeGames');
                                    ...
                                    $winstring = '';
                                    $slotSettings->SetGameData('DesertTalesCTGambleStep', 5);
                                    $hist = $slotSettings->GetGameData('DesertTalesCTCards');
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $state = 'freespin';
                                        ...
                                        if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('DesertTalesCTBonusWin') > 0 ) 
                                        {
                                            $responseData['play'][0]['result'][$spinLoop]['is_free'] = true;
                                    ...
                                    $responseData['play'][0]['result'][$spinLoop]['total_win'] = $slotSettings->GetGameData('DesertTalesCTTotalWin') / $denom;
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"result":' . json_encode($responseData['play'][0]['result']) . ',"slotDenom":' . $denom . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('DesertTalesCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('DesertTalesCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeBalance') . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('DesertTalesCTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'gamble':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('DesertTalesCTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                ...
                                $hist = $slotSettings->GetGameData('DesertTalesCTCards');
                                $slotSettings->SetGameData('DesertTalesCTGambleStep', $slotSettings->GetGameData('DesertTalesCTGambleStep') - 1);
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isGambleWin = 0;
                                }
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('DesertTalesCTGambleStep', 0);
                                    if( $postData['continue_params']['double_key'] != 'red' && $postData['continue_params']['double_key'] != 'black' ) 
                                    {
                                        if( $postData['continue_params']['double_key'] == 'diamond' ) 
                                        {
                                ...
                                $slotSettings->SetGameData('DesertTalesCTCards', $hist);
