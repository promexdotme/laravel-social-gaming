            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['action'] == 'risk' && $slotSettings->GetGameData('SweetLife2IGWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['action'] ) 
                        {
                            case 'init':
                                ...
                                break;
                            case 'status':
                                if( !$slotSettings->HasGameData('SweetLife2IGLines') || !$slotSettings->HasGameData('SweetLife2IGBet') ) 
                                {
                                    $slotSettings->SetGameData('SweetLife2IGLines', 1);
                                    $slotSettings->SetGameData('SweetLife2IGBet', 1);
                                }
                                $balance = $slotSettings->GetBalance();
                                if( $slotSettings->GetGameData('SweetLife2IGWin') > 0 ) 
                                {
                                    $balance = $balance - $slotSettings->GetGameData('SweetLife2IGWin');
                                }
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $slotSettings->GetGameData('SweetLife2IGBet') . ',"LastFuse":false,"LastLines":' . $slotSettings->GetGameData('SweetLife2IGLines') . ',"MaxWin":675000,"RawCredit":"' . $balance . '","Reels":[[7,1,5,3,8],[5,3,6,7,7],[6,8,7,8,1]],"Win":0,"args":{},"cmd":"status","time":"20181111T111512"}';
                                break;
                            case 'start':
                                $balance = $slotSettings->GetBalance();
                                $bet = $postData['bet'];
                                ...
                                $slotSettings->SetGameData('SweetLife2IGReels', $reelStr);
                                $slotSettings->SetGameData('SweetLife2IGWin', $totalWin);
                                $slotSettings->SetGameData('SweetLife2IGRisk', 1);
                                $slotSettings->SetGameData('SweetLife2IGLines', $lines);
                                $slotSettings->SetGameData('SweetLife2IGBet', $bet);
                                $slotSettings->SetGameData('SweetLife2IGDealerCard', $card);
                                $rState = 'bet';
                                if( $isBonus ) 
                                {
                                    $rState = 'BG';
                            ...
                            case 'finish':
                                $balance = $slotSettings->GetBalance();
                                $slotSettings->SetGameData('SweetLife2IGWin', 0);
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Credit":' . floor($balance) . ',"RawCredit":' . floor($balance) . ',"Reels":[' . $slotSettings->GetGameData('SweetLife2IGReels') . '],"args":{},"cmd":"finish","time":"20181111T171830"}';
                                break;
                            case 'risk':
                                $onTabCards = [];
