                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[10,40,100],"multiplier":1},"1":{"coef":[10,40,100],"multiplier":1},"2":{"coef":[10,40,100],"multiplier":1},"3":{"coef":[10,40,100],"multiplier":1},"4":{"coef":[15,100,200],"multiplier":1},"5":{"coef":[15,125,250],"multiplier":1},"6":{"coef":[25,150,350],"multiplier":1},"7":{"coef":[30,200,500],"multiplier":1},"8":{"coef":[40,250,750],"multiplier":1},"9":{"coef":[50,500,1000],"multiplier":1},"10":{"coef":[2],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[2,4,8,3,2,7,0,6,1,9,9,0,5,10,1,2,8,4,5,8],[7,8,0,9,9,2,4,6,0,5,7,0,7,1,5,0,1,2,9,9],[0,3,7,10,1,4,6,2,0,8,3,0,6,1,9,9,2,7,4,5],[3,6,9,9,7,0,4,8,0,2,6,9,9,7,3,6,1,4,8,2],[2,8,3,0,1,7,0,10,6,2,7,4,9,9,0,6,5,1,8,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('SpanishPassionsEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('SpanishPassionsEGTBonusWin', 0);
                                $slotSettings->SetGameData('SpanishPassionsEGTFreeGames', 0);
                                $slotSettings->SetGameData('SpanishPassionsEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SpanishPassionsEGTTotalWin', 0);
                                $slotSettings->SetGameData('SpanishPassionsEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('SpanishPassionsEGTCurrentFreeGame') < $slotSettings->GetGameData('SpanishPassionsEGTFreeGames') && $slotSettings->GetGameData('SpanishPassionsEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('SpanishPassionsEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('SpanishPassionsEGTTotalWin', 0);
                                    $slotSettings->SetGameData('SpanishPassionsEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SpanishPassionsEGTCurrentFreeGame', $slotSettings->GetGameData('SpanishPassionsEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('SpanishPassionsEGTBonusWin', $slotSettings->GetGameData('SpanishPassionsEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('SpanishPassionsEGTTotalWin', $slotSettings->GetGameData('SpanishPassionsEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('SpanishPassionsEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SpanishPassionsEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('SpanishPassionsEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('SpanishPassionsEGTFreeGames', $slotSettings->GetGameData('SpanishPassionsEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('SpanishPassionsEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('SpanishPassionsEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('SpanishPassionsEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('SpanishPassionsEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('SpanishPassionsEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SpanishPassionsEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('SpanishPassionsEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('SpanishPassionsEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('SpanishPassionsEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('SpanishPassionsEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('SpanishPassionsEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('SpanishPassionsEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('SpanishPassionsEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('SpanishPassionsEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('SpanishPassionsEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('SpanishPassionsEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('SpanishPassionsEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('SpanishPassionsEGTGambleStep', $slotSettings->GetGameData('SpanishPassionsEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('SpanishPassionsEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('SpanishPassionsEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('SpanishPassionsEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
