                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('PartyNightAMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('PartyNightAMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('PartyNightAMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('PartyNightAMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('PartyNightAMTotalWin', 0);
                                $slotSettings->SetGameData('PartyNightAMFreeBalance', 0);
                                $slotSettings->SetGameData('PartyNightAMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('PartyNightAMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('PartyNightAMFreeGames') - $slotSettings->GetGameData('PartyNightAMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('PartyNightAMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('PartyNightAMCurrentFreeGame') < $slotSettings->GetGameData('PartyNightAMFreeGames') && $slotSettings->GetGameData('PartyNightAMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('PartyNightAMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('PartyNightAMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('PartyNightAMBonusWin', 0);
                                $slotSettings->SetGameData('PartyNightAMFreeGames', 0);
                                $slotSettings->SetGameData('PartyNightAMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PartyNightAMTotalWin', 0);
                                $slotSettings->SetGameData('PartyNightAMFreeBalance', 0);
                                $slotSettings->SetGameData('PartyNightAMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('PartyNightAMCurrentFreeGame', $slotSettings->GetGameData('PartyNightAMCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('PartyNightAMFreeGames') <= $slotSettings->GetGameData('PartyNightAMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('PartyNightAMBonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('PartyNightAMBonusWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('PartyNightAMBonusWin', $slotSettings->GetGameData('PartyNightAMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('PartyNightAMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('PartyNightAMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('PartyNightAMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('PartyNightAMFreeGames', $slotSettings->GetGameData('PartyNightAMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('PartyNightAMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('PartyNightAMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('PartyNightAMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"symDouble":' . $symDouble . ',"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('PartyNightAMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('PartyNightAMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('PartyNightAMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('PartyNightAMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('PartyNightAMFreeGames') - $slotSettings->GetGameData('PartyNightAMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('PartyNightAMFreeGames') <= $slotSettings->GetGameData('PartyNightAMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('PartyNightAMBonusWin');
                                $bonusMpl = $slotSettings->slotFreeMpl;
                            }
                            else
                            {
                            ...
                            $slotSettings->SetGameData('PartyNightAMDoubleAnswer', $reelSate . $bet_h . $ln_h . $freeInfo . $freeWinState . $slotSettings->HexFormat($bonusMpl) . '1010' . $reelSate . '0' . $symCountAll . $winLinesFormated);
                            $slotSettings->SetGameData('PartyNightAMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('PartyNightAMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('PartyNightAMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('PartyNightAMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('PartyNightAMCollect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('PartyNightAMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('PartyNightAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('PartyNightAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $slotSettings->SetGameData('PartyNightAMDoubleWin', $winall);
                            $slotSettings->SetGameData('PartyNightAMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('PartyNightAMCollectP0');
                            $slotSettings->SetGameData('PartyNightAMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('PartyNightAMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('PartyNightAMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('PartyNightAMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
