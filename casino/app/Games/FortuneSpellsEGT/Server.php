                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'subscribe':
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('FortuneSpellsEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('FortuneSpellsEGTBonusWin', 0);
                                $slotSettings->SetGameData('FortuneSpellsEGTFreeGames', 0);
                                $slotSettings->SetGameData('FortuneSpellsEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('FortuneSpellsEGTTotalWin', 0);
                                $slotSettings->SetGameData('FortuneSpellsEGTFreeBalance', 0);
                                $slotSettings->SetGameData('FortuneSpellsEGTExpandSymbol', -1);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('FortuneSpellsEGTCurrentFreeGame') < $slotSettings->GetGameData('FortuneSpellsEGTFreeGames') && $slotSettings->GetGameData('FortuneSpellsEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('FortuneSpellsEGTTotalWin', 0);
                                    $slotSettings->SetGameData('FortuneSpellsEGTExpandSymbol', -1);
                                    $slotSettings->SetGameData('FortuneSpellsEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', '');
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('FortuneSpellsEGTCurrentFreeGame', $slotSettings->GetGameData('FortuneSpellsEGTCurrentFreeGame') + 1);
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        for( $r = 1; $r <= 5; $r++ ) 
                                        {
                                            for( $p = 0; $p <= 2; $p++ ) 
                                            {
                                                if( $reelsEx['reel' . $r][$p] == $slotSettings->GetGameData('FortuneSpellsEGTExpandSymbol') ) 
                                                {
                                                    $reelsEx['reel' . $r][0] = $slotSettings->GetGameData('FortuneSpellsEGTExpandSymbol');
                                                    $reelsEx['reel' . $r][1] = $slotSettings->GetGameData('FortuneSpellsEGTExpandSymbol');
                                                    $reelsEx['reel' . $r][2] = $slotSettings->GetGameData('FortuneSpellsEGTExpandSymbol');
                                                    break;
                                                }
                                            }
                                        }
                                    ...
                                    for( $k = 0; $k < $lines; $k++ ) 
                                    {
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if( $postData['slotEvent'] == 'freespin' && $csym == $slotSettings->GetGameData('FortuneSpellsEGTExpandSymbol') ) 
                                                {
                                                    $cLineExp = [];
                                                    $exCount = 0;
                                                    for( $ex = 0; $ex < 5; $ex++ ) 
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('FortuneSpellsEGTBonusWin', $slotSettings->GetGameData('FortuneSpellsEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('FortuneSpellsEGTTotalWin', $slotSettings->GetGameData('FortuneSpellsEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('FortuneSpellsEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('FortuneSpellsEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                $firstSpinLog = '';
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                ...
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('FortuneSpellsEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('FortuneSpellsEGTFreeGames', $slotSettings->GetGameData('FortuneSpellsEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('FortuneSpellsEGTExpandSymbol', rand(0, 8));
                                        $slotSettings->SetGameData('FortuneSpellsEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('FortuneSpellsEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('FortuneSpellsEGTFreeGames', $slotSettings->slotFreeCount);
                                        $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', ',"firstSpin":{"expand":[],"reels":[' . $curReels . '],"lines":[],"scatters":[' . $scattersStr . ']}');
                                    }
                                    $fs = $slotSettings->GetGameData('FortuneSpellsEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $winStringExpand = implode(',', $lineWins2);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('FortuneSpellsEGTFreeGames') . ',"freeSpinsExpandLines": [' . $winStringExpand . '],"freeSpinsExpandSymbol":' . $slotSettings->GetGameData('FortuneSpellsEGTExpandSymbol') . ',"freeSpinsExpandWinAmount": ' . $expandWin . ',"currentFreeGames":' . $slotSettings->GetGameData('FortuneSpellsEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('FortuneSpellsEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('FortuneSpellsEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('FortuneSpellsEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('FortuneSpellsEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('FortuneSpellsEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('FortuneSpellsEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('FortuneSpellsEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('FortuneSpellsEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('FortuneSpellsEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('FortuneSpellsEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('FortuneSpellsEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('FortuneSpellsEGTGambleStep', $slotSettings->GetGameData('FortuneSpellsEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('FortuneSpellsEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('FortuneSpellsEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('FortuneSpellsEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
