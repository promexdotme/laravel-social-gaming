                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('ChineseKitchenPTBonusWin', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTFreeGames', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTTotalWin', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTFreeBalance', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('ChineseKitchenPTCurrentFreeGame', $slotSettings->GetGameData('ChineseKitchenPTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || $s[1] == $csym || $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('ChineseKitchenPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('ChineseKitchenPTBonusWin', $slotSettings->GetGameData('ChineseKitchenPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('ChineseKitchenPTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('ChineseKitchenPTTotalWin', $totalWin);
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('ChineseKitchenPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('ChineseKitchenPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('ChineseKitchenPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('ChineseKitchenPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
