                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'SELGAME':
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('BurningRingVSCards', $hist);
                                ...
                                $slotSettings->SetGameData('BurningRingVSBonusWin', 0);
                                $slotSettings->SetGameData('BurningRingVSFreeGames', 0);
                                $slotSettings->SetGameData('BurningRingVSCurrentFreeGame', 0);
                                $slotSettings->SetGameData('BurningRingVSTotalWin', 0);
                                $slotSettings->SetGameData('BurningRingVSFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('BurningRingVSCurrentFreeGame') < $slotSettings->GetGameData('BurningRingVSFreeGames') && $slotSettings->GetGameData('BurningRingVSFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'GA_SPIN':
                                ...
                                if( $slotSettings->GetGameData('BurningRingVSCurrentFreeGame') < $slotSettings->GetGameData('BurningRingVSFreeGames') && $slotSettings->GetGameData('BurningRingVSFreeGames') > 0 ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('BurningRingVSCurrentFreeGame', $slotSettings->GetGameData('BurningRingVSCurrentFreeGame') + 1);
                                }
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('BurningRingVSFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BurningRingVSCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('BurningRingVSBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                ...
                                $hist = $slotSettings->GetGameData('BurningRingVSCards');
                                $gtypeaft = 1;
                                $slotSettings->SetGameData('BurningRingVSBonusStart', 0);
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('BurningRingVSFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('BurningRingVSFreeGames', $slotSettings->GetGameData('BurningRingVSFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('BurningRingVSBonusWin', 0);
                                        $slotSettings->SetGameData('BurningRingVSBonusStart', 1);
                                        $slotSettings->SetGameData('BurningRingVSFreeGames', $slotSettings->slotFreeCount);
                                        $gtypeaft = 2;
                                    }
                                }
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('BurningRingVSTotalWin', $totalWin);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('BurningRingVSTotalWin', $totalWin);
                            ...
                            case 'GA_TAKE':
                                ...
                                {
                                    $gtype = 2;
                                    $slotSettings->SetGameData('BurningRingVSBonusWin', $slotSettings->GetGameData('BurningRingVSBonusWin') + $slotSettings->GetGameData('BurningRingVSTotalWin'));
                                }
                                $slotSettings->SetGameData('BurningRingVSTotalWin', 0);
                                $result_tmp[] = '{"status":"OK","authtoken":"1/90317550919705477001","for":"GA_TAKE","totsg":' . $slotSettings->GetGameData('BurningRingVSFreeGames') . ',"plsg":' . $slotSettings->GetGameData('BurningRingVSCurrentFreeGame') . ',"inscatwin":' . ($slotSettings->GetGameData('BurningRingVSBonusWin') * 100) . ',"gtype":' . $gtype . ',"credit":{"tot":' . $balanceInCents . '}}';
                                break;
                            case 'QUITGAME':
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            case 'GA_GAMBLE':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('BurningRingVSTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                ...
                                if( $slotSettings->GetGameData('BurningRingVSBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"status":"OK","gtypeaft":' . $gtype . ',"gtype":' . $gtype . ',"totsg":' . $slotSettings->GetGameData('BurningRingVSFreeGames') . ',"plsg":' . $slotSettings->GetGameData('BurningRingVSCurrentFreeGame') . ',"inscatwin":' . ($slotSettings->GetGameData('BurningRingVSBonusWin') * 100) . ',"authtoken":"1/90317550923628407358","for":"GA_GAMBLE","rno":29444,"state":3,"gmblwinf":2,"card":' . $dealerCard . ',"halfs":0,"snd":"' . $sndID . '","gmblamount":' . ($totalWin * 100) . ',"gmblmayhalf":2,"gmblhalfmin":100,"takesnd":"slidein","gmblhist":[' . implode(',', $hist) . '],"win":' . ($totalWin * 100) . ',"sgwin":0,"credit":{"tot":40885}}';
                                break;
                        }
