                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('DragonChampionsPTBonusWin', 0);
                                $slotSettings->SetGameData('DragonChampionsPTFreeGames', 0);
                                $slotSettings->SetGameData('DragonChampionsPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DragonChampionsPTTotalWin', 0);
                                $slotSettings->SetGameData('DragonChampionsPTFreeBalance', 0);
                                $slotSettings->SetGameData('DragonChampionsPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('DragonChampionsPTCurrentFreeGame', $slotSettings->GetGameData('DragonChampionsPTCurrentFreeGame') + 1);
                                $bonusMpl = 1;
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('DragonChampionsPTBonusWin', $slotSettings->GetGameData('DragonChampionsPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('DragonChampionsPTTotalWin', $totalWin);
                                $sID = 48820;
                                $rI = 7;
                                $sType = 'FREE';
                                $gType = 1;
                            ...
                            if( $scattersCount >= 2 ) 
                            {
                                if( $slotSettings->GetGameData('DragonChampionsPTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('DragonChampionsPTFreeGames', $slotSettings->GetGameData('DragonChampionsPTFreeGames') + $slotSettings->slotFreeCountAdd[$scattersCount]);
                                }
                                else if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('DragonChampionsPTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('DragonChampionsPTBonusWin', 0);
                                    $slotSettings->SetGameData('DragonChampionsPTFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[4096],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('DragonChampionsPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('DragonChampionsPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('DragonChampionsPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('DragonChampionsPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
