            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['action'] == 'risk' && $slotSettings->GetGameData('KeksIGWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['action'] ) 
                        {
                            case 'init':
                                ...
                                break;
                            case 'status':
                                if( !$slotSettings->HasGameData('KeksIGLines') || !$slotSettings->HasGameData('KeksIGBet') ) 
                                {
                                    $slotSettings->SetGameData('KeksIGLines', 1);
                                    $slotSettings->SetGameData('KeksIGBet', 1);
                                }
                                $balance = $slotSettings->GetBalance();
                                if( $slotSettings->GetGameData('KeksIGWin') > 0 ) 
                                {
                                    $balance = $balance - $slotSettings->GetGameData('KeksIGWin');
                                }
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $slotSettings->GetGameData('KeksIGBet') . ',"LastFuse":false,"LastLines":' . $slotSettings->GetGameData('KeksIGLines') . ',"MaxWin":675000,"RawCredit":"' . $balance . '","Reels":[[7,1,5,3,8],[5,3,6,7,7],[6,8,7,8,1]],"Win":0,"args":{},"cmd":"status","time":"20181111T111512"}';
                                break;
                            case 'start':
                                $balance = $slotSettings->GetBalance();
                                $bet = $postData['bet'];
                                ...
                                $slotSettings->SetGameData('KeksIGReels', $reelStr);
                                $slotSettings->SetGameData('KeksIGWin', $totalWin);
                                $slotSettings->SetGameData('KeksIGRisk', 1);
                                $slotSettings->SetGameData('KeksIGLines', $lines);
                                $slotSettings->SetGameData('KeksIGBet', $bet);
                                $slotSettings->SetGameData('KeksIGDealerCard', $card);
                                $rState = 'bet';
                                if( $isBonus ) 
                                {
                                    $rState = 'BG';
                            ...
                            case 'finish':
                                $balance = $slotSettings->GetBalance();
                                $slotSettings->SetGameData('KeksIGWin', 0);
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Credit":' . floor($balance) . ',"RawCredit":' . floor($balance) . ',"Reels":[' . $slotSettings->GetGameData('KeksIGReels') . '],"args":{},"cmd":"finish","time":"20181111T171830"}';
                                break;
                            case 'risk':
                                $onTabCards = [];
