                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[10,30,150],"multiplier":1},"1":{"coef":[10,30,150],"multiplier":1},"2":{"coef":[10,30,150],"multiplier":1},"3":{"coef":[10,30,150],"multiplier":1},"4":{"coef":[20,40,200],"multiplier":1},"5":{"coef":[40,120,700],"multiplier":1},"6":{"coef":[40,120,700],"multiplier":1},"7":{"coef":[10,50,250,5000],"multiplier":1},"8":{"coef":[],"multiplier":1},"9":{"coef":[20],"multiplier":1},"10":{"coef":[5,20,100],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[6,2,2,2,0,0,0,5,4,1,5,10,1,1,3,4,7,4,9,4],[7,2,2,2,5,3,0,0,10,6,3,3,1,1,4,1,4,8,4,2],[6,1,1,8,7,1,5,3,3,9,1,4,0,4,2,2,2,10,0,0],[1,1,1,7,1,3,3,8,6,4,3,4,2,2,10,2,0,0,5,1],[7,3,3,2,2,1,6,10,1,1,3,3,3,2,7,0,0,4,9,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":532,"gameNumber":"","sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('BurningHeart5EGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('BurningHeart5EGTBonusWin', 0);
                                $slotSettings->SetGameData('BurningHeart5EGTFreeGames', 0);
                                $slotSettings->SetGameData('BurningHeart5EGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('BurningHeart5EGTTotalWin', 0);
                                $slotSettings->SetGameData('BurningHeart5EGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('BurningHeart5EGTCurrentFreeGame') < $slotSettings->GetGameData('BurningHeart5EGTFreeGames') && $slotSettings->GetGameData('BurningHeart5EGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('BurningHeart5EGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BurningHeart5EGTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('BurningHeart5EGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $reels = $tmpReels;
                                $slotSettings->SetGameData('BurningHeart5EGTTotalWin', $totalWin);
                                $slotSettings->SetGameData('BurningHeart5EGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('BurningHeart5EGTCards');
                                $curReels = '' . rand(1, 8) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(1, 8);
                                $curReels .= (',' . rand(1, 8) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(1, 8));
                                $curReels .= (',' . rand(1, 8) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(1, 8));
                                $curReels .= (',' . rand(1, 8) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(1, 8));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('BurningHeart5EGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"expand":[' . implode(',', $wldPos) . '],"specialExpand":[],"gambles":5,"freespins":0,"jackpot":' . $isJack . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":532,"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('BurningHeart5EGTTotalWin') * 100) . ',"gameIdentificationNumber":532,"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('BurningHeart5EGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('BurningHeart5EGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('BurningHeart5EGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('BurningHeart5EGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('BurningHeart5EGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('BurningHeart5EGTGambleStep', $slotSettings->GetGameData('BurningHeart5EGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('BurningHeart5EGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('BurningHeart5EGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('BurningHeart5EGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":532,"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
