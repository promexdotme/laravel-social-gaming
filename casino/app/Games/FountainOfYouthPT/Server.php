                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('FountainOfYouthPTBonusWin', 0);
                                $slotSettings->SetGameData('FountainOfYouthPTFreeGames', 0);
                                $slotSettings->SetGameData('FountainOfYouthPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('FountainOfYouthPTTotalWin', 0);
                                $slotSettings->SetGameData('FountainOfYouthPTFreeBalance', 0);
                                $slotSettings->SetGameData('FountainOfYouthPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('FountainOfYouthPTCurrentFreeGame', $slotSettings->GetGameData('FountainOfYouthPTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('FountainOfYouthPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('FountainOfYouthPTBonusWin', $slotSettings->GetGameData('FountainOfYouthPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('FountainOfYouthPTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('FountainOfYouthPTTotalWin', $totalWin);
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('FountainOfYouthPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('FountainOfYouthPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('FountainOfYouthPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('FountainOfYouthPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
