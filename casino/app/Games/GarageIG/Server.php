            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['action'] == 'risk' && $slotSettings->GetGameData('GarageIGWin') <= 0 ) 
                        {
                        ...
                        {
                            case 'open':
                                if( $slotSettings->GetGameData('GarageIGSuperKeySteps') <= 0 ) 
                                {
                                    exit( '{"error":"Internal Error"}' );
                                }
                                $bankReserved = $slotSettings->GetGameData('GarageIGBankReserved');
                                $finished = 'false';
                                $bet = $slotSettings->GetGameData('GarageIGLines');
                                $lines = $slotSettings->GetGameData('GarageIGBet');
                                if( $postData['Super'] > 0 ) 
                                {
                                    $slotSettings->SetGameData('GarageIGSuperKey', 0);
                                }
                                $slotSettings->SetGameData('GarageIGSuperKeySteps', $slotSettings->GetGameData('GarageIGSuperKeySteps') - 1);
                                if( $slotSettings->GetGameData('GarageIGSuperKeySteps') <= 0 ) 
                                {
                                    $finished = 'true';
                                    $slotSettings->SetGameData('GarageIGSuperKey', 0);
                                ...
                                if( $finished ) 
                                {
                                    $slotSettings->SetGameData('GarageIGBankReserved', 0);
                                    $slotSettings->SetBank('bonus', $bankReserved);
                                }
                                $bank = $slotSettings->GetBank('bonus');
                                for( $i = 0; $i <= 1000; $i++ ) 
                                {
                                    $bTmp = $slotSettings->Bonus2($bet * $lines, $postData['Super'], $finished);
                                    $totalWin = $bTmp['win'];
                                    $infoBonus = $bTmp['info'];
                                    if( $totalWin <= $bank ) 
                                    {
                                ...
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '",' . $infoBonus . '"Credit":' . floor($balance) . ',"Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","Finished":' . $finished . ',"RawCredit":954,"Win":' . $totalWin . ',"args":{"Super":' . $postData['Super'] . '},"cmd":"open","time":"20190328T124737"}';
                                $slotSettings->SaveLogReport($response, 0, 1, $totalWin, 'BG2');
                                break;
                            case 'init':
                                $balance = $slotSettings->GetBalance();
                                $restoreLog = $slotSettings->GetHistory();
                                if( !$slotSettings->HasGameData('GarageIGSuperKey') ) 
                                {
                                    $slotSettings->SetGameData('GarageIGSuperKey', 0);
                                }
                                $slotSettings->SetGameData('GarageIGBankReserved', 0);
                                ...
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","slotViewState":"' . $slotSettings->slotViewState . '","slotKeyConfig":' . $slotSettings->slotKeyConfig . ',"BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":' . floor($balance) . ',"Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","Denominations":{"100":"1.00"},"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $bet . ',"LastLines":' . $lines . ',"MaxWin":675000,"RawCredit":1000,"Reels":[' . $reelStr . '],"Win":0,"args":{},"cmd":"init","time":"20190327T143954"}';
                                break;
                            case 'status':
                                if( !$slotSettings->HasGameData('GarageIGLines') || !$slotSettings->HasGameData('GarageIGBet') ) 
                                {
                                    $slotSettings->SetGameData('GarageIGLines', 1);
                                    $slotSettings->SetGameData('GarageIGBet', 1);
                                }
                                $balance = $slotSettings->GetBalance();
                                if( $slotSettings->GetGameData('GarageIGWin') > 0 ) 
                                {
                                    $balance = $balance - $slotSettings->GetGameData('GarageIGWin');
                                }
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $slotSettings->GetGameData('GarageIGBet') . ',"LastFuse":false,"LastLines":' . $slotSettings->GetGameData('GarageIGLines') . ',"MaxWin":675000,"RawCredit":"' . $balance . '","Reels":[[7,1,5,3,8],[5,3,6,7,7],[6,8,7,8,1]],"Win":0,"args":{},"cmd":"status","time":"20181111T111512"}';
                                break;
                            case 'start':
                                $balance = $slotSettings->GetBalance();
                                $bet = $postData['bet'];
                                ...
                                for( $i = 0; $i <= 1500; $i++ ) 
                                {
                                    if( $isBonus ) 
                                    {
                                        $slotSettings->bonusType = rand(1, 2);
                                        if( $slotSettings->bonusType == 1 ) 
                                        {
                                            $bTmp = $slotSettings->Bonus1($bet * $lines, $fuseBet);
                                        }
                                        else
                                        {
                                            $bankReserved = $bet * $lines * 20;
                                            ...
                                            {
                                                if( $slotSettings->GetGameData('GarageIGSuperKey') >= 9 ) 
                                                {
                                                    $bTmp = [];
                                                    $bTmp['win'] = 0;
                                                    $bTmp['info'] = '"NeedBonus2": 4,';
                                                    $slotSettings->SetGameData('GarageIGSuperKeySteps', 5);
                                                    $slotSettings->SetGameData('GarageIGBankReserved', $bankReserved);
                                                    $slotSettings->SetBank('bonus', -1 * $bankReserved);
                                                }
                                                else
                                                {
                                                    $bTmp = $slotSettings->Bonus2Static($bet * $lines, $fuseBet);
                                                }
                                            }
                                        }
                                        $totalWin = $bTmp['win'];
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            for( $lp = 0; $lp <= 4; $lp++ ) 
                                            {
                                                ...
                                                if( $reels['reel' . ($lp + 1)][0] == 1 ) 
                                                {
                                                    $bonusSym2++;
                                                }
                                                if( $reels['reel' . ($lp + 1)][1] == 1 ) 
                                                {
                                                    $bonusSym2++;
                                                }
                                                if( $reels['reel' . ($lp + 1)][2] == 1 ) 
                                                {
                                                    $bonusSym2++;
                                                }
                                            }
                                            if( ($bonusSym >= 3 || $bonusSym2 >= 3) && !$isBonus ) 
                                            {
                                            }
                                ...
                                if( $bonusSym == 2 && $slotSettings->GetGameData('GarageIGSuperKey') < 9 ) 
                                {
                                    $slotSettings->SetGameData('GarageIGSuperKey', $slotSettings->GetGameData('GarageIGSuperKey') + 1);
                                }
                                $response = '{"bs":"' . $bonusSym . '","SuperKey":' . $slotSettings->GetGameData('GarageIGSuperKey') . ',"SuperKeyPresent":' . $skPresent . ',"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '",' . $infoBonus . '"Credit":' . floor($balance) . ',"Fuse":false,"LineWins":' . json_encode($lineWinsInfo) . ',"RawCredit":' . floor($balance) . ',"RiskCard":' . $slotSettings->cardsID[$card] . ',"Reels":[' . $reelStr . '],"Lines":' . $lines . ',"Bet":' . $bet . ',"TotalBet":' . ($bet * $lines) . ',"Win":' . $totalWin . ',"args":{},"cmd":"start","Bonusing":' . $jackPay . ',"time":"20181111T174113"}';
                                $slotSettings->SetGameData('GarageIGReels', $reelStr);
                                $slotSettings->SetGameData('GarageIGWin', $totalWin);
                                $slotSettings->SetGameData('GarageIGRisk', 1);
                                $slotSettings->SetGameData('GarageIGLines', $lines);
                                $slotSettings->SetGameData('GarageIGBet', $bet);
                                $slotSettings->SetGameData('GarageIGDealerCard', $card);
                                $rState = 'bet';
                                if( $isBonus ) 
                                {
                                    if( $slotSettings->bonusType == 1 ) 
                                    {
                                        $slotSettings->SaveAccumBonus($bTmp['accCount'], $bTmp['accWin']);
                                    }
                                    $rState = 'BG';
                                }
                            ...
                            case 'finish':
                                $balance = $slotSettings->GetBalance();
                                $slotSettings->SetGameData('GarageIGWin', 0);
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Credit":' . floor($balance) . ',"RawCredit":' . floor($balance) . ',"Reels":[' . $slotSettings->GetGameData('GarageIGReels') . '],"args":{},"cmd":"finish","time":"20181111T171830"}';
                                break;
                            case 'risk':
                                $onTabCards = [];
