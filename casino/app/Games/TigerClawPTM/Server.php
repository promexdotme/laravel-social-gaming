                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('TigerClawPTMBonusWin', 0);
                                $slotSettings->SetGameData('TigerClawPTMFreeGames', 0);
                                $slotSettings->SetGameData('TigerClawPTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('TigerClawPTMTotalWin', 0);
                                $slotSettings->SetGameData('TigerClawPTMFreeBalance', 0);
                                $slotSettings->SetGameData('TigerClawPTMFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('TigerClawPTMCurrentFreeGame', $slotSettings->GetGameData('TigerClawPTMCurrentFreeGame') + 1);
                                $tmpMpl = [
                                    2, 
                                    3, 
                                    5
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('TigerClawPTMBonusWin', $slotSettings->GetGameData('TigerClawPTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('TigerClawPTMTotalWin', $totalWin);
                                $sID = 47752;
                                $rI = 5;
                            }
                            else
                            ...
                            if( $scattersCount >= 2 ) 
                            {
                                if( $slotSettings->GetGameData('TigerClawPTMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('TigerClawPTMFreeGames', $slotSettings->GetGameData('TigerClawPTMFreeGames') + $slotSettings->slotFreeCountAdd[$scattersCount]);
                                }
                                else if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('TigerClawPTMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('TigerClawPTMBonusWin', 0);
                                    $slotSettings->SetGameData('TigerClawPTMFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            $result_tmp[] = '3:::{"data":{"wildMultipliers":[' . $bonusMpl . '],"multiplier":1,"ww":"' . $totalWin . '","dbg":"' . $dbgstr . '","balance":' . $balanceInCents . ',"results":[' . implode(',', $reels['rp']) . '],"reelIndex":' . $rI . ',"windowId":"rnTQwa"},"ID":' . $sID . ',"umid":5}';
                            $result_tmp[] = '3:::{"data":{"typeBalance":0,"currency":"' . $slotSettings->slotCurrency . '","balanceInCents":' . $balanceInCents . ',"deltaBalanceInCents":1},"ID":40085}';
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[4096],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('TigerClawPTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('TigerClawPTMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('TigerClawPTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('TigerClawPTMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '40066':
                                ...
                                $result_tmp[] = '3:::{"data":{"funNoticeGames":0,"funNoticePayouts":0,"gameGroup":"tigc","minBet":0,"maxBet":0,"minPosBet":0,"maxPosBet":50000,"coinSizes":[' . implode(',', $gameBets) . ']},"ID":40025,"umid":21}';
                                break;
                            case '40036':
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    {
                                        $slotSettings->SetGameData($slotSettings->slotId . 'brokenGames', 'tigc');
                                    }
                                }
