            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'choice':
                                if( $slotSettings->GetGameData('TheExplorersEGTBonusStart') != 1 ) 
                                {
                                    exit();
                                }
                                $ChoiceArr = $slotSettings->GetGameData('TheExplorersEGTChoiceArr');
                                $fs = $slotSettings->GetGameData('TheExplorersEGTFreeGames');
                                $fw = $slotSettings->GetGameData('TheExplorersEGTMultiplier');
                                ...
                                if( count($ChoiceArr) >= 3 && $slotSettings->GetGameData('TheExplorersEGTFreeGames') > 0 ) 
                                {
                                    $isEnd = true;
                                    $pl_cur = 'true';
                                }
                                ...
                                $slotSettings->SetGameData('TheExplorersEGTChoiceArr', $ChoiceArr);
                                $slotSettings->SetGameData('TheExplorersEGTFreeGames', $fs + $cfr_cur);
                                $slotSettings->SetGameData('TheExplorersEGTMultiplier', $fw + $cfr0_cur);
                                $result_tmp[] = '{"complex":{"pos":' . $choice . ',"multiplier":' . $slotSettings->GetGameData('TheExplorersEGTMultiplier') . ',"freespins":' . $slotSettings->GetGameData('TheExplorersEGTFreeGames') . ',"mult":' . $cfr0_cur . ',"fs":' . $cfr_cur . ',"play":' . $pl_cur . ',"closed":[' . $closed . '],"freespins":' . $slotSettings->GetGameData('TheExplorersEGTFreeGames') . ',"choices":1,"gameCommand":"choice"},"state":"' . $state . '","winAmount":' . ($slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":1787203010852,"sessionKey":"5af6a26c031aa4ec491159428c5a7742","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":1566584969191}';
                                break;
                            case 'login':
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,20,100],"multiplier":2},"1":{"coef":[5,20,100],"multiplier":2},"2":{"coef":[5,20,150],"multiplier":2},"3":{"coef":[5,20,150],"multiplier":2},"4":{"coef":[10,50,200],"multiplier":2},"5":{"coef":[10,50,200],"multiplier":2},"6":{"coef":[10,50,250],"multiplier":2},"7":{"coef":[20,100,500],"multiplier":2},"8":{"coef":[20,150,750],"multiplier":2},"9":{"coef":[10,100,1000,5000],"multiplier":1},"10":{"coef":[5],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[8,1,4,0,5,2,6,1,9,7,3,8,1,4,2,3,5,6,0,2],[2,8,0,6,1,5,9,2,4,3,5,1,2,3,5,7,0,8,3,6],[4,0,2,1,9,3,7,4,8,1,10,3,2,0,1,3,6,1,6,2],[3,6,1,5,3,9,1,7,0,5,2,1,7,1,4,1,2,3,4,10],[6,1,7,3,5,0,1,4,8,1,5,10,2,4,9,7,5,0,3,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('TheExplorersEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('TheExplorersEGTBonusWin', 0);
                                $slotSettings->SetGameData('TheExplorersEGTFreeGames', 0);
                                $slotSettings->SetGameData('TheExplorersEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('TheExplorersEGTTotalWin', 0);
                                $slotSettings->SetGameData('TheExplorersEGTFreeBalance', 0);
                                $slotSettings->SetGameData('TheExplorersEGTMultiplier', 1);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('TheExplorersEGTCurrentFreeGame') < $slotSettings->GetGameData('TheExplorersEGTFreeGames') && $slotSettings->GetGameData('TheExplorersEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('TheExplorersEGTTotalWin', 0);
                                    $slotSettings->SetGameData('TheExplorersEGTMultiplier', 1);
                                    $slotSettings->SetGameData('TheExplorersEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('TheExplorersEGTCurrentFreeGame', $slotSettings->GetGameData('TheExplorersEGTCurrentFreeGame') + 1);
                                    $bonusMpl = $slotSettings->GetGameData('TheExplorersEGTMultiplier');
                                }
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('TheExplorersEGTBonusWin', $slotSettings->GetGameData('TheExplorersEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('TheExplorersEGTTotalWin', $slotSettings->GetGameData('TheExplorersEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('TheExplorersEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('TheExplorersEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('TheExplorersEGTChoiceArr', []);
                                    if( $slotSettings->GetGameData('TheExplorersEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->slotFreeCount = 1;
                                        $slotSettings->SetGameData('TheExplorersEGTFreeGames', $slotSettings->GetGameData('TheExplorersEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('TheExplorersEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('TheExplorersEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('TheExplorersEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('TheExplorersEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('TheExplorersEGTFreeGames') . ',"Multiplier":' . $slotSettings->GetGameData('TheExplorersEGTMultiplier') . ',"currentFreeGames":' . $slotSettings->GetGameData('TheExplorersEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('TheExplorersEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('TheExplorersEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('TheExplorersEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('TheExplorersEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('TheExplorersEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                {
                                    $state = 'choice';
                                    $slotSettings->SetGameData('TheExplorersEGTBonusStart', 1);
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . ']' . $bstate . ',"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"expand":[],"specialExpand":[],"gambles":5,"freespins":' . $fs . ',"jackpot":' . $isJack . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('TheExplorersEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('TheExplorersEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('TheExplorersEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('TheExplorersEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('TheExplorersEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('TheExplorersEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('TheExplorersEGTGambleStep', $slotSettings->GetGameData('TheExplorersEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('TheExplorersEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
