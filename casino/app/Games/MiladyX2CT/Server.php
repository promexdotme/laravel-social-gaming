            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'create_session':
                                $slotSettings->SetGameData('MiladyX2CTCards', [
                                    19, 
                                    44, 
                                    0, 
                                    37, 
                            ...
                            case 'init_game':
                                ...
                                $slotSettings->SetGameData('MiladyX2CTBonusWin', 0);
                                $slotSettings->SetGameData('MiladyX2CTFreeGames', 0);
                                $slotSettings->SetGameData('MiladyX2CTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('MiladyX2CTCurrentFreeGame0', 0);
                                $slotSettings->SetGameData('MiladyX2CTTotalWin', 0);
                                $slotSettings->SetGameData('MiladyX2CTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    if( $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame') < $slotSettings->GetGameData('MiladyX2CTFreeGames') && $slotSettings->GetGameData('MiladyX2CTFreeGames') > 0 ) 
                                    {
                            ...
                            case 'poll':
                                ...
                                {
                                    $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance() - $slotSettings->GetGameData('MiladyX2CTTotalWin')) * 100;
                                }
                            ...
                            case 'game_finished':
                                $slotSettings->SetGameData('MiladyX2CTBonusWin', 0);
                                $slotSettings->SetGameData('MiladyX2CTTotalWin', 0);
                                $responseData['game_finished'][0]['freeround_play'] = $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame0');
                            ...
                            case 'subgame_finished':
                                if( $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame0') < $slotSettings->GetGameData('MiladyX2CTFreeGames') && $slotSettings->GetGameData('MiladyX2CTFreeGames') > 0 ) 
                                {
                                    ...
                                    $response = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"result":' . json_encode($result) . ',"slotDenom":' . $slotSettings->GetGameData($slotSettings->slotId . 'Denom') . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('MiladyX2CTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData('MiladyX2CTFreeBalance') . ',"afterBalance":' . $slotSettings->GetGameData('MiladyX2CTFreeBalance') . ',"bonusWin":' . $slotSettings->GetGameData('MiladyX2CTBonusWin') . ',"totalWin":' . $reportWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Reels')) . '}}';
                                    if( $slotSettings->GetGameData('MiladyX2CTFreeGames') <= $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame0') ) 
                                    {
                                        if( $reportWin > 0 ) 
                                        {
                                    ...
                                    $balanceInCents = $slotSettings->GetGameData('MiladyX2CTFreeBalance');
                                    $responseData['subgame_finished'][0]['account_status']['credit'] = round($balanceInCents);
                                    $responseData['subgame_finished'][0]['freeround_play'] = $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame0');
                                    $responseData['subgame_finished'][1]['freeround_play'] = $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame0');
                            ...
                            case 'play':
                                ...
                                for( $spinLoop = 0; $spinLoop < $spinLoopLimit; $spinLoop++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] != 'freespin' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('MiladyX2CTCurrentFreeGame0', 0);
                                        $slotSettings->SetGameData('MiladyX2CTTotalWin', 0);
                                        $slotSettings->SetGameData('MiladyX2CTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    ...
                                    {
                                        $slotSettings->SetGameData('MiladyX2CTCurrentFreeGame', $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame') + 1);
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $slotSettings->SetGameData('MiladyX2CTBonusWin', $slotSettings->GetGameData('MiladyX2CTBonusWin') + $totalWin);
                                        $slotSettings->SetGameData('MiladyX2CTTotalWin', $slotSettings->GetGameData('MiladyX2CTTotalWin') + $totalWin);
                                        $balanceInCents = $slotSettings->GetGameData('MiladyX2CTFreeBalance');
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('MiladyX2CTTotalWin', $totalWin);
                                    }
                                    $fs = 0;
                                    if( $scattersCount >= 2 ) 
                                    {
                                    ...
                                    $winstring = '';
                                    $slotSettings->SetGameData('MiladyX2CTGambleStep', 5);
                                    $hist = $slotSettings->GetGameData('MiladyX2CTCards');
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $state = 'freespin';
                                        ...
                                        if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('MiladyX2CTBonusWin') > 0 ) 
                                        {
                                            $responseData['play'][0]['result'][$spinLoop]['is_free'] = true;
                                    ...
                                    $responseData['play'][0]['result'][$spinLoop]['total_win'] = $slotSettings->GetGameData('MiladyX2CTTotalWin') / $denom;
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"result":' . json_encode($responseData['play'][0]['result']) . ',"slotDenom":' . $denom . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('MiladyX2CTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('MiladyX2CTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeBalance') . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('MiladyX2CTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'gamble':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('MiladyX2CTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $hist = $slotSettings->GetGameData('MiladyX2CTCards');
                                $slotSettings->SetGameData('MiladyX2CTGambleStep', $slotSettings->GetGameData('MiladyX2CTGambleStep') - 1);
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isGambleWin = 0;
                                }
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('MiladyX2CTGambleStep', 0);
                                    if( $postData['continue_params']['double_key'] != 'red' && $postData['continue_params']['double_key'] != 'black' ) 
                                    {
                                        if( $postData['continue_params']['double_key'] == 'diamond' ) 
                                        {
                                ...
                                $slotSettings->SetGameData('MiladyX2CTCards', $hist);
