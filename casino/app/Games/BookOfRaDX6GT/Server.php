                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('BookOfRaDX6GTDealerCard');
                            $totalWin = $slotSettings->GetGameData('BookOfRaDX6GTTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('BookOfRaDX6GTTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('BookOfRaDX6GTDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('BookOfRaDX6GTExpSymbol', 'NONE');
                                $slotSettings->SetGameData('BookOfRaDX6GTBonusWin', 0);
                                $slotSettings->SetGameData('BookOfRaDX6GTFreeGames', 0);
                                $slotSettings->SetGameData('BookOfRaDX6GTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('BookOfRaDX6GTTotalWin', 0);
                                $slotSettings->SetGameData('BookOfRaDX6GTFreeBalance', 0);
                            }
                            else
                            {
                                $slotSettings->SetGameData('BookOfRaDX6GTCurrentFreeGame', $slotSettings->GetGameData('BookOfRaDX6GTCurrentFreeGame') + 1);
                            ...
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    for( $e = 1; $e <= 6; $e++ ) 
                                    {
                                        if( $reels['reel' . $e][0] == $slotSettings->GetGameData('BookOfRaDX6GTExpSymbol') || $reels['reel' . $e][1] == $slotSettings->GetGameData('BookOfRaDX6GTExpSymbol') || $reels['reel' . $e][2] == $slotSettings->GetGameData('BookOfRaDX6GTExpSymbol') ) 
                                        {
                                            $expReels[$e] = true;
                                            $expPayCnt++;
                                        }
                                    }
                                    if( $slotSettings->Paytable[$slotSettings->GetGameData('BookOfRaDX6GTExpSymbol')][$expPayCnt] * $postData['slotBet'] > 0 ) 
                                    {
                                        for( $ll = 0; $ll < $postData['slotLines']; $ll++ ) 
                                        {
                                            $expPay += ($slotSettings->Paytable[$slotSettings->GetGameData('BookOfRaDX6GTExpSymbol')][$expPayCnt] * $postData['slotBet']);
                                            $tmpStringWin = '{"lineWin":' . ($slotSettings->Paytable[$slotSettings->GetGameData('BookOfRaDX6GTExpSymbol')][$expPayCnt] * $postData['slotBet']) . ',"Count":' . $expPayCnt . ',"Line":' . $ll . ',"Win":' . $expPay . ',"stepWin":' . ($expPay + $totalWin);
                                            for( $e = 1; $e <= 6; $e++ ) 
                                            {
                                                if( $expReels[$e] ) 
                                                {
                                                    $tmpStringWin .= (',"winReel' . $e . '":[' . ($linesId[$ll][$e - 1] - 1) . ',"' . $slotSettings->GetGameData('BookOfRaDX6GTExpSymbol') . '"]');
                                                }
                                                else
                                                {
                            ...
                            {
                                $slotSettings->SetGameData('BookOfRaDX6GTTotalWin', $totalWin);
                                $slotSettings->SetGameData('BookOfRaDX6GTBonusWin', $slotSettings->GetGameData('BookOfRaDX6GTBonusWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('BookOfRaDX6GTTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('BookOfRaDX6GTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('BookOfRaDX6GTFreeGames', $slotSettings->GetGameData('BookOfRaDX6GTFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('BookOfRaDX6GTExpSymbol', $slotSettings->SymbolGameExp[0]);
                                    $slotSettings->SetGameData('BookOfRaDX6GTFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $reels = $tmpReels;
                            ...
                            if( $slotSettings->GetGameData('BookOfRaDX6GTExpSymbol') != 'NONE' ) 
                            {
                                $expSymbol = '"expSymbol":"' . $slotSettings->GetGameData('BookOfRaDX6GTExpSymbol') . '",';
                            }
                            else
                            {
                                $expSymbol = '';
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"slotExtraBet":"' . $postData['slotExtraBet'] . '",' . $expLinesStr . ',"expPay":' . $expPay . ',' . $expReelsStr . ',' . $expSymbol . '"bonusWin":' . $slotSettings->GetGameData('BookOfRaDX6GTBonusWin') . ',"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('BookOfRaDX6GTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BookOfRaDX6GTCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
