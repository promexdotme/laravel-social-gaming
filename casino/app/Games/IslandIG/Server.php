            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['action'] == 'risk' && $slotSettings->GetGameData('IslandIGWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['action'] ) 
                        {
                            case 'init':
                                ...
                                break;
                            case 'status':
                                if( !$slotSettings->HasGameData('IslandIGLines') || !$slotSettings->HasGameData('IslandIGBet') ) 
                                {
                                    $slotSettings->SetGameData('IslandIGLines', 1);
                                    $slotSettings->SetGameData('IslandIGBet', 1);
                                }
                                $balance = $slotSettings->GetBalance();
                                if( $slotSettings->GetGameData('IslandIGWin') > 0 ) 
                                {
                                    $balance = $balance - $slotSettings->GetGameData('IslandIGWin');
                                }
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","BetArray":[' . implode(',', $slotSettings->Bet) . '],"Credit":"' . floor($balance) . '","Currency":"' . $slotSettings->slotCurrency . '","Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","FuseBet":' . $slotSettings->fuseBet . ',"HelpCoef":[[0,0,0],[200,1000,5000],[100,500,2000],[30,100,500],[20,50,200],[10,30,100],[5,10,50],[3,5,20],[2,3,10]],"LastBet":' . $slotSettings->GetGameData('IslandIGBet') . ',"LastFuse":false,"LastLines":' . $slotSettings->GetGameData('IslandIGLines') . ',"MaxWin":675000,"RawCredit":"' . $balance . '","Reels":[[7,1,5,3,8],[5,3,6,7,7],[6,8,7,8,1]],"Win":0,"args":{},"cmd":"status","time":"20181111T111512"}';
                                break;
                            case 'start':
                                $balance = $slotSettings->GetBalance();
                                $bet = $postData['bet'];
                                ...
                                $slotSettings->SetGameData('IslandIGReels', $reelStr);
                                $slotSettings->SetGameData('IslandIGWin', $totalWin);
                                $slotSettings->SetGameData('IslandIGRisk', 1);
                                $slotSettings->SetGameData('IslandIGLines', $lines);
                                $slotSettings->SetGameData('IslandIGBet', $bet);
                                $slotSettings->SetGameData('IslandIGDealerCard', $card);
                                $rState = 'bet';
                                if( $isBonus ) 
                                {
                                    $rState = 'BG';
                            ...
                            case 'finish':
                                $balance = $slotSettings->GetBalance();
                                $slotSettings->SetGameData('IslandIGWin', 0);
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Credit":' . floor($balance) . ',"RawCredit":' . floor($balance) . ',"Reels":[' . $slotSettings->GetGameData('IslandIGReels') . '],"args":{},"cmd":"finish","time":"20181111T171830"}';
                                break;
                            case 'risk':
                                $idArr = [
                                    '2', 
                                ...
                                $dealerCard = $slotSettings->GetGameData('IslandIGDealerCard');
                                $isWinAccept = rand(1, 2);
                                $bank = $slotSettings->GetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''));
                                $curWin = $slotSettings->GetGameData('IslandIGWin');
                                $reportBet = $slotSettings->GetGameData('IslandIGWin');
                                $nextWin = $curWin * 2;
                                if( $slotSettings->MaxWin < ($nextWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isWinAccept = 0;
                                }
                                $slotSettings->SetGameData('IslandIGRisk', $slotSettings->GetGameData('IslandIGRisk') + 1);
                                $riskStep = $slotSettings->GetGameData('IslandIGRisk');
                                $otherCards = [];
                                ...
                                $balance = $slotSettings->GetBalance();
                                $slotSettings->SetGameData('IslandIGWin', $nextWin);
                                $slotSettings->SetGameData('IslandIGDealerCard', $nextCard);
                                $response = '{"cc":"' . $playerCardNum . '|' . $cardNum . '","Lines":' . $slotSettings->GetGameData('IslandIGLines') . ',"Bet":' . $slotSettings->GetGameData('IslandIGBet') . ',"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '","Reels":[' . $slotSettings->GetGameData('IslandIGReels') . '],"Credit":' . floor($balance) . ',"Dealer":' . $slotSettings->cardsID[$dealerCard] . ',"Other":[' . implode(',', $otherCards) . '],"Player":' . $slotSettings->cardsID[$playerCard] . ',"PrevWin":' . $curWin . ',"RawCredit":' . floor($balance) . ',"Step":' . $riskStep . ',"Win":' . $nextWin . ',"args":{},"cmd":"risk","time":"20181111T172027"}';
