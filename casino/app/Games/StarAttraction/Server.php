            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('StarAttractionDealerCard');
                        $totalWin = $slotSettings->GetGameData('StarAttractionTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('StarAttractionTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('StarAttractionDealerCard', $tmpDc);
                    ...
                    else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                    {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $bonusMpl = 1;
                            $slotSettings->SetGameData('StarAttractionBonusWin', 0);
                            $slotSettings->SetGameData('StarAttractionFreeGames', 0);
                            $slotSettings->SetGameData('StarAttractionCurrentFreeGame', 0);
                            $slotSettings->SetGameData('StarAttractionTotalWin', 0);
                            $slotSettings->SetGameData('StarAttractionFreeBalance', 0);
                        }
                        else
                        {
                            $slotSettings->SetGameData('StarAttractionCurrentFreeGame', $slotSettings->GetGameData('StarAttractionCurrentFreeGame') + 1);
                                ...
                                for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                        {
                                            ...
                                            {
                                                $cWins[$k] = $tmpWin;
                                                $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('StarAttractionBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                            }
                                        }
                                        if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                        {
                                            ...
                                            {
                                                $cWins[$k] = $tmpWin;
                                                $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('StarAttractionBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                            }
                                        }
                                        if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) ) 
                                        {
                                            ...
                                            {
                                                $cWins[$k] = $tmpWin;
                                                $tmpStringWin = '{"Count":4,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('StarAttractionBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":["none","none"]}';
                                            }
                                        }
                                        if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) ) 
                                        {
                                            ...
                                            {
                                                $cWins[$k] = $tmpWin;
                                                $tmpStringWin = '{"Count":5,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('StarAttractionBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":[' . ($linesId[$k][4] - 1) . ',"' . $s[4] . '"]}';
                                            }
                                        }
                                    }
                                }
                        ...
                        if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('StarAttractionFreeGames') <= $slotSettings->GetGameData('StarAttractionCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('StarAttractionTotalWin') + $totalWin > 0 ) 
                        {
                            $slotSettings->SetBalance($slotSettings->GetGameData('StarAttractionTotalWin') + $totalWin);
                        }
                        ...
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('StarAttractionBonusWin', $slotSettings->GetGameData('StarAttractionBonusWin') + $totalWin);
                            $slotSettings->SetGameData('StarAttractionTotalWin', $slotSettings->GetGameData('StarAttractionTotalWin') + $totalWin);
                            $totalWin = $slotSettings->GetGameData('StarAttractionBonusWin');
                            $Balance = $slotSettings->GetGameData('StarAttractionFreeBalance');
                        }
                        else
                        {
                            $slotSettings->SetGameData('StarAttractionTotalWin', $totalWin);
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('StarAttractionFreeGames') > 0 ) 
                            {
                                $slotSettings->SetGameData('StarAttractionFreeBalance', $Balance);
                                $slotSettings->SetGameData('StarAttractionBonusWin', $totalWin);
                                $slotSettings->SetGameData('StarAttractionFreeGames', $slotSettings->GetGameData('StarAttractionFreeGames') + $slotSettings->slotFreeCount);
                            }
                            else
                            {
                                $slotSettings->SetGameData('StarAttractionFreeBalance', $Balance);
                                $slotSettings->SetGameData('StarAttractionBonusWin', $totalWin);
                                $slotSettings->SetGameData('StarAttractionFreeGames', $slotSettings->slotFreeCount);
                            }
                        }
                        $jsSpin = '' . json_encode($reels) . '';
                        $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                        $winString = implode(',', $lineWins);
                        $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('StarAttractionFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('StarAttractionCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
