                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('SiXiangDealerCard');
                            $totalWin = $slotSettings->GetGameData('SiXiangTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('SiXiangTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('SiXiangDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'spin' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('SiXiangBonusWin', 0);
                                $slotSettings->SetGameData('SiXiangFreeGames', 0);
                                $slotSettings->SetGameData('SiXiangCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SiXiangTotalWin', 0);
                                $slotSettings->SetGameData('SiXiangFreeBalance', 0);
                            }
                            else
                            {
                                $slotSettings->SetGameData('SiXiangCurrentFreeGame', $slotSettings->GetGameData('SiXiangCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('SiXiangBonusWin', $slotSettings->GetGameData('SiXiangBonusWin') + $totalWin);
                                $slotSettings->SetGameData('SiXiangTotalWin', $slotSettings->GetGameData('SiXiangTotalWin') + $totalWin);
                                $totalWin = $slotSettings->GetGameData('SiXiangBonusWin');
                                $Balance = $slotSettings->GetGameData('SiXiangFreeBalance');
                            }
                            else
                            {
                                $slotSettings->SetGameData('SiXiangTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('SiXiangFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('SiXiangFreeBalance', $Balance);
                                    $slotSettings->SetGameData('SiXiangBonusWin', $totalWin);
                                    $slotSettings->SetGameData('SiXiangFreeGames', $slotSettings->GetGameData('SiXiangFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SiXiangFreeBalance', $Balance);
                                    $slotSettings->SetGameData('SiXiangBonusWin', $totalWin);
                                    $slotSettings->SetGameData('SiXiangFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"totalFreeGames":' . $slotSettings->GetGameData('SiXiangFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SiXiangCurrentFreeGame') . ',"bet":' . $postData['slotBet'] . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
