                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('ChineseKitchenPTMBonusWin', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTMFreeGames', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTMTotalWin', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTMFreeBalance', 0);
                                $slotSettings->SetGameData('ChineseKitchenPTMFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('ChineseKitchenPTMCurrentFreeGame', $slotSettings->GetGameData('ChineseKitchenPTMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || $s[1] == $csym || $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('ChineseKitchenPTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( $s[0] == $csym && $s[1] == $csym || $s[2] == $csym && $s[1] == $csym ) 
                                            {
                                                $mpl = 1;
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('ChineseKitchenPTMBonusWin', $slotSettings->GetGameData('ChineseKitchenPTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('ChineseKitchenPTMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('ChineseKitchenPTMTotalWin', $totalWin);
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('ChineseKitchenPTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('ChineseKitchenPTMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('ChineseKitchenPTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('ChineseKitchenPTMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '40066':
                                ...
                                $result_tmp[] = '3:::{"data":{"funNoticeGames":0,"funNoticePayouts":0,"gameGroup":"cm","minBet":0,"maxBet":0,"minPosBet":0,"maxPosBet":50000,"coinSizes":[' . implode(',', $gameBets) . ']},"ID":40025,"umid":21}';
                                break;
                            case '40036':
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    {
                                        $slotSettings->SetGameData($slotSettings->slotId . 'brokenGames', 'cm');
                                    }
                                }
