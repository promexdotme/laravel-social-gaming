                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'subscribe':
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('RainbowQueenEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('RainbowQueenEGTBonusWin', 0);
                                $slotSettings->SetGameData('RainbowQueenEGTFreeGames', 0);
                                $slotSettings->SetGameData('RainbowQueenEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('RainbowQueenEGTTotalWin', 0);
                                $slotSettings->SetGameData('RainbowQueenEGTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('RainbowQueenEGTCurrentFreeGame') < $slotSettings->GetGameData('RainbowQueenEGTFreeGames') && $slotSettings->GetGameData('RainbowQueenEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('RainbowQueenEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('RainbowQueenEGTTotalWin', 0);
                                    $slotSettings->SetGameData('RainbowQueenEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'allWilds', []);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'passedClovers', 0);
                                    $bonusMpl = 1;
                                }
                                ...
                                {
                                    $slotSettings->SetGameData('RainbowQueenEGTCurrentFreeGame', $slotSettings->GetGameData('RainbowQueenEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('RainbowQueenEGTBonusWin', $slotSettings->GetGameData('RainbowQueenEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('RainbowQueenEGTTotalWin', $slotSettings->GetGameData('RainbowQueenEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('RainbowQueenEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('RainbowQueenEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('RainbowQueenEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('RainbowQueenEGTFreeGames', $slotSettings->GetGameData('RainbowQueenEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('RainbowQueenEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('RainbowQueenEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('RainbowQueenEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('RainbowQueenEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"allWilds":[' . implode(',', $slotSettings->GetGameData($slotSettings->slotId . 'allWilds')) . '],"passedClovers":' . $slotSettings->GetGameData($slotSettings->slotId . 'passedClovers') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('RainbowQueenEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('RainbowQueenEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('RainbowQueenEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('RainbowQueenEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('RainbowQueenEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('RainbowQueenEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('RainbowQueenEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('RainbowQueenEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('RainbowQueenEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('RainbowQueenEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('RainbowQueenEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('RainbowQueenEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('RainbowQueenEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('RainbowQueenEGTGambleStep', $slotSettings->GetGameData('RainbowQueenEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('RainbowQueenEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
