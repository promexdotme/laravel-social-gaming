            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        $balanceInCents = floor(sprintf('%01.2f', $slotSettings->GetBalance() * ($slotSettings->CurrentDenom * 100)));
                        $result_tmp = [];
                        $aid = '';
                            ...
                            case 'Act18':
                                if( isset($postData['reqDat']) ) 
                                {
                                                ...
                                                for( $rl = 1; $rl <= 5; $rl++ ) 
                                                {
                                                    ...
                                                    for( $rs = 0; $rs <= 2; $rs++ ) 
                                                    {
                                                        if( $reels['reel' . $rl][$rs] == $csym || in_array($reels['reel' . $rl][$rs], $wild) ) 
                                                        {
                                                            ...
                                                            {
                                                                $wildMpl = $slotSettings->slotWildMpl;
                                                            }
                                                        }
                                                    }
                                                    if( !$isSeq ) 
                                    ...
                                    if( $scattersCount >= 3 ) 
                                    {
                                        ...
                                        {
                                            $balanceInCents0 = floor(sprintf('%01.2f', $slotSettings->GetBalance() * ($slotSettings->CurrentDenom * 100)));
