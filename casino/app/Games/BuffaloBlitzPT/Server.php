                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('BuffaloBlitzPTBonusWin', 0);
                                $slotSettings->SetGameData('BuffaloBlitzPTFreeGames', 0);
                                $slotSettings->SetGameData('BuffaloBlitzPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('BuffaloBlitzPTTotalWin', 0);
                                $slotSettings->SetGameData('BuffaloBlitzPTFreeBalance', 0);
                                $slotSettings->SetGameData('BuffaloBlitzPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('BuffaloBlitzPTCurrentFreeGame', $slotSettings->GetGameData('BuffaloBlitzPTCurrentFreeGame') + 1);
                                $tmpMpl = [
                                    2, 
                                    3, 
                                    5
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('BuffaloBlitzPTBonusWin', $slotSettings->GetGameData('BuffaloBlitzPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('BuffaloBlitzPTTotalWin', $totalWin);
                                $sID = 47752;
                                $rI = 5;
                            }
                            else
                            ...
                            if( $scattersCount >= 2 ) 
                            {
                                if( $slotSettings->GetGameData('BuffaloBlitzPTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('BuffaloBlitzPTFreeGames', $slotSettings->GetGameData('BuffaloBlitzPTFreeGames') + $slotSettings->slotFreeCountAdd[$scattersCount]);
                                }
                                else if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('BuffaloBlitzPTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('BuffaloBlitzPTBonusWin', 0);
                                    $slotSettings->SetGameData('BuffaloBlitzPTFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[4096],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('BuffaloBlitzPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BuffaloBlitzPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('BuffaloBlitzPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('BuffaloBlitzPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '40066':
                                ...
                                $result_tmp[] = '3:::{"data":{"funNoticeGames":0,"funNoticePayouts":0,"gameGroup":"bfb","minBet":0,"maxBet":0,"minPosBet":0,"maxPosBet":50000,"coinSizes":[' . implode(',', $gameBets) . ']},"ID":40025,"umid":21}';
                                break;
                            case '40036':
