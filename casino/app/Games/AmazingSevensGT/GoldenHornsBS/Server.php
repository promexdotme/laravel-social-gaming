                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            case 'ENTER':
                                ...
                                $result_tmp[0] = 'RESULT=OK&BALANCE=' . $balanceInCents . '&game_mode=free&server_type=AP&LASTHAND=NO&AUTOPLAY_VALUES=5|10|15|20|25|30|40|50|100&IBETVALUES=' . implode('', $slotSettings->Bet) . '&DEFCOIN=' . $bet . '&POSSIBLE_LINES=1&DEFAULTNUMLINES=1&DEFAULTBETPERLINE=1&IPAYOUT=288|88|18|13|8|6|3|2&BONUSBALANCE=0&GID=822&TIME=' . time() . '&END=0';
                                break;
                            case 'DOBONUS':
                            case 'PLACEBET':
                                $linesId = [];
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if( $s[0] == '0' || $s[1] == '0' || $s[2] == '0' ) 
                                                {
                                                    ...
                                                    $slotSettings->slotWildMpl = $wwmpl[0];
                                                }
                                                if( $s[0] == '0' && $s[1] == '0' && $s[2] == '0' ) 
                                                {
                                                    $slotSettings->slotWildMpl = 1;
                                                ...
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                                {
                                                    ...
                                                    $tmpWin = $slotSettings->Paytable['SYM_' . $csym][3] * $betline * $slotSettings->slotWildMpl;
                                                    if( $cWins[$k] < $tmpWin ) 
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '' . $csym . '-' . $slotSettings->Paytable['SYM_' . $csym][3] . '-' . $svp[0] . ',' . $svp[1] . ',' . $svp[2];
                                                    }
                                                }
                                            }
                                        }
                                ...
                                if( count($lineWins) > 0 ) 
                                {
                                    $winString = '&MULTIPLIER=' . $slotSettings->slotWildMpl . '&PAYOUTS=' . implode('|', $lineWins);
                                }
                                ...
                                $result_tmp[0] = 'RESULT=OK&BALANCE=' . $balanceInCents . '&STOPREEL=' . $curReels . '&BONUSBALANCE=0&GID=822&TIME=' . time() . '&END=0' . $winString . $freeDataStr;
                                break;
                        }
