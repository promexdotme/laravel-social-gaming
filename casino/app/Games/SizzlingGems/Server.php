                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('SizzlingGemsDealerCard');
                            $totalWin = $slotSettings->GetGameData('SizzlingGemsTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('SizzlingGemsTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('SizzlingGemsDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('SizzlingGemsBonusWin', 0);
                                $slotSettings->SetGameData('SizzlingGemsFreeGames', 0);
                                $slotSettings->SetGameData('SizzlingGemsCurrentFreeGame', 0);
                                $slotSettings->SetGameData('SizzlingGemsTotalWin', 0);
                                $slotSettings->SetGameData('SizzlingGemsFreeBalance', 0);
                            }
                            else
                            {
                                $slotSettings->SetGameData('SizzlingGemsCurrentFreeGame', $slotSettings->GetGameData('SizzlingGemsCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SizzlingGemsBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SizzlingGemsBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SizzlingGemsBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":4,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SizzlingGemsBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":5,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('SizzlingGemsBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":[' . ($linesId[$k][4] - 1) . ',"' . $s[4] . '"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('SizzlingGemsBonusWin', $slotSettings->GetGameData('SizzlingGemsBonusWin') + $totalWin);
                                $slotSettings->SetGameData('SizzlingGemsTotalWin', $slotSettings->GetGameData('SizzlingGemsTotalWin') + $totalWin);
                                $totalWin = $slotSettings->GetGameData('SizzlingGemsBonusWin');
                                $Balance = $slotSettings->GetGameData('SizzlingGemsFreeBalance');
                            }
                            else
                            {
                                $slotSettings->SetGameData('SizzlingGemsTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('SizzlingGemsFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('SizzlingGemsFreeBalance', $Balance);
                                    $slotSettings->SetGameData('SizzlingGemsBonusWin', $totalWin);
                                    $slotSettings->SetGameData('SizzlingGemsFreeGames', $slotSettings->GetGameData('SizzlingGemsFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('SizzlingGemsFreeBalance', $Balance);
                                    $slotSettings->SetGameData('SizzlingGemsBonusWin', $totalWin);
                                    $slotSettings->SetGameData('SizzlingGemsFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"totalFreeGames":' . $slotSettings->GetGameData('SizzlingGemsFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('SizzlingGemsCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
