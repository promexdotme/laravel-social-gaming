                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $denoms = [];
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"currencyISO":"' . $slotSettings->slotCurrency . '","denomination":' . ($slotSettings->CurrentDenomGame * 100) . ',"chipColumnMaxbet":240000,"chipColumnMinbet":' . $gameBets[0] . ',"chipCornerMaxbet":80000,"chipCornerMinbet":' . $gameBets[0] . ',"chipDozensMaxbet":240000,"chipDozensMinbet":' . $gameBets[0] . ',"chipEvenMaxbet":360000,"chipEvenMinbet":' . $gameBets[0] . ',"chipLineMaxbet":120000,"chipLineMinbet":' . $gameBets[0] . ',"chipSplitMaxbet":40000,"chipSplitMinbet":' . $gameBets[0] . ',"chipStraightMaxbet":20000,"chipStraightMinbet":' . $gameBets[0] . ',"chipStreetMaxbet":60000,"chipStreetMinbet":' . $gameBets[0] . ',"gameMaxbet":4000000,"gameMinbet":' . $gameBets[0] . ',"serverWincoefColumn":3,"serverWincoefCorner":9,"serverWincoefDozens":3,"serverWincoefEven":2,"serverWincoefLine":6,"serverWincoefSplit":18,"serverWincoefStraight":36,"serverWincoefStreet":12,"jackpot":true,"jackpotMinBet":1,"jackpotMaxBet":100000},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"c729a7f1239b8189386c0f7cd0d78876","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('VirtualRouletteEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('VirtualRouletteEGTBonusWin', 0);
                                $slotSettings->SetGameData('VirtualRouletteEGTFreeGames', 0);
                                $slotSettings->SetGameData('VirtualRouletteEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('VirtualRouletteEGTTotalWin', 0);
                                $slotSettings->SetGameData('VirtualRouletteEGTFreeBalance', 0);
                                $gtype = 1;
                                $lines = 10;
                            ...
                            case 'bet':
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    $winBets = [];
                                    $loseBets = [];
                                    $postData['bets'] = (array)$postData['bet']['bet'];
                                    foreach( $postData['bets'] as $key => $vl ) 
                                    {
                                        $allbet += ($vl / 100);
                                        $curNums = [];
                                        $curNums[0] = $slotSettings->GetFieldName($key);
                                        $curNums[1] = $slotSettings->NumbersId[$key];
                                        ...
                                        {
                                            $winBets[] = '"' . $key . '":' . $vl;
                                            $curWin = $paysArr[$curNums[0]] * ($vl / 100);
                                            $totalWin += $curWin;
                                        }
                                        else
                                        {
                                ...
                                $reportWin = $totalWin;
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":1,"slotBet":' . $allbet . ',"totalFreeGames":0,"currentFreeGames":0,"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":0,"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":[]}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('VirtualRouletteEGTTotalWin', $totalWin);
                                $slotSettings->SetGameData('VirtualRouletteEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('VirtualRouletteEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('VirtualRouletteEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                                $result_tmp[] = '{"complex":{"winNumber":' . $randNumber . ',"winBets":{' . implode(',', $winBets) . '},"loseBets":{' . implode(',', $loseBets) . '},"gameCommand":"bet","history":[],"jackpot":false},"state":"idle","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":1839855988884,"balance":' . $balanceInCents . ',"sessionKey":"571429166117d3e2fda6238b7b15e9d8","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('VirtualRouletteEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('VirtualRouletteEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('VirtualRouletteEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('VirtualRouletteEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('VirtualRouletteEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('VirtualRouletteEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('VirtualRouletteEGTGambleStep', $slotSettings->GetGameData('VirtualRouletteEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('VirtualRouletteEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('VirtualRouletteEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('VirtualRouletteEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
