            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        if( $postData['act'] == '464529TBNS' && $slotSettings->GetGameData('FenixPlay27DeluxeWDTotalWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['act'] ) 
                        {
                            ...
                            case 'WXMNPP7H8F':
                                ...
                                $slotSettings->SetGameData('FenixPlay27DeluxeWDBonusWin', 0);
                                $slotSettings->SetGameData('FenixPlay27DeluxeWDFreeGames', 0);
                                $slotSettings->SetGameData('FenixPlay27DeluxeWDCurrentFreeGame', 0);
                                $slotSettings->SetGameData('FenixPlay27DeluxeWDTotalWin', 0);
                                $slotSettings->SetGameData('FenixPlay27DeluxeWDFreeBalance', 0);
                                $slotSettings->SetGameData('FenixPlay27DeluxeWDFreeStartWin', 0);
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('FenixPlay27DeluxeWDBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                    }
                                                }
                                            }
                                        }
                                ...
                                $reportWin = $totalWin;
                                $slotSettings->SetGameData('FenixPlay27DeluxeWDTotalWin', $totalWin);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $winString = implode(',', $lineWins);
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('FenixPlay27DeluxeWDFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('FenixPlay27DeluxeWDCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('FenixPlay27DeluxeWDBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('FenixPlay27DeluxeWDFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case '464529TBNS':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('FenixPlay27DeluxeWDTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $winState = 0;
