                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,20,100],"multiplier":1},"1":{"coef":[5,20,100],"multiplier":1},"2":{"coef":[5,20,100],"multiplier":1},"3":{"coef":[5,20,100],"multiplier":1},"4":{"coef":[20,30,125],"multiplier":1},"5":{"coef":[20,100,200],"multiplier":1},"6":{"coef":[20,100,200],"multiplier":1},"7":{"coef":[30,125,350],"multiplier":1},"8":{"coef":[30,125,350],"multiplier":1},"9":{"coef":[5,40,350,500],"multiplier":1},"10":{"coef":[5,50,500,1000],"multiplier":1},"12":{"coef":[5,20,100],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[2,4,10,3,2,7,0,6,1,4,9,0,5,6,12,2,10,4,5,8],[7,8,0,11,11,11,4,6,0,5,7,0,9,12,5,0,1,2,5,1],[0,3,11,11,11,0,6,12,0,8,3,0,6,1,4,10,2,7,4,5],[3,6,0,1,7,0,4,8,0,2,6,8,5,7,11,11,11,4,10,2],[2,10,3,0,1,7,0,5,6,12,9,4,7,3,0,6,5,1,8,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('KangarooLandEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('KangarooLandEGTBonusWin', 0);
                                $slotSettings->SetGameData('KangarooLandEGTFreeGames', 0);
                                $slotSettings->SetGameData('KangarooLandEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('KangarooLandEGTTotalWin', 0);
                                $slotSettings->SetGameData('KangarooLandEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('KangarooLandEGTCurrentFreeGame') < $slotSettings->GetGameData('KangarooLandEGTFreeGames') && $slotSettings->GetGameData('KangarooLandEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('KangarooLandEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('KangarooLandEGTTotalWin', 0);
                                    $slotSettings->SetGameData('KangarooLandEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('KangarooLandEGTCurrentFreeGame', $slotSettings->GetGameData('KangarooLandEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('KangarooLandEGTBonusWin', $slotSettings->GetGameData('KangarooLandEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('KangarooLandEGTTotalWin', $slotSettings->GetGameData('KangarooLandEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('KangarooLandEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('KangarooLandEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('KangarooLandEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('KangarooLandEGTFreeGames', $slotSettings->GetGameData('KangarooLandEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('KangarooLandEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('KangarooLandEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('KangarooLandEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('KangarooLandEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('KangarooLandEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('KangarooLandEGTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('KangarooLandEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('KangarooLandEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('KangarooLandEGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(0, 6));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('KangarooLandEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('KangarooLandEGTBonusWin');
                                    $state = 'freespin';
                                    if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('KangarooLandEGTBonusWin') > 0 ) 
                                    {
                                        $state = 'gamble';
                                    }
                                }
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"expand":[],"specialExpand":[],"gambles":5,"freespins":' . $sgwin . ',"jackpot":' . $isJack . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('KangarooLandEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('KangarooLandEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('KangarooLandEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('KangarooLandEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('KangarooLandEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('KangarooLandEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('KangarooLandEGTGambleStep', $slotSettings->GetGameData('KangarooLandEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('KangarooLandEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('KangarooLandEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('KangarooLandEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
