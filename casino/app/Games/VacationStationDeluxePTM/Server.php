                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('VacationStationDeluxePTMBonusWin', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTMFreeGames', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTMTotalWin', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTMFreeBalance', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTMFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('VacationStationDeluxePTMCurrentFreeGame', $slotSettings->GetGameData('VacationStationDeluxePTMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('VacationStationDeluxePTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('VacationStationDeluxePTMBonusWin', $slotSettings->GetGameData('VacationStationDeluxePTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('VacationStationDeluxePTMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('VacationStationDeluxePTMTotalWin', $totalWin);
                            ...
                            $result_tmp[] = '3:::{"data":{"credit":' . $balanceInCents . ',"results":[' . $symStr . '],"windowId":"pomAnz"},"ID":40022,"umid":34}';
                            $result_tmp[] = '3:::{"data":{"typeBalance":0,"currency":"' . $slotSettings->slotCurrency . '","balanceInCents":' . $balanceInCents . ',"deltaBalanceInCents":1},"ID":40085}';
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('VacationStationDeluxePTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('VacationStationDeluxePTMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('VacationStationDeluxePTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('VacationStationDeluxePTMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
