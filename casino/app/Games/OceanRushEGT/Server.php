            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'bonuschoice':
                                if( $slotSettings->GetGameData('OceanRushEGTBonusStart') != 1 ) 
                                {
                                    exit();
                                }
                                $allbet = $slotSettings->GetGameData('OceanRushEGTBonusBet');
                                $choice = (int)$postData['bet']['choice'];
                                $curWin = $slotSettings->GetGameData('OceanRushEGTChoiceWinAmount');
                                $scnt = $slotSettings->GetGameData('OceanRushEGTChoiceCount');
                                $scWinsCoef = $curWin[2];
                                shuffle($scWinsCoef);
                                $scWins = [
                                ...
                                $slotSettings->SetGameData('OceanRushEGTBonusStart', 2);
                                ...
                                break;
                            case 'choice':
                                if( $slotSettings->GetGameData('OceanRushEGTBonusStart') != 1 && $slotSettings->GetGameData('OceanRushEGTBonusStart') != 2 ) 
                                {
                                    exit();
                                }
                                $ChoiceArr = $slotSettings->GetGameData('OceanRushEGTChoiceArr');
                                $fs = $slotSettings->GetGameData('OceanRushEGTFreeGames');
                                $fw = $slotSettings->GetGameData('OceanRushEGTMultiplier');
                                $choice = (int)$postData['bet']['choice'];
                                $opt = [];
                                ...
                                if( count($ChoiceArr) >= 3 && $slotSettings->GetGameData('OceanRushEGTFreeGames') > 0 ) 
                                {
                                    $isEnd = true;
                                    $pl_cur = 'true';
                                    $slotSettings->SetGameData('OceanRushEGTBonusStart', 0);
                                }
                                $state = 'choice';
                                $closed = '';
                                if( $isEnd ) 
                                ...
                                $slotSettings->SetGameData('OceanRushEGTChoiceArr', $ChoiceArr);
                                $slotSettings->SetGameData('OceanRushEGTFreeGames', $fs + $cfr_cur);
                                $slotSettings->SetGameData('OceanRushEGTMultiplier', $fw + $cfr0_cur);
                                $result_tmp[] = '{"complex":{"choice":{"pos":' . $choice . ',"multiplier":' . $slotSettings->GetGameData('OceanRushEGTMultiplier') . ',"freespins":' . $slotSettings->GetGameData('OceanRushEGTFreeGames') . ',"mult":' . $cfr0_cur . ',"fs":' . $cfr_cur . ',"play":' . $pl_cur . ',"closed":[' . $closed . ']},"freespins":' . $slotSettings->GetGameData('OceanRushEGTFreeGames') . ',"multiplier":1,"gameCommand":"choice"},"state":"' . $state . '","winAmount":' . ($slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":1789706666325,"sessionKey":"46e640104224cef8245c4329c25ef561","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":1567196976582}';
                                break;
                            case 'login':
                            ...
                            case 'subscribe':
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('OceanRushEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('OceanRushEGTBonusWin', 0);
                                $slotSettings->SetGameData('OceanRushEGTFreeGames', 0);
                                $slotSettings->SetGameData('OceanRushEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('OceanRushEGTTotalWin', 0);
                                $slotSettings->SetGameData('OceanRushEGTFreeBalance', 0);
                                $slotSettings->SetGameData('OceanRushEGTMultiplier', 1);
                                $slotSettings->SetGameData('OceanRushEGTFreeWild', -1);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('OceanRushEGTCurrentFreeGame') < $slotSettings->GetGameData('OceanRushEGTFreeGames') && $slotSettings->GetGameData('OceanRushEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('OceanRushEGTBonusStart', 0);
                                    $slotSettings->SetGameData('OceanRushEGTBonusBet', $allbet);
                                    $slotSettings->SetGameData('OceanRushEGTBonusWin', 0);
                                    $slotSettings->SetGameData('OceanRushEGTFreeGames', 0);
                                    $slotSettings->SetGameData('OceanRushEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('OceanRushEGTTotalWin', 0);
                                    $slotSettings->SetGameData('OceanRushEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData('OceanRushEGTMultiplier', 1);
                                    $slotSettings->SetGameData('OceanRushEGTFreeWild', -1);
                                    $bonusMpl = 1;
                                }
                                ...
                                {
                                    $slotSettings->SetGameData('OceanRushEGTCurrentFreeGame', $slotSettings->GetGameData('OceanRushEGTCurrentFreeGame') + 1);
                                    $bonusMpl = $bonusMpl = $slotSettings->GetGameData('OceanRushEGTMultiplier');
                                }
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $totalWin <= $spinWinLimit && $scattersCount2 >= 3 ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('OceanRushEGTChoiceWinAmount', [
                                            $scf[0], 
                                            $scattersWin3, 
                                            $scf
                                        ]);
                                        $slotSettings->SetGameData('OceanRushEGTChoiceCount', $scattersCount2);
                                    }
                                    if( $i > 1000 ) 
                                    {
                                        $winType = 'none';
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('OceanRushEGTBonusWin', $slotSettings->GetGameData('OceanRushEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('OceanRushEGTTotalWin', $slotSettings->GetGameData('OceanRushEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('OceanRushEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('OceanRushEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 && $postData['slotEvent'] != 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('OceanRushEGTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('OceanRushEGTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('OceanRushEGTFreeGames', 0);
                                    $slotSettings->SetGameData('OceanRushEGTMultiplier', 1);
                                    $slotSettings->SetGameData('OceanRushEGTFreeWild', rand(5, 7));
                                    $slotSettings->SetGameData('OceanRushEGTChoiceArr', []);
                                    $fs = $slotSettings->GetGameData('OceanRushEGTFreeGames');
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"Multiplier":' . $slotSettings->GetGameData('OceanRushEGTMultiplier') . ',"FreeWild":' . $slotSettings->GetGameData('OceanRushEGTFreeWild') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('OceanRushEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('OceanRushEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('OceanRushEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('OceanRushEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('OceanRushEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('OceanRushEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('OceanRushEGTBonusWin');
                                    $state = 'freespin';
                                    if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('OceanRushEGTBonusWin') > 0 ) 
                                    {
                                        $state = 'gamble';
                                    }
                                }
                                ...
                                if( $scattersCount2 >= 3 ) 
                                {
                                    $state = 'bonuschoice';
                                    $slotSettings->SetGameData('OceanRushEGTTotalWin', $totalWin);
                                    $slotSettings->SetGameData('OceanRushEGTBonusStart', 1);
                                    $slotSettings->SetGameData('OceanRushEGTBonusCnt', $scattersCount2);
                                ...
                                {
                                    $state = 'freespin';
                                    $sgwin = $slotSettings->GetGameData('OceanRushEGTFreeGames');
                                    $slotSettings->SetGameData('OceanRushEGTFreeGames', $slotSettings->GetGameData('OceanRushEGTFreeGames') + 1);
                                }
                                if( !isset($sgwin) ) 
                                {
                                    $fs = 0;
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"choices": 0,"expand":[]' . $bstate . ',"multiplier":' . $slotSettings->GetGameData('OceanRushEGTMultiplier') . ',"wildcard":' . $slotSettings->GetGameData('OceanRushEGTFreeWild') . ',"specialExpand":[],"gambles":5,"freespins":' . $fs . ',"jackpot":' . $isJack . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('OceanRushEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('OceanRushEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('OceanRushEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('OceanRushEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('OceanRushEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('OceanRushEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('OceanRushEGTGambleStep', $slotSettings->GetGameData('OceanRushEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('OceanRushEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
