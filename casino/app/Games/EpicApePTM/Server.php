                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('EpicApePTMBonusWin', 0);
                                $slotSettings->SetGameData('EpicApePTMFreeGames', 0);
                                $slotSettings->SetGameData('EpicApePTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('EpicApePTMTotalWin', 0);
                                $slotSettings->SetGameData('EpicApePTMFreeBalance', 0);
                                $slotSettings->SetGameData('EpicApePTMFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('EpicApePTMCurrentFreeGame', $slotSettings->GetGameData('EpicApePTMCurrentFreeGame') + 1);
                                $tmpMpl = [
                                    2, 
                                    3, 
                                    5
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('EpicApePTMBonusWin', $slotSettings->GetGameData('EpicApePTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('EpicApePTMTotalWin', $totalWin);
                                $sID = 47752;
                                $rI = 5;
                            }
                            else
                            ...
                            if( $scattersCount >= 2 ) 
                            {
                                if( $slotSettings->GetGameData('EpicApePTMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('EpicApePTMFreeGames', $slotSettings->GetGameData('EpicApePTMFreeGames') + $slotSettings->slotFreeCountAdd[$scattersCount]);
                                }
                                else if( $scattersCount >= 3 ) 
                                {
                                    $slotSettings->SetGameData('EpicApePTMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('EpicApePTMBonusWin', 0);
                                    $slotSettings->SetGameData('EpicApePTMFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[4096],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('EpicApePTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('EpicApePTMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('EpicApePTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('EpicApePTMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '40066':
                                ...
                                $result_tmp[] = '3:::{"data":{"funNoticeGames":0,"funNoticePayouts":0,"gameGroup":"epa","minBet":0,"maxBet":0,"minPosBet":0,"maxPosBet":50000,"coinSizes":[' . implode(',', $gameBets) . ']},"ID":40025,"umid":21}';
                                break;
                            case '40036':
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    {
                                        $slotSettings->SetGameData($slotSettings->slotId . 'brokenGames', 'epa');
                                    }
                                }
