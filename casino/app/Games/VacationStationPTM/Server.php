                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('VacationStationPTMBonusWin', 0);
                                $slotSettings->SetGameData('VacationStationPTMFreeGames', 0);
                                $slotSettings->SetGameData('VacationStationPTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('VacationStationPTMTotalWin', 0);
                                $slotSettings->SetGameData('VacationStationPTMFreeBalance', 0);
                                $slotSettings->SetGameData('VacationStationPTMFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('VacationStationPTMCurrentFreeGame', $slotSettings->GetGameData('VacationStationPTMCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('VacationStationPTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('VacationStationPTMBonusWin', $slotSettings->GetGameData('VacationStationPTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('VacationStationPTMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('VacationStationPTMTotalWin', $totalWin);
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('VacationStationPTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('VacationStationPTMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('VacationStationPTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('VacationStationPTMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
