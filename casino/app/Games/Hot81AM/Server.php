                    ...
                    switch( $gameData['slotEvent'] ) 
                    {
                        ...
                        case 'A/u25':
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $slotSettings->SetGameData('Hot81AMLines', $curLines);
                                $freeMpl = '11';
                                $slotSettings->SetGameData('Hot81AMBonusWin', $lastEvent->serverResponse->bonusWin);
                                $slotSettings->SetGameData('Hot81AMFreeGames', $lastEvent->serverResponse->totalFreeGames);
                                $slotSettings->SetGameData('Hot81AMCurrentFreeGame', $lastEvent->serverResponse->currentFreeGames);
                                $slotSettings->SetGameData('Hot81AMTotalWin', 0);
                                $slotSettings->SetGameData('Hot81AMFreeBalance', 0);
                                $slotSettings->SetGameData('Hot81AMFreeStartWin', 0);
                                $tFree = dechex($slotSettings->GetGameData('Hot81AMFreeGames'));
                                $cFree = dechex($slotSettings->GetGameData('Hot81AMFreeGames') - $slotSettings->GetGameData('Hot81AMCurrentFreeGame'));
                                $freeInfo = strlen($tFree) . $tFree . strlen($cFree) . $cFree;
                                $stateWin = $slotSettings->HexFormat($slotSettings->GetGameData('Hot81AMBonusWin') * $floatBet);
                                if( $slotSettings->GetGameData('Hot81AMCurrentFreeGame') < $slotSettings->GetGameData('Hot81AMFreeGames') && $slotSettings->GetGameData('Hot81AMFreeGames') > 0 ) 
                                {
                                    $slotState = '6';
                            ...
                            {
                                $slotSettings->SetGameData('Hot81AMLines', $fixedLinesFormated);
                                $slotState = '4';
                            ...
                            break;
                        case 'A/u250':
                            $fixedLinesFormated = $slotSettings->GetGameData('Hot81AMLines');
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $lastEvent = $slotSettings->GetHistory();
                            if( $lastEvent != 'NULL' ) 
                            {
                        ...
                        case 'A/u251':
                            if( $postData['spinType'] == 'regular' && $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('Hot81AMBonusWin', 0);
                                $slotSettings->SetGameData('Hot81AMFreeGames', 0);
                                $slotSettings->SetGameData('Hot81AMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Hot81AMTotalWin', 0);
                                $slotSettings->SetGameData('Hot81AMFreeBalance', 0);
                                $slotSettings->SetGameData('Hot81AMFreeStartWin', 0);
                            }
                            if( $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') < $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') ) 
                            {
                                ...
                                $slotSettings->SetGameData('Hot81AMCurrentFreeGame', $slotSettings->GetGameData('Hot81AMCurrentFreeGame') + 1);
                                ...
                                for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if( $wsym[0] > 0 && $wsym[1] > 0 && $wsym[2] > 0 && $wsym[3] > 0 ) 
                                        {
                                            ...
                                            $offsetMpl = 1;
                                            $offsetMpl0 = 1;
                                            $sMpl = $cntsym[0] * $cntsym[1] * $cntsym[2] * $cntsym[3];
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('Hot81AMFreeGames') <= $slotSettings->GetGameData('Hot81AMCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('Hot81AMBonusWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('Hot81AMBonusWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('Hot81AMBonusWin', $slotSettings->GetGameData('Hot81AMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('Hot81AMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('Hot81AMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                ...
                                if( $slotSettings->GetGameData('Hot81AMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('Hot81AMFreeGames', $slotSettings->GetGameData('Hot81AMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('Hot81AMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('Hot81AMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('Hot81AMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $reels = $reelsTmp;
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"symDouble":' . $symDouble . ',"slotLines":' . $lines . ',"slotBet":' . $betCnt . ',"totalFreeGames":' . $slotSettings->GetGameData('Hot81AMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('Hot81AMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('Hot81AMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('Hot81AMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            for( $i = 0; $i < count($slotSettings->SymbolGame); $i++ ) 
                            {
                                ...
                                $winLinesFormated .= $slotSettings->HexFormat(round($cWins[$i] * 10));
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                ...
                                $cFree = dechex($slotSettings->GetGameData('Hot81AMFreeGames') - $slotSettings->GetGameData('Hot81AMCurrentFreeGame'));
                                $gameState = '06';
                                if( $slotSettings->GetGameData('Hot81AMFreeGames') <= $slotSettings->GetGameData('Hot81AMCurrentFreeGame') ) 
                                {
                                    $gameState = '0c';
                                }
                                if( $scattersCount >= 3 ) 
                                ...
                                $totalWin = $slotSettings->GetGameData('Hot81AMBonusWin');
                                $bonusMpl = $slotSettings->slotFreeMpl;
                            }
                            else
                            {
                            ...
                            $response = '1' . $gameState . '010' . $balanceFormated . $slotSettings->HexFormat(round($totalWin * 100)) . $reelSate . $bet_h . $ln_h . $freeInfo . $freeWinState . $slotSettings->HexFormat($bonusMpl) . '1010' . $reelSate . '0' . $symCountAll . $winLinesFormated . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards')) . '#' . $dbgstr;
                            $response .= ('_' . json_encode($reels));
                            $slotSettings->SetGameData('Hot81AMDoubleAnswer', $reelSate . $bet_h . $ln_h . $freeInfo . $freeWinState . $slotSettings->HexFormat($bonusMpl) . '1010' . $reelSate . '0' . $symCountAll . $winLinesFormated);
                            $slotSettings->SetGameData('Hot81AMDoubleBalance', $balanceFormated);
                            $slotSettings->SetGameData('Hot81AMDoubleWin', $totalWin);
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                            ...
                            $slotSettings->SetGameData('Hot81AMCollectP0', $response_collect0);
                            $gameState = '04';
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            $slotSettings->SetGameData('Hot81AMCollect', $response_collect);
                            break;
                        case 'A/u254':
                            $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                            $response = $slotSettings->GetGameData('Hot81AMCollect');
                            break;
                        case 'A/u257':
                            $doubleWin = rand(1, 2);
                            $winall = $slotSettings->GetGameData('Hot81AMDoubleWin');
                            $dbet = $winall;
                            $daction = $tmpPar[1];
                            ...
                            $response = '107010' . $slotSettings->GetGameData('Hot81AMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('Hot81AMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            $response = '107010' . $slotSettings->GetGameData('Hot81AMDoubleBalance') . strlen($winall_h) . $winall_h . '21b21321e21110090810101010101010101010100810101010101c1010' . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards')) . '#101010';
                            $slotSettings->SetGameData('Hot81AMDoubleWin', $winall);
                            $slotSettings->SetGameData('Hot81AMTotalWin', $winall);
                            $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            $response_collect = '104010' . $balanceFormated . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('Hot81AMCollectP0');
                            $slotSettings->SetGameData('Hot81AMCollect', $response_collect);
                        ...
                        case 'A/u258':
                            ...
                            $slotSettings->SetGameData('Hot81AMDoubleBalance', $rtnBalance);
                            $winall = sprintf('%01.2f', $winall) * $floatBet;
                            $winall_h1 = str_replace('.', '', $winall . '');
                            $winall_h = dechex($winall_h1);
                            $doubleCards = '26280b2714161d0c';
                            $response = '108010' . $slotSettings->GetGameData('Hot81AMDoubleBalance') . strlen($winall_h) . $winall_h . $slotSettings->GetGameData('Hot81AMDoubleAnswer') . implode('', $slotSettings->GetGameData($slotSettings->slotId . 'Cards'));
                            break;
                    }
