            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            $response = '{"status":"success","microtime":0.0026731491088867,"dateTime":"2020-02-13 13:23:32","error":"","content":{"cmd":"sessionInfo","serverMathematics":"\/game\/PompeiiAT\/server","serverResources":"","sessionId":"54594109_4133a64673d3883d42cd003ee905ba3e","exitUlt":"\/","exitUrl":"\/","id":"341","name":"GameName","currency":"ALL","language":"en","type":"aristocrat","systemName":"pompeii","version":"2","mobile":"1"}}';
                        }
                        ...
                        else if( $postData['slotEvent'] == 'gamePick' ) 
                        {
                            ...
                            if( $_POST['pick'] == 0 ) 
                            {
                                $slotSettings->SetGameData('PompeiiATMpl', 10);
                                $slotSettings->SetGameData('PompeiiATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PompeiiATFreeGames', 5);
                            }
                            else if( $_POST['pick'] == 1 ) 
                            {
                                $slotSettings->SetGameData('PompeiiATMpl', 5);
                                $slotSettings->SetGameData('PompeiiATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PompeiiATFreeGames', 10);
                            }
                            else if( $_POST['pick'] == 2 ) 
                            {
                                $slotSettings->SetGameData('PompeiiATMpl', 3);
                                $slotSettings->SetGameData('PompeiiATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PompeiiATFreeGames', 15);
                            }
                            else if( $_POST['pick'] == 3 ) 
                            {
                                $slotSettings->SetGameData('PompeiiATMpl', 2);
                                $slotSettings->SetGameData('PompeiiATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PompeiiATFreeGames', 20);
                            }
                            $response = '{"status":"success","microtime":0.0093810558319092,"dateTime":"2020-02-15 14:26:21","error":"","content":{"session":"54990161_9709e13bca7763b46396cda1868d2e7e","cmd":"gamePick","balance":' . $balanceInCents . ',"win":0,"symbols":{"1":{"1":"symbol_6","2":"symbol_9","3":"symbol_13"},"2":{"1":"symbol_10","2":"symbol_5","3":"symbol_8"},"3":{"1":"symbol_10","2":"symbol_13","3":"symbol_11"},"4":{"1":"symbol_13","2":"symbol_10","3":"symbol_6"},"5":{"1":"symbol_3","2":"symbol_8","3":"symbol_1"}},"winLines":[],"freeSpin":{"id":"54990161_20200215132548","current":0,"add":' . $slotSettings->GetGameData('PompeiiATFreeGames') . ',"total":' . $slotSettings->GetGameData('PompeiiATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('PompeiiATBonusWin') * 100) . ',"multiplayer":' . $slotSettings->GetGameData('PompeiiATMpl') . ',"pick":' . $_POST['pick'] . '},"actionId":"54990161_10_60","hash":"1ace145628d930d28bdf38789a28a9d7"}}';
                        }
                        else if( $postData['slotEvent'] == 'slotGamble' ) 
                        {
                            ...
                            $dealerCard = '';
                            $totalWin = $slotSettings->GetGameData('PompeiiATTotalWin');
                            $gambleWin = 0;
                            $statBet = $totalWin;
                            ...
                            $slotSettings->SetGameData('PompeiiATTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('PompeiiATBonusWin', 0);
                                $slotSettings->SetGameData('PompeiiATFreeGames', 0);
                                $slotSettings->SetGameData('PompeiiATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PompeiiATTotalWin', 0);
                                $slotSettings->SetGameData('PompeiiATFreeBalance', 0);
                                $slotSettings->SetGameData('PompeiiATMpl', 1);
                            }
                            else
                            {
                                $slotSettings->SetGameData('PompeiiATCurrentFreeGame', $slotSettings->GetGameData('PompeiiATCurrentFreeGame') + 1);
                                $bonusMpl = $slotSettings->GetGameData('PompeiiATMpl');
                            }
                            $Balance = $slotSettings->GetBalance();
                            ...
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                                ...
                                $scattersWin = $slotSettings->Paytable[$scatter][$scattersCount] * $postData['slotBet'];
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('PompeiiATBonusWin', $slotSettings->GetGameData('PompeiiATBonusWin') + $totalWin);
                                $slotSettings->SetGameData('PompeiiATTotalWin', $slotSettings->GetGameData('PompeiiATTotalWin') + $totalWin);
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('PompeiiATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('PompeiiATMpl') . ',"xWin":[' . implode(',', $wildsMplArr) . '],"add":0,"total":' . $slotSettings->GetGameData('PompeiiATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('PompeiiATBonusWin') * 100) . '';
                            }
                            else
                            {
                                $slotSettings->SetGameData('PompeiiATTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('PompeiiATFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('PompeiiATFreeGames', $slotSettings->GetGameData('PompeiiATFreeGames') + $slotSettings->slotFreeCount[$scattersCount]);
                                    $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('PompeiiATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('PompeiiATMpl') . ',"add":' . $slotSettings->slotFreeCount[$scattersCount] . ',"total":' . $slotSettings->GetGameData('PompeiiATFreeGames') . ',"xWin":[' . implode(',', $wildsMplArr) . '],"totalWin":' . ($slotSettings->GetGameData('PompeiiATBonusWin') * 100) . '';
                                }
                                else
                                {
                                    $slotSettings->SetGameData('PompeiiATFreeBalance', $Balance);
                                    $slotSettings->SetGameData('PompeiiATBonusWin', $totalWin);
                                    $slotSettings->SetGameData('PompeiiATFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                    $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('PompeiiATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('PompeiiATMpl') . ',"add":' . $slotSettings->slotFreeCount[$scattersCount] . ',"total":' . $slotSettings->GetGameData('PompeiiATFreeGames') . ',"xWin":[' . implode(',', $wildsMplArr) . '],"totalWin":' . ($slotSettings->GetGameData('PompeiiATBonusWin') * 100) . '';
                                }
                            }
                            ...
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('PompeiiATFreeGames') . ',"Mpl":' . $slotSettings->GetGameData('PompeiiATMpl') . ',"currentFreeGames":' . $slotSettings->GetGameData('PompeiiATCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":{},"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            $reelStrArr = [];
                            for( $i = 1; $i <= 5; $i++ ) 
                            {
