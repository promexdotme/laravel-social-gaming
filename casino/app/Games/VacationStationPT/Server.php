                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('VacationStationPTBonusWin', 0);
                                $slotSettings->SetGameData('VacationStationPTFreeGames', 0);
                                $slotSettings->SetGameData('VacationStationPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('VacationStationPTTotalWin', 0);
                                $slotSettings->SetGameData('VacationStationPTFreeBalance', 0);
                                $slotSettings->SetGameData('VacationStationPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('VacationStationPTCurrentFreeGame', $slotSettings->GetGameData('VacationStationPTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('VacationStationPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('VacationStationPTBonusWin', $slotSettings->GetGameData('VacationStationPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('VacationStationPTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('VacationStationPTTotalWin', $totalWin);
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('VacationStationPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('VacationStationPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('VacationStationPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('VacationStationPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
