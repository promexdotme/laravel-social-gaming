                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('FunkyMonkeyPTBonusWin', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTFreeGames', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTTotalWin', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTFreeBalance', 0);
                                $slotSettings->SetGameData('FunkyMonkeyPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('FunkyMonkeyPTCurrentFreeGame', $slotSettings->GetGameData('FunkyMonkeyPTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('FunkyMonkeyPTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( in_array($s[0], [
                                                7, 
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('FunkyMonkeyPTBonusWin', $slotSettings->GetGameData('FunkyMonkeyPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('FunkyMonkeyPTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('FunkyMonkeyPTTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('FunkyMonkeyPTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('FunkyMonkeyPTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyPTFreeGames', $slotSettings->GetGameData('FunkyMonkeyPTFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('FunkyMonkeyPTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyPTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('FunkyMonkeyPTFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('FunkyMonkeyPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('FunkyMonkeyPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('FunkyMonkeyPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('FunkyMonkeyPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
