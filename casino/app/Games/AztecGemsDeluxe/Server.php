            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('AztecGemsDeluxeDealerCard');
                        $totalWin = $slotSettings->GetGameData('AztecGemsDeluxeTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('AztecGemsDeluxeTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('AztecGemsDeluxeDealerCard', $tmpDc);
                    ...
                    else if( $postData['slotEvent'] == 'doSpin' ) 
                    {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $bonusMpl = 1;
                            $slotSettings->SetGameData('AztecGemsDeluxeBonusWin', 0);
                            $slotSettings->SetGameData('AztecGemsDeluxeFreeGames', 0);
                            $slotSettings->SetGameData('AztecGemsDeluxeCurrentFreeGame', 0);
                            $slotSettings->SetGameData('AztecGemsDeluxeTotalWin', 0);
                            $slotSettings->SetGameData('AztecGemsDeluxeFreeBalance', 0);
                        }
                        else
                        {
                            $slotSettings->SetGameData('AztecGemsDeluxeCurrentFreeGame', $slotSettings->GetGameData('AztecGemsDeluxeCurrentFreeGame') + 1);
                        ...
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('AztecGemsDeluxeBonusWin', $slotSettings->GetGameData('AztecGemsDeluxeBonusWin') + $totalWin);
                            $slotSettings->SetGameData('AztecGemsDeluxeTotalWin', $slotSettings->GetGameData('AztecGemsDeluxeTotalWin') + $totalWin);
                            $totalWin = $slotSettings->GetGameData('AztecGemsDeluxeBonusWin');
                            $Balance = $slotSettings->GetGameData('AztecGemsDeluxeFreeBalance');
                        }
                        else
                        {
                            $slotSettings->SetGameData('AztecGemsDeluxeTotalWin', $totalWin);
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('AztecGemsDeluxeFreeGames') > 0 ) 
                            {
                                $slotSettings->SetGameData('AztecGemsDeluxeFreeBalance', $Balance);
                                $slotSettings->SetGameData('AztecGemsDeluxeBonusWin', $totalWin);
                                $slotSettings->SetGameData('AztecGemsDeluxeFreeGames', $slotSettings->GetGameData('AztecGemsDeluxeFreeGames') + $slotSettings->slotFreeCount);
                            }
                            else
                            {
                                $slotSettings->SetGameData('AztecGemsDeluxeFreeBalance', $Balance);
                                $slotSettings->SetGameData('AztecGemsDeluxeBonusWin', $totalWin);
                                $slotSettings->SetGameData('AztecGemsDeluxeFreeGames', $slotSettings->slotFreeCount);
                            }
                        }
                        $jsSpin = '' . json_encode($reels) . '';
                        $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                        $winString = '&' . implode('&', $lineWins);
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('AztecGemsDeluxeFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AztecGemsDeluxeCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        if( $bonusMpl == 1 ) 
                        {
                            $gmpl = 0;
                        }
