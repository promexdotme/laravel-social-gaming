            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'create_session':
                                $slotSettings->SetGameData('Treasure40CTCards', [
                                    19, 
                                    44, 
                                    0, 
                                    37, 
                            ...
                            case 'init_game':
                                ...
                                $slotSettings->SetGameData('Treasure40CTBonusWin', 0);
                                $slotSettings->SetGameData('Treasure40CTFreeGames', 0);
                                $slotSettings->SetGameData('Treasure40CTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('Treasure40CTCurrentFreeGame0', 0);
                                $slotSettings->SetGameData('Treasure40CTTotalWin', 0);
                                $slotSettings->SetGameData('Treasure40CTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    if( $slotSettings->GetGameData('Treasure40CTCurrentFreeGame') < $slotSettings->GetGameData('Treasure40CTFreeGames') && $slotSettings->GetGameData('Treasure40CTFreeGames') > 0 ) 
                                    {
                            ...
                            case 'poll':
                                ...
                                {
                                    $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance() - $slotSettings->GetGameData('Treasure40CTTotalWin')) * 100;
                                }
                            ...
                            case 'game_finished':
                                $slotSettings->SetGameData('Treasure40CTBonusWin', 0);
                                $slotSettings->SetGameData('Treasure40CTTotalWin', 0);
                                $responseData['game_finished'][0]['freeround_play'] = $slotSettings->GetGameData('Treasure40CTCurrentFreeGame0');
                            ...
                            case 'subgame_finished':
                                if( $slotSettings->GetGameData('Treasure40CTCurrentFreeGame0') < $slotSettings->GetGameData('Treasure40CTFreeGames') && $slotSettings->GetGameData('Treasure40CTFreeGames') > 0 ) 
                                {
                                    ...
                                    $response = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"result":' . json_encode($result) . ',"slotDenom":' . $slotSettings->GetGameData($slotSettings->slotId . 'Denom') . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('Treasure40CTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('Treasure40CTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData('Treasure40CTFreeBalance') . ',"afterBalance":' . $slotSettings->GetGameData('Treasure40CTFreeBalance') . ',"bonusWin":' . $slotSettings->GetGameData('Treasure40CTBonusWin') . ',"totalWin":' . $reportWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Reels')) . '}}';
                                    if( $slotSettings->GetGameData('Treasure40CTFreeGames') <= $slotSettings->GetGameData('Treasure40CTCurrentFreeGame0') ) 
                                    {
                                        if( $reportWin > 0 ) 
                                        {
                                    ...
                                    $balanceInCents = $slotSettings->GetGameData('Treasure40CTFreeBalance');
                                    $responseData['subgame_finished'][0]['account_status']['credit'] = round($balanceInCents);
                                    $responseData['subgame_finished'][0]['freeround_play'] = $slotSettings->GetGameData('Treasure40CTCurrentFreeGame0');
                                    $responseData['subgame_finished'][1]['freeround_play'] = $slotSettings->GetGameData('Treasure40CTCurrentFreeGame0');
                            ...
                            case 'play':
                                ...
                                for( $spinLoop = 0; $spinLoop < $spinLoopLimit; $spinLoop++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] != 'freespin' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('Treasure40CTCurrentFreeGame0', 0);
                                        $slotSettings->SetGameData('Treasure40CTTotalWin', 0);
                                        $slotSettings->SetGameData('Treasure40CTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    ...
                                    {
                                        $slotSettings->SetGameData('Treasure40CTCurrentFreeGame', $slotSettings->GetGameData('Treasure40CTCurrentFreeGame') + 1);
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $slotSettings->SetGameData('Treasure40CTBonusWin', $slotSettings->GetGameData('Treasure40CTBonusWin') + $totalWin);
                                        $slotSettings->SetGameData('Treasure40CTTotalWin', $slotSettings->GetGameData('Treasure40CTTotalWin') + $totalWin);
                                        $balanceInCents = $slotSettings->GetGameData('Treasure40CTFreeBalance');
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('Treasure40CTTotalWin', $totalWin);
                                    }
                                    $fs = 0;
                                    if( $scattersCount >= 3 ) 
                                    {
                                    ...
                                    $winstring = '';
                                    $slotSettings->SetGameData('Treasure40CTGambleStep', 5);
                                    $hist = $slotSettings->GetGameData('Treasure40CTCards');
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $state = 'freespin';
                                        ...
                                        if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('Treasure40CTBonusWin') > 0 ) 
                                        {
                                            $responseData['play'][0]['result'][$spinLoop]['is_free'] = true;
                                    ...
                                    $responseData['play'][0]['result'][$spinLoop]['total_win'] = $slotSettings->GetGameData('Treasure40CTTotalWin') / $denom;
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"result":' . json_encode($responseData['play'][0]['result']) . ',"slotDenom":' . $denom . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('Treasure40CTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('Treasure40CTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeBalance') . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('Treasure40CTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'gamble':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('Treasure40CTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                ...
                                $hist = $slotSettings->GetGameData('Treasure40CTCards');
                                $slotSettings->SetGameData('Treasure40CTGambleStep', $slotSettings->GetGameData('Treasure40CTGambleStep') - 1);
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isGambleWin = 0;
                                }
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('Treasure40CTGambleStep', 0);
                                    if( $postData['continue_params']['double_key'] != 'red' && $postData['continue_params']['double_key'] != 'black' ) 
                                    {
                                        if( $postData['continue_params']['double_key'] == 'diamond' ) 
                                        {
                                ...
                                $slotSettings->SetGameData('Treasure40CTCards', $hist);
