                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('VacationStationDeluxePTBonusWin', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTFreeGames', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTTotalWin', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTFreeBalance', 0);
                                $slotSettings->SetGameData('VacationStationDeluxePTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('VacationStationDeluxePTCurrentFreeGame', $slotSettings->GetGameData('VacationStationDeluxePTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('VacationStationDeluxePTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('VacationStationDeluxePTBonusWin', $slotSettings->GetGameData('VacationStationDeluxePTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('VacationStationDeluxePTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('VacationStationDeluxePTTotalWin', $totalWin);
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('VacationStationDeluxePTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('VacationStationDeluxePTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('VacationStationDeluxePTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('VacationStationDeluxePTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
