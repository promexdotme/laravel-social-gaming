                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('TempleOfSecretsGTMDealerCard');
                            $totalWin = $slotSettings->GetGameData('TempleOfSecretsGTMTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('TempleOfSecretsGTMTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('TempleOfSecretsGTMDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $jackState = $slotSettings->UpdateJackpots($postData['slotBet'] * $postData['slotLines'], $slotSettings->GetGameData('TempleOfSecretsGTMFreeMpl') - 1, false);
                                $slotSettings->SetBalance(-1 * ($postData['slotBet'] * $postData['slotLines']), $postData['slotEvent']);
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('TempleOfSecretsGTMBonusWin', 0);
                                $slotSettings->SetGameData('TempleOfSecretsGTMFreeGames', 0);
                                $slotSettings->SetGameData('TempleOfSecretsGTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('TempleOfSecretsGTMTotalWin', 0);
                                $slotSettings->SetGameData('TempleOfSecretsGTMFreeBalance', 0);
                                $slotSettings->SetGameData('TempleOfSecretsGTMFreeMpl', 1);
                            }
                            else
                            {
                                $jackState = $slotSettings->UpdateJackpots($postData['slotBet'] * $postData['slotLines'], 0, true);
                                $slotSettings->SetGameData('TempleOfSecretsGTMCurrentFreeGame', $slotSettings->GetGameData('TempleOfSecretsGTMCurrentFreeGame') + 1);
                                $bonusMpl = $slotSettings->GetGameData('TempleOfSecretsGTMFreeMpl');
                            }
                            $Balance = $slotSettings->GetBalance();
                            for( $i = 0; $i <= 2000; $i++ ) 
                            {
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('TempleOfSecretsGTMBonusWin', $slotSettings->GetGameData('TempleOfSecretsGTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('TempleOfSecretsGTMTotalWin', $totalWin);
                            }
                            else
                            {
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('TempleOfSecretsGTMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('TempleOfSecretsGTMFreeBalance', $Balance);
                                    $slotSettings->SetGameData('TempleOfSecretsGTMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('TempleOfSecretsGTMFreeGames', $slotSettings->GetGameData('TempleOfSecretsGTMFreeGames') + $slotSettings->slotFreeCount);
                                    $slotSettings->SetGameData('TempleOfSecretsGTMFreeMpl', $slotSettings->GetGameData('TempleOfSecretsGTMFreeMpl') + 1);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('TempleOfSecretsGTMBonusWin', $slotSettings->GetGameData('TempleOfSecretsGTMBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('TempleOfSecretsGTMFreeGames', $slotSettings->slotFreeCount);
                                    $slotSettings->SetGameData('TempleOfSecretsGTMFreeMpl', 2);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"bonusWin":' . $slotSettings->GetGameData('TempleOfSecretsGTMBonusWin') . ',"slotFreeMpl":' . $slotSettings->GetGameData('TempleOfSecretsGTMFreeMpl') . ',"slotJackpot":[' . implode(',', $slotSettings->slotJackpot) . '],"totalFreeGames":' . $slotSettings->GetGameData('TempleOfSecretsGTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('TempleOfSecretsGTMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
