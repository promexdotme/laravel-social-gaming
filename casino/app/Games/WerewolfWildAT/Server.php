            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            $response = '{"status":"success","microtime":0.0026731491088867,"dateTime":"2020-02-13 13:23:32","error":"","content":{"cmd":"sessionInfo","serverMathematics":"\/game\/WerewolfWildAT\/server","serverResources":"","sessionId":"54594109_4133a64673d3883d42cd003ee905ba3e","exitUlt":"\/","exitUrl":"\/","id":"341","name":"GameName","currency":"ALL","language":"en","type":"aristocrat","systemName":"werewolf_wild","version":"2","mobile":"1"}}';
                        }
                        ...
                        else if( $postData['slotEvent'] == 'gamePick' ) 
                        {
                            ...
                            if( $_POST['pick'] == 0 ) 
                            {
                                $slotSettings->SetGameData('WerewolfWildATMpl', 10);
                                $slotSettings->SetGameData('WerewolfWildATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('WerewolfWildATFreeGames', 5);
                            }
                            else if( $_POST['pick'] == 1 ) 
                            {
                                $slotSettings->SetGameData('WerewolfWildATMpl', 5);
                                $slotSettings->SetGameData('WerewolfWildATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('WerewolfWildATFreeGames', 10);
                            }
                            else if( $_POST['pick'] == 2 ) 
                            {
                                $slotSettings->SetGameData('WerewolfWildATMpl', 3);
                                $slotSettings->SetGameData('WerewolfWildATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('WerewolfWildATFreeGames', 15);
                            }
                            else if( $_POST['pick'] == 3 ) 
                            {
                                $slotSettings->SetGameData('WerewolfWildATMpl', 2);
                                $slotSettings->SetGameData('WerewolfWildATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('WerewolfWildATFreeGames', 20);
                            }
                            $response = '{"status":"success","microtime":0.0093810558319092,"dateTime":"2020-02-15 14:26:21","error":"","content":{"session":"54990161_9709e13bca7763b46396cda1868d2e7e","cmd":"gamePick","balance":' . $balanceInCents . ',"win":0,"symbols":{"1":{"1":"symbol_6","2":"symbol_9","3":"symbol_13"},"2":{"1":"symbol_10","2":"symbol_5","3":"symbol_8"},"3":{"1":"symbol_10","2":"symbol_13","3":"symbol_11"},"4":{"1":"symbol_13","2":"symbol_10","3":"symbol_6"},"5":{"1":"symbol_3","2":"symbol_8","3":"symbol_1"}},"winLines":[],"freeSpin":{"id":"54990161_20200215132548","current":0,"add":' . $slotSettings->GetGameData('WerewolfWildATFreeGames') . ',"total":' . $slotSettings->GetGameData('WerewolfWildATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('WerewolfWildATBonusWin') * 100) . ',"multiplayer":' . $slotSettings->GetGameData('WerewolfWildATMpl') . ',"pick":' . $_POST['pick'] . '},"actionId":"54990161_10_60","hash":"1ace145628d930d28bdf38789a28a9d7"}}';
                        }
                        else if( $postData['slotEvent'] == 'slotGamble' ) 
                        {
                            ...
                            $dealerCard = '';
                            $totalWin = $slotSettings->GetGameData('WerewolfWildATTotalWin');
                            $gambleWin = 0;
                            $statBet = $totalWin;
                            ...
                            $slotSettings->SetGameData('WerewolfWildATTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('WerewolfWildATBonusWin', 0);
                                $slotSettings->SetGameData('WerewolfWildATFreeGames', 0);
                                $slotSettings->SetGameData('WerewolfWildATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('WerewolfWildATTotalWin', 0);
                                $slotSettings->SetGameData('WerewolfWildATFreeBalance', 0);
                                $slotSettings->SetGameData('WerewolfWildATMpl', 3);
                            }
                            else
                            {
                                $slotSettings->SetGameData('WerewolfWildATCurrentFreeGame', $slotSettings->GetGameData('WerewolfWildATCurrentFreeGame') + 1);
                                $bonusMpl = $slotSettings->GetGameData('WerewolfWildATMpl');
                            }
                            $Balance = $slotSettings->GetBalance();
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('WerewolfWildATBonusWin', $slotSettings->GetGameData('WerewolfWildATBonusWin') + $totalWin);
                                $slotSettings->SetGameData('WerewolfWildATTotalWin', $slotSettings->GetGameData('WerewolfWildATTotalWin') + $totalWin);
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('WerewolfWildATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('WerewolfWildATMpl') . ',"add":0,"total":' . $slotSettings->GetGameData('WerewolfWildATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('WerewolfWildATBonusWin') * 100) . '';
                            }
                            else
                            {
                                $slotSettings->SetGameData('WerewolfWildATTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('WerewolfWildATFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('WerewolfWildATFreeGames', $slotSettings->GetGameData('WerewolfWildATFreeGames') + $slotSettings->slotFreeCount);
                                    $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('WerewolfWildATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('WerewolfWildATMpl') . ',"add":' . $slotSettings->slotFreeCount . ',"total":' . $slotSettings->GetGameData('WerewolfWildATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('WerewolfWildATBonusWin') * 100) . '';
                                }
                                else
                                {
                                    $slotSettings->SetGameData('WerewolfWildATFreeBalance', $Balance);
                                    $slotSettings->SetGameData('WerewolfWildATBonusWin', $totalWin);
                                    $slotSettings->SetGameData('WerewolfWildATFreeGames', $slotSettings->slotFreeCount);
                                    $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('WerewolfWildATCurrentFreeGame') . ',"multiplayer":' . $slotSettings->GetGameData('WerewolfWildATMpl') . ',"add":-1,"total":-1,"totalWin":' . ($slotSettings->GetGameData('WerewolfWildATBonusWin') * 100) . '';
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('WerewolfWildATFreeGames') . ',"Mpl":' . $slotSettings->GetGameData('WerewolfWildATMpl') . ',"currentFreeGames":' . $slotSettings->GetGameData('WerewolfWildATCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":{},"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            $reelStrArr = [];
                            for( $i = 1; $i <= 5; $i++ ) 
                            {
