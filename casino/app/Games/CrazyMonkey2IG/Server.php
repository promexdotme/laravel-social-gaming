            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['action'] == 'risk' && $slotSettings->GetGameData('CrazyMonkey2IGWin') <= 0 ) 
                        {
                        ...
                        switch( $postData['action'] ) 
                        {
                            case 'super':
                                ...
                                $curWin = $slotSettings->GetGameData('CrazyMonkey2IGSuperWin');
                                $curStep = $slotSettings->GetGameData('CrazyMonkey2IGSuperStep');
                                $curCoef = $slotSettings->GetGameData('CrazyMonkey2IGSuperCoef');
                                if( $curStep > 3 ) 
                                {
                                    $response = '{"responseEvent":"error","responseType":"' . $postData['action'] . '","serverResponse":"invalid super bonus state"}';
                                    exit( $response );
                                }
                                $slotSettings->SetBalance(-1 * $curWin);
                                $slotSettings->SetBank('bonus', $curWin);
                                $bank = $slotSettings->GetBank('bonus');
                                for( $i = 0; $i <= 1000; $i++ ) 
                                {
                                    $bTmp = $slotSettings->SuperWin($bet * $lines);
                                    $superWinsCur = [
                                        0, 
                                        0, 
                                        0, 
                                    ...
                                    {
                                        $superCoefNew = $curCoef + $superWinsCur[0];
                                    }
                                    $totalWin = $superCoefNew * ($bet * $lines);
                                    $infoBonus = '"SuperWin":{"Coef":' . $superCoefNew . ',"Win":' . $totalWin . '},';
                                    if( $totalWin <= $bank ) 
                                    {
                                        break;
                                    }
                                ...
                                $slotSettings->SetGameData('CrazyMonkey2IGSuperStep', $curStep);
                                $slotSettings->SetGameData('CrazyMonkey2IGSuperCoef', $superCoefNew);
                                $balance = $slotSettings->GetBalance();
                                $response = '{"Amount":"' . sprintf('%01.2f', $balance * $slotSettings->CurrentDenom) . '",' . $infoBonus . '"Credit":' . floor($balance) . ',"Denomination":"' . sprintf('%01.2f', $slotSettings->CurrentDenom) . '","RawCredit":' . floor($balance) . ',"Win":' . $totalWin . ',"args":{"Super":1},"cmd":"super","time":"20190328T124737"}';
                                if( $totalWin == 0 ) 
                                {
                                    $totalWin = -1 * $curWin;
                                }
                                $slotSettings->SaveLogReport($response, $curWin, 1, $totalWin, 'BG2');
                                break;
                            case 'init':
                                $balance = $slotSettings->GetBalance();
                            ...
                            case 'start':
                                ...
                                $slotSettings->SetGameData('CrazyMonkey2IGReels', $reelStr);
                                ...
                                $slotSettings->SetGameData('CrazyMonkey2IGDealerCard', $card);
                                if( $isBonus && $isSuper ) 
                                {
                                    $slotSettings->SetGameData('CrazyMonkey2IGSuperStep', 1);
                                    $slotSettings->SetGameData('CrazyMonkey2IGSuperCoef', 50);
                                    $slotSettings->SetGameData('CrazyMonkey2IGSuperWin', $superWin);
                                }
                                $rState = 'bet';
                                if( $isBonus ) 
                                {
