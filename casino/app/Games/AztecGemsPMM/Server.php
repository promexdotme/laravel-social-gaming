            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('AztecGemsPMMDealerCard');
                        $totalWin = $slotSettings->GetGameData('AztecGemsPMMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('AztecGemsPMMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('AztecGemsPMMDealerCard', $tmpDc);
                    ...
                    else if( $postData['slotEvent'] == 'doSpin' ) 
                    {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $bonusMpl = 1;
                            $slotSettings->SetGameData('AztecGemsPMMBonusWin', 0);
                            $slotSettings->SetGameData('AztecGemsPMMFreeGames', 0);
                            $slotSettings->SetGameData('AztecGemsPMMCurrentFreeGame', 0);
                            $slotSettings->SetGameData('AztecGemsPMMTotalWin', 0);
                            $slotSettings->SetGameData('AztecGemsPMMFreeBalance', 0);
                        }
                        else
                        {
                            $slotSettings->SetGameData('AztecGemsPMMCurrentFreeGame', $slotSettings->GetGameData('AztecGemsPMMCurrentFreeGame') + 1);
                        ...
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('AztecGemsPMMBonusWin', $slotSettings->GetGameData('AztecGemsPMMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('AztecGemsPMMTotalWin', $slotSettings->GetGameData('AztecGemsPMMTotalWin') + $totalWin);
                            $totalWin = $slotSettings->GetGameData('AztecGemsPMMBonusWin');
                            $Balance = $slotSettings->GetGameData('AztecGemsPMMFreeBalance');
                        }
                        else
                        {
                            $slotSettings->SetGameData('AztecGemsPMMTotalWin', $totalWin);
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('AztecGemsPMMFreeGames') > 0 ) 
                            {
                                $slotSettings->SetGameData('AztecGemsPMMFreeBalance', $Balance);
                                $slotSettings->SetGameData('AztecGemsPMMBonusWin', $totalWin);
                                $slotSettings->SetGameData('AztecGemsPMMFreeGames', $slotSettings->GetGameData('AztecGemsPMMFreeGames') + $slotSettings->slotFreeCount);
                            }
                            else
                            {
                                $slotSettings->SetGameData('AztecGemsPMMFreeBalance', $Balance);
                                $slotSettings->SetGameData('AztecGemsPMMBonusWin', $totalWin);
                                $slotSettings->SetGameData('AztecGemsPMMFreeGames', $slotSettings->slotFreeCount);
                            }
                        }
                        $jsSpin = '' . json_encode($reels) . '';
                        $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                        $winString = '&' . implode('&', $lineWins);
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('AztecGemsPMMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AztecGemsPMMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        if( $bonusMpl == 1 ) 
                        {
                            $gmpl = 0;
                        }
