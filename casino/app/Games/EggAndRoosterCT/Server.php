            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            case 'create_session':
                                $slotSettings->SetGameData('EggAndRoosterCTCards', [
                                    19, 
                                    44, 
                                    0, 
                                    37, 
                            ...
                            case 'init_game':
                                ...
                                $slotSettings->SetGameData('EggAndRoosterCTBonusWin', 0);
                                $slotSettings->SetGameData('EggAndRoosterCTFreeGames', 0);
                                $slotSettings->SetGameData('EggAndRoosterCTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('EggAndRoosterCTCurrentFreeGame0', 0);
                                $slotSettings->SetGameData('EggAndRoosterCTTotalWin', 0);
                                $slotSettings->SetGameData('EggAndRoosterCTFreeBalance', 0);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    if( $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame') < $slotSettings->GetGameData('EggAndRoosterCTFreeGames') && $slotSettings->GetGameData('EggAndRoosterCTFreeGames') > 0 ) 
                                    {
                            ...
                            case 'poll':
                                ...
                                {
                                    $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance() - $slotSettings->GetGameData('EggAndRoosterCTTotalWin')) * 100;
                                }
                            ...
                            case 'game_finished':
                                $slotSettings->SetGameData('EggAndRoosterCTBonusWin', 0);
                                $slotSettings->SetGameData('EggAndRoosterCTTotalWin', 0);
                                $responseData['game_finished'][0]['freeround_play'] = $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame0');
                            ...
                            case 'subgame_finished':
                                if( $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame0') < $slotSettings->GetGameData('EggAndRoosterCTFreeGames') && $slotSettings->GetGameData('EggAndRoosterCTFreeGames') > 0 ) 
                                {
                                    ...
                                    $response = '{"responseEvent":"spin","responseType":"freespin","serverResponse":{"result":' . json_encode($result) . ',"slotDenom":' . $slotSettings->GetGameData($slotSettings->slotId . 'Denom') . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('EggAndRoosterCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData('EggAndRoosterCTFreeBalance') . ',"afterBalance":' . $slotSettings->GetGameData('EggAndRoosterCTFreeBalance') . ',"bonusWin":' . $slotSettings->GetGameData('EggAndRoosterCTBonusWin') . ',"totalWin":' . $reportWin . ',"winLines":[],"Jackpots":[],"reelsSymbols":' . json_encode($slotSettings->GetGameData($slotSettings->slotId . 'Reels')) . '}}';
                                    if( $slotSettings->GetGameData('EggAndRoosterCTFreeGames') <= $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame0') ) 
                                    {
                                        if( $reportWin > 0 ) 
                                        {
                                    ...
                                    $balanceInCents = $slotSettings->GetGameData('EggAndRoosterCTFreeBalance');
                                    $responseData['subgame_finished'][0]['account_status']['credit'] = round($balanceInCents);
                                    $responseData['subgame_finished'][0]['freeround_play'] = $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame0');
                                    $responseData['subgame_finished'][1]['freeround_play'] = $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame0');
                            ...
                            case 'play':
                                ...
                                for( $spinLoop = 0; $spinLoop < $spinLoopLimit; $spinLoop++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] != 'freespin' ) 
                                    {
                                        ...
                                        $slotSettings->SetGameData('EggAndRoosterCTCurrentFreeGame0', 0);
                                        $slotSettings->SetGameData('EggAndRoosterCTTotalWin', 0);
                                        $slotSettings->SetGameData('EggAndRoosterCTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    ...
                                    {
                                        $slotSettings->SetGameData('EggAndRoosterCTCurrentFreeGame', $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame') + 1);
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $slotSettings->SetGameData('EggAndRoosterCTBonusWin', $slotSettings->GetGameData('EggAndRoosterCTBonusWin') + $totalWin);
                                        $slotSettings->SetGameData('EggAndRoosterCTTotalWin', $slotSettings->GetGameData('EggAndRoosterCTTotalWin') + $totalWin);
                                        $balanceInCents = $slotSettings->GetGameData('EggAndRoosterCTFreeBalance');
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('EggAndRoosterCTTotalWin', $totalWin);
                                    }
                                    $fs = 0;
                                    if( $scattersCount >= 3 ) 
                                    {
                                    ...
                                    $winstring = '';
                                    $slotSettings->SetGameData('EggAndRoosterCTGambleStep', 5);
                                    $hist = $slotSettings->GetGameData('EggAndRoosterCTCards');
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        $state = 'freespin';
                                        ...
                                        if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('EggAndRoosterCTBonusWin') > 0 ) 
                                        {
                                            $responseData['play'][0]['result'][$spinLoop]['is_free'] = true;
                                    ...
                                    $responseData['play'][0]['result'][$spinLoop]['total_win'] = $slotSettings->GetGameData('EggAndRoosterCTTotalWin') / $denom;
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"result":' . json_encode($responseData['play'][0]['result']) . ',"slotDenom":' . $denom . ',"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('EggAndRoosterCTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('EggAndRoosterCTCurrentFreeGame0') . ',"Balance":' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeBalance') . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('EggAndRoosterCTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            ...
                            case 'gamble':
                                ...
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('EggAndRoosterCTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                ...
                                $hist = $slotSettings->GetGameData('EggAndRoosterCTCards');
                                $slotSettings->SetGameData('EggAndRoosterCTGambleStep', $slotSettings->GetGameData('EggAndRoosterCTGambleStep') - 1);
                                if( $slotSettings->MaxWin < ($totalWin * $slotSettings->CurrentDenom) ) 
                                {
                                    $isGambleWin = 0;
                                }
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('EggAndRoosterCTGambleStep', 0);
                                    if( $postData['continue_params']['double_key'] != 'red' && $postData['continue_params']['double_key'] != 'black' ) 
                                    {
                                        if( $postData['continue_params']['double_key'] == 'diamond' ) 
                                        {
                                ...
                                $slotSettings->SetGameData('EggAndRoosterCTCards', $hist);
