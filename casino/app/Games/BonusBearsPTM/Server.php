            ...
            function get_($request, $game)
            {
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        {
                            $umid = '0';
                            $startStick = $slotSettings->GetGameData('BonusBearsPTMstartStick');
                            $treeStep = $slotSettings->GetGameData('BonusBearsPTMtreeStep');
                            $lastWins = $slotSettings->GetGameData('BonusBearsPTMlastWins');
                            $winArr = [
                                0, 
                                0, 
                                0, 
                            ...
                            {
                                shuffle($winArr);
                                $win = $winArr[0] * $slotSettings->GetGameData('BonusBearsPTMAllBet');
                                if( $win <= $bank ) 
                                {
                                    break;
                                }
                            ...
                            $result_tmp = [];
                            $result_tmp[] = '3:::{"data":{"winresult":' . $winArr[0] . ',"win":"' . $slotSettings->GetGameData('BonusBearsPTMChosenTree') . '|' . $slotSettings->GetGameData('BonusBearsPTMAllBet') . '","windowId":"SuJLru"},"ID":46451,"umid":19}';
                            if( $slotSettings->GetGameData('BonusBearsPTMChosenTree') == 0 ) 
                            {
                                $slotSettings->SetGameData('BonusBearsPTMChosenTree', $postData['chosenTree']);
                            }
                            $slotSettings->SetGameData('BonusBearsPTMstartStick', $startStick);
                            $slotSettings->SetGameData('BonusBearsPTMtreeStep', $treeStep);
                            $slotSettings->SetGameData('BonusBearsPTMlastWins', $lastWins);
                            $bStr = '"chosenTree":"' . $slotSettings->GetGameData('BonusBearsPTMChosenTree') . '","startStick":"' . $slotSettings->GetGameData('BonusBearsPTMstartStick') . '","treeStep":[' . implode(',', $slotSettings->GetGameData('BonusBearsPTMtreeStep')) . '],"lastWins":[' . implode(',', $slotSettings->GetGameData('BonusBearsPTMlastWins')) . '],';
                            $response = '{"responseEvent":"spin","responseType":"tree_bonus","serverResponse":{' . $bStr . '"allbet":' . $slotSettings->GetGameData('BonusBearsPTMAllBet') . '}}';
                            $slotSettings->SaveLogReport($response, $slotSettings->GetGameData('BonusBearsPTMAllBet'), 1, $win, 'tree_bonus');
                        }
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('BonusBearsPTMBonusWin', 0);
                                $slotSettings->SetGameData('BonusBearsPTMFreeGames', 0);
                                $slotSettings->SetGameData('BonusBearsPTMCurrentFreeGame', 0);
                                $slotSettings->SetGameData('BonusBearsPTMTotalWin', 0);
                                $slotSettings->SetGameData('BonusBearsPTMFreeBalance', 0);
                                $slotSettings->SetGameData('BonusBearsPTMFreeStartWin', 0);
                                $chosenTree = 0;
                                $startStick = 0;
                                $treeStep = [
                                    -1, 
                                ...
                                $slotSettings->SetGameData('BonusBearsPTMChosenTree', $chosenTree);
                                $slotSettings->SetGameData('BonusBearsPTMstartStick', $startStick);
                                $slotSettings->SetGameData('BonusBearsPTMtreeStep', $treeStep);
                                $slotSettings->SetGameData('BonusBearsPTMlastWins', $lastWins);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                ...
                                $slotSettings->SetGameData('BonusBearsPTMCurrentFreeGame', $slotSettings->GetGameData('BonusBearsPTMCurrentFreeGame') + 1);
                            ...
                            $slotSettings->SetGameData('BonusBearsPTMAllBet', $postData['bet']);
                            $winTypeTmp = $slotSettings->GetSpinSettings($postData['slotEvent'], $postData['bet'], $lines);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('BonusBearsPTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('BonusBearsPTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('BonusBearsPTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":4,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('BonusBearsPTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":5,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('BonusBearsPTMBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":[' . ($linesId[$k][3] - 1) . ',"' . $s[3] . '"],"winReel5":[' . ($linesId[$k][4] - 1) . ',"' . $s[4] . '"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('BonusBearsPTMBonusWin', $slotSettings->GetGameData('BonusBearsPTMBonusWin') + $totalWin);
                                $slotSettings->SetGameData('BonusBearsPTMTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('BonusBearsPTMTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('BonusBearsPTMFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('BonusBearsPTMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('BonusBearsPTMFreeGames', $slotSettings->GetGameData('BonusBearsPTMFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('BonusBearsPTMFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('BonusBearsPTMBonusWin', $totalWin);
                                    $slotSettings->SetGameData('BonusBearsPTMFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            if( $scattersCount2 >= 3 ) 
                            {
                            ...
                            $bStr = '"chosenTree":"' . $slotSettings->GetGameData('BonusBearsPTMChosenTree') . '","startStick":"' . $slotSettings->GetGameData('BonusBearsPTMstartStick') . '","treeStep":[' . implode(',', $slotSettings->GetGameData('BonusBearsPTMtreeStep')) . '],"lastWins":[' . implode(',', $slotSettings->GetGameData('BonusBearsPTMlastWins')) . '],';
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{' . $bStr . '"allbet":' . $slotSettings->GetGameData('BonusBearsPTMAllBet') . ',"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('BonusBearsPTMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BonusBearsPTMCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('BonusBearsPTMBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('BonusBearsPTMFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                        ...
                        switch( $umid ) 
                        {
                            ...
                            case '40036':
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('BonusBearsPTMChosenTree', $lastEvent->serverResponse->chosenTree);
                                    $slotSettings->SetGameData('BonusBearsPTMstartStick', $lastEvent->serverResponse->startStick);
                                    $slotSettings->SetGameData('BonusBearsPTMtreeStep', $lastEvent->serverResponse->treeStep);
                                    $slotSettings->SetGameData('BonusBearsPTMlastWins', $lastEvent->serverResponse->lastWins);
                                    ...
                                    $slotSettings->SetGameData('BonusBearsPTMresponseType', $lastEvent->responseType);
                                }
                            ...
                            case '40030':
                                ...
                                {
                                    if( $slotSettings->GetGameData('BonusBearsPTMresponseType') == 'tree_bonus' ) 
                                    {
                                        $chosenTree = $slotSettings->GetGameData('BonusBearsPTMChosenTree') + 3;
                                        if( $chosenTree < 4 ) 
                                        {
                                            $chosenTree = 0;
                                        }
