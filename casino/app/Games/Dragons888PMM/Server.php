            ...
            \DB::transaction(function() use ($request, $game)
            {
                try
                {
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                    {
                        ...
                        $dealerCard = $slotSettings->GetGameData('Dragons888PMMDealerCard');
                        $totalWin = $slotSettings->GetGameData('Dragons888PMMTotalWin');
                        $gambleWin = 0;
                        ...
                        $slotSettings->SetGameData('Dragons888PMMTotalWin', $totalWin);
                    ...
                    else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                    {
                        ...
                        $slotSettings->SetGameData('Dragons888PMMDealerCard', $tmpDc);
                    ...
                    else if( $postData['slotEvent'] == 'doSpin' ) 
                    {
                        ...
                        if( $postData['slotEvent'] != 'freespin' ) 
                        {
                            ...
                            $bonusMpl = 1;
                            $slotSettings->SetGameData('Dragons888PMMBonusWin', 0);
                            $slotSettings->SetGameData('Dragons888PMMFreeGames', 0);
                            $slotSettings->SetGameData('Dragons888PMMCurrentFreeGame', 0);
                            $slotSettings->SetGameData('Dragons888PMMTotalWin', 0);
                            $slotSettings->SetGameData('Dragons888PMMFreeBalance', 0);
                        }
                        else
                        {
                            $slotSettings->SetGameData('Dragons888PMMCurrentFreeGame', $slotSettings->GetGameData('Dragons888PMMCurrentFreeGame') + 1);
                        ...
                        if( $postData['slotEvent'] == 'freespin' ) 
                        {
                            $slotSettings->SetGameData('Dragons888PMMBonusWin', $slotSettings->GetGameData('Dragons888PMMBonusWin') + $totalWin);
                            $slotSettings->SetGameData('Dragons888PMMTotalWin', $slotSettings->GetGameData('Dragons888PMMTotalWin') + $totalWin);
                            $totalWin = $slotSettings->GetGameData('Dragons888PMMBonusWin');
                            $Balance = $slotSettings->GetGameData('Dragons888PMMFreeBalance');
                        }
                        else
                        {
                            $slotSettings->SetGameData('Dragons888PMMTotalWin', $totalWin);
                        ...
                        if( $scattersCount >= 3 ) 
                        {
                            if( $slotSettings->GetGameData('Dragons888PMMFreeGames') > 0 ) 
                            {
                                $slotSettings->SetGameData('Dragons888PMMFreeBalance', $Balance);
                                $slotSettings->SetGameData('Dragons888PMMBonusWin', $totalWin);
                                $slotSettings->SetGameData('Dragons888PMMFreeGames', $slotSettings->GetGameData('Dragons888PMMFreeGames') + $slotSettings->slotFreeCount);
                            }
                            else
                            {
                                $slotSettings->SetGameData('Dragons888PMMFreeBalance', $Balance);
                                $slotSettings->SetGameData('Dragons888PMMBonusWin', $totalWin);
                                $slotSettings->SetGameData('Dragons888PMMFreeGames', $slotSettings->slotFreeCount);
                            }
                        }
                        $jsSpin = '' . json_encode($reels) . '';
                        $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                        $winString = implode(',', $lineWins);
                        $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"lines":' . $postData['slotLines'] . ',"bet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('Dragons888PMMFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('Dragons888PMMCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
