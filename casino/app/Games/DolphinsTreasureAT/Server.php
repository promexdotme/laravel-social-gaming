                    ...
                    try
                    {
                        ...
                        if( $postData['slotEvent'] == 'gameInit' ) 
                        {
                            ...
                            if( $lastEvent != 'NULL' ) 
                            {
                                ...
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $lastEvent->serverResponse->currentFreeGames . ',"add":0,"total":' . $lastEvent->serverResponse->totalFreeGames . ',"totalWin":17';
                        ...
                        {
                            $response = '{"status":"success","microtime":0.0026731491088867,"dateTime":"2020-02-13 13:23:32","error":"","content":{"cmd":"sessionInfo","serverMathematics":"\/game\/DolphinsTreasureAT\/server","serverResources":"","sessionId":"54594109_4133a64673d3883d42cd003ee905ba3e","exitUlt":"\/","exitUrl":"\/","id":"341","name":"GameName","currency":"ALL","language":"en","type":"aristocrat","systemName":"dolphins_treasure","version":"2","mobile":"1"}}';
                        }
                        ...
                        else if( $postData['slotEvent'] == 'slotGamble' ) 
                        {
                            ...
                            $dealerCard = '';
                            $totalWin = $slotSettings->GetGameData('DolphinsTreasureATTotalWin');
                            $gambleWin = 0;
                            $statBet = $totalWin;
                            ...
                            $slotSettings->SetGameData('DolphinsTreasureATTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('DolphinsTreasureATBonusWin', 0);
                                $slotSettings->SetGameData('DolphinsTreasureATFreeGames', 0);
                                $slotSettings->SetGameData('DolphinsTreasureATCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DolphinsTreasureATTotalWin', 0);
                                $slotSettings->SetGameData('DolphinsTreasureATFreeBalance', 0);
                            }
                            else
                            {
                                $slotSettings->SetGameData('DolphinsTreasureATCurrentFreeGame', $slotSettings->GetGameData('DolphinsTreasureATCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('DolphinsTreasureATBonusWin', $slotSettings->GetGameData('DolphinsTreasureATBonusWin') + $totalWin);
                                $slotSettings->SetGameData('DolphinsTreasureATTotalWin', $slotSettings->GetGameData('DolphinsTreasureATTotalWin') + $totalWin);
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('DolphinsTreasureATCurrentFreeGame') . ',"add":0,"total":' . $slotSettings->GetGameData('DolphinsTreasureATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('DolphinsTreasureATBonusWin') * 100) . '';
                            }
                            else
                            {
                                $slotSettings->SetGameData('DolphinsTreasureATTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('DolphinsTreasureATFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('DolphinsTreasureATFreeGames', $slotSettings->GetGameData('DolphinsTreasureATFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('DolphinsTreasureATFreeBalance', $Balance);
                                    $slotSettings->SetGameData('DolphinsTreasureATBonusWin', $totalWin);
                                    $slotSettings->SetGameData('DolphinsTreasureATFreeGames', $slotSettings->slotFreeCount);
                                }
                                $freeStateStr = '"id":"54594109_20200214110301","current":' . $slotSettings->GetGameData('DolphinsTreasureATCurrentFreeGame') . ',"add":' . $slotSettings->slotFreeCount . ',"total":' . $slotSettings->GetGameData('DolphinsTreasureATFreeGames') . ',"totalWin":' . ($slotSettings->GetGameData('DolphinsTreasureATBonusWin') * 100) . '';
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response_log = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('DolphinsTreasureATFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('DolphinsTreasureATCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":{},"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                            $reelStrArr = [];
                            for( $i = 1; $i <= 5; $i++ ) 
                            {
