                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[2,5,25,100],"multiplier":2},"1":{"coef":[5,25,100],"multiplier":2},"2":{"coef":[5,25,100],"multiplier":2},"3":{"coef":[5,25,100],"multiplier":2},"4":{"coef":[10,50,125],"multiplier":2},"5":{"coef":[10,50,125],"multiplier":2},"6":{"coef":[15,75,250],"multiplier":2},"7":{"coef":[15,75,250],"multiplier":2},"8":{"coef":[20,100,400],"multiplier":2},"9":{"coef":[2,25,125,750],"multiplier":2},"10":{"coef":[2,25,125,750],"multiplier":2},"11":{"coef":[10,250,2500,10000],"multiplier":1},"12":{"coef":[2,5,20,500],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[2,4,10,3,2,7,0,6,1,11,9,0,5,12,1,2,10,4,5,8],[7,8,0,11,3,2,4,6,0,5,7,0,9,1,5,0,1,2,12,1],[0,3,9,12,1,4,6,2,0,8,3,0,6,1,11,10,2,7,4,5],[3,6,11,1,7,0,4,8,0,2,6,12,5,7,3,6,1,4,10,2],[2,10,3,0,1,7,0,12,6,2,9,4,11,3,0,6,5,1,8,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('AmazingAmazoniaEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('AmazingAmazoniaEGTBonusWin', 0);
                                $slotSettings->SetGameData('AmazingAmazoniaEGTFreeGames', 0);
                                $slotSettings->SetGameData('AmazingAmazoniaEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('AmazingAmazoniaEGTTotalWin', 0);
                                $slotSettings->SetGameData('AmazingAmazoniaEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('AmazingAmazoniaEGTCurrentFreeGame') < $slotSettings->GetGameData('AmazingAmazoniaEGTFreeGames') && $slotSettings->GetGameData('AmazingAmazoniaEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTTotalWin', 0);
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTCurrentFreeGame', $slotSettings->GetGameData('AmazingAmazoniaEGTCurrentFreeGame') + 1);
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTBonusWin', $slotSettings->GetGameData('AmazingAmazoniaEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTTotalWin', $slotSettings->GetGameData('AmazingAmazoniaEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('AmazingAmazoniaEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('AmazingAmazoniaEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('AmazingAmazoniaEGTFreeGames', $slotSettings->GetGameData('AmazingAmazoniaEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('AmazingAmazoniaEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('AmazingAmazoniaEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('AmazingAmazoniaEGTFreeGames', $slotSettings->slotFreeCount);
                                    }
                                    $fs = $slotSettings->GetGameData('AmazingAmazoniaEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('AmazingAmazoniaEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AmazingAmazoniaEGTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('AmazingAmazoniaEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('AmazingAmazoniaEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('AmazingAmazoniaEGTCards');
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel2'][0] . ',' . $reels['reel2'][1] . ',' . $reels['reel2'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel3'][0] . ',' . $reels['reel3'][1] . ',' . $reels['reel3'][2] . ',' . rand(0, 6));
                                $curReels .= (',' . rand(0, 6) . ',' . $reels['reel4'][0] . ',' . $reels['reel4'][1] . ',' . $reels['reel4'][2] . ',' . rand(0, 6));
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('AmazingAmazoniaEGTBonusWin');
                                    $state = 'freespin';
                                    if( $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') <= $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') && $slotSettings->GetGameData('AmazingAmazoniaEGTBonusWin') > 0 ) 
                                    {
                                        $state = 'gamble';
                                    }
                                }
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('AmazingAmazoniaEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('AmazingAmazoniaEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('AmazingAmazoniaEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('AmazingAmazoniaEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('AmazingAmazoniaEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('AmazingAmazoniaEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('AmazingAmazoniaEGTGambleStep', $slotSettings->GetGameData('AmazingAmazoniaEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('AmazingAmazoniaEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('AmazingAmazoniaEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('AmazingAmazoniaEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
