                ...
                \DB::transaction(function() use ($request, $game)
                {
                    try
                    {
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetUserCards' ) 
                        {
                            ...
                            $dealerCard = $slotSettings->GetGameData('AlwaysHotDXGTDealerCard');
                            $totalWin = $slotSettings->GetGameData('AlwaysHotDXGTTotalWin');
                            $gambleWin = 0;
                            ...
                            $slotSettings->SetGameData('AlwaysHotDXGTTotalWin', $totalWin);
                        ...
                        else if( $postData['slotEvent'] == 'gamble5GetDealerCard' ) 
                        {
                            ...
                            $slotSettings->SetGameData('AlwaysHotDXGTDealerCard', $tmpDc);
                        ...
                        else if( $postData['slotEvent'] == 'bet' || $postData['slotEvent'] == 'freespin' ) 
                        {
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('AlwaysHotDXGTBonusWin', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTFreeGames', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTTotalWin', 0);
                                $slotSettings->SetGameData('AlwaysHotDXGTFreeBalance', 0);
                            }
                            else
                            {
                                $slotSettings->SetGameData('AlwaysHotDXGTCurrentFreeGame', $slotSettings->GetGameData('AlwaysHotDXGTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym || in_array($s[0], $wild) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":1,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotDXGTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":["none","none"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":2,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotDXGTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":["none","none"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            $s[0] = $reels['reel1'][$linesId[$k][0] - 1];
                                            $s[1] = $reels['reel2'][$linesId[$k][1] - 1];
                                            ...
                                            if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('AlwaysHotDXGTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                        }
                                    }
                            ...
                            if( $postData['slotEvent'] == 'freespin' && $slotSettings->GetGameData('AlwaysHotDXGTFreeGames') <= $slotSettings->GetGameData('AlwaysHotDXGTCurrentFreeGame') && $winType != 'bonus' && $slotSettings->GetGameData('AlwaysHotDXGTTotalWin') + $totalWin > 0 ) 
                            {
                                $slotSettings->SetBalance($slotSettings->GetGameData('AlwaysHotDXGTTotalWin') + $totalWin);
                            }
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('AlwaysHotDXGTBonusWin', $slotSettings->GetGameData('AlwaysHotDXGTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('AlwaysHotDXGTTotalWin', $slotSettings->GetGameData('AlwaysHotDXGTTotalWin') + $totalWin);
                                $totalWin = $slotSettings->GetGameData('AlwaysHotDXGTBonusWin');
                                $Balance = $slotSettings->GetGameData('AlwaysHotDXGTFreeBalance');
                            }
                            else
                            {
                                $slotSettings->SetGameData('AlwaysHotDXGTTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('AlwaysHotDXGTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('AlwaysHotDXGTFreeBalance', $Balance);
                                    $slotSettings->SetGameData('AlwaysHotDXGTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('AlwaysHotDXGTFreeGames', $slotSettings->GetGameData('AlwaysHotDXGTFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('AlwaysHotDXGTFreeBalance', $Balance);
                                    $slotSettings->SetGameData('AlwaysHotDXGTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('AlwaysHotDXGTFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $jsSpin = '' . json_encode($reels) . '';
                            $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                            $winString = implode(',', $lineWins);
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $postData['slotLines'] . ',"slotBet":' . $postData['slotBet'] . ',"totalFreeGames":' . $slotSettings->GetGameData('AlwaysHotDXGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('AlwaysHotDXGTCurrentFreeGame') . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
