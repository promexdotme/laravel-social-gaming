                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,25,100],"multiplier":1},"1":{"coef":[5,25,100],"multiplier":1},"2":{"coef":[5,25,100],"multiplier":1},"3":{"coef":[5,40,150],"multiplier":1},"4":{"coef":[5,40,150],"multiplier":1},"5":{"coef":[5,30,100,750],"multiplier":1},"6":{"coef":[5,30,100,750],"multiplier":1},"7":{"coef":[5,40,400,2000],"multiplier":1},"8":{"coef":[10,100,1000,5000],"multiplier":1},"9":{"coef":[2,25,250],"multiplier":1},"-1":{"coef":[],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[8,1,4,0,5,2,6,1,5,7,3,3,1,4,2,9,5,6,8,2],[2,8,0,6,1,5,8,2,4,3,5,9,2,4,1,7,2,8,3,6],[4,0,5,1,8,3,6,4,8,1,9,3,2,5,1,7,0,3,6,2],[2,6,2,5,3,8,1,7,0,5,2,5,7,1,4,8,6,3,4,9],[6,1,7,3,5,0,5,4,8,1,5,9,2,4,8,7,5,0,3,4]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('MajesticForestEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('MajesticForestEGTBonusWin', 0);
                                $slotSettings->SetGameData('MajesticForestEGTFreeGames', 0);
                                $slotSettings->SetGameData('MajesticForestEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('MajesticForestEGTTotalWin', 0);
                                $slotSettings->SetGameData('MajesticForestEGTFreeBalance', 0);
                                $slotSettings->SetGameData('MajesticForestEGTExpandSymbol', -1);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData($slotSettings->slotId . 'ExpandSymbol', $lastEvent->serverResponse->freeSpinsExpandSymbol);
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('MajesticForestEGTCurrentFreeGame') < $slotSettings->GetGameData('MajesticForestEGTFreeGames') && $slotSettings->GetGameData('MajesticForestEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('MajesticForestEGTTotalWin', 0);
                                    $slotSettings->SetGameData('MajesticForestEGTExpandSymbol', -1);
                                    $slotSettings->SetGameData('MajesticForestEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', '');
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('MajesticForestEGTCurrentFreeGame', $slotSettings->GetGameData('MajesticForestEGTCurrentFreeGame') + 1);
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $postData['slotEvent'] == 'freespin' ) 
                                    {
                                        for( $r = 1; $r <= 5; $r++ ) 
                                        {
                                            for( $p = 0; $p <= 2; $p++ ) 
                                            {
                                                if( $reelsEx['reel' . $r][$p] == $slotSettings->GetGameData('MajesticForestEGTExpandSymbol') ) 
                                                {
                                                    $reelsEx['reel' . $r][0] = $slotSettings->GetGameData('MajesticForestEGTExpandSymbol');
                                                    $reelsEx['reel' . $r][1] = $slotSettings->GetGameData('MajesticForestEGTExpandSymbol');
                                                    $reelsEx['reel' . $r][2] = $slotSettings->GetGameData('MajesticForestEGTExpandSymbol');
                                                    break;
                                                }
                                            }
                                        }
                                    ...
                                    for( $k = 0; $k < $lines; $k++ ) 
                                    {
                                        ...
                                        for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                $s2[0] = $reelsEx['reel1'][$linesId[$k][0] - 1];
                                                $s2[1] = $reelsEx['reel2'][$linesId[$k][1] - 1];
                                                $s2[2] = $reelsEx['reel3'][$linesId[$k][2] - 1];
                                                $s2[3] = $reelsEx['reel4'][$linesId[$k][3] - 1];
                                                $s2[4] = $reelsEx['reel5'][$linesId[$k][4] - 1];
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"line":' . $k . ',"winAmount":' . ($cWins[$k] * 100) . ',"cells":[0,' . ($linesId[$k][0] - 1) . ',1,' . ($linesId[$k][1] - 1) . '],"card":' . $csym . '}';
                                                    }
                                                }
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"line":' . $k . ',"winAmount":' . ($cWins[$k] * 100) . ',"cells":[0,' . ($linesId[$k][0] - 1) . ',1,' . ($linesId[$k][1] - 1) . ',2,' . ($linesId[$k][2] - 1) . '],"card":' . $csym . '}';
                                                    }
                                                }
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"line":' . $k . ',"winAmount":' . ($cWins[$k] * 100) . ',"cells":[0,' . ($linesId[$k][0] - 1) . ',1,' . ($linesId[$k][1] - 1) . ',2,' . ($linesId[$k][2] - 1) . ',3,' . ($linesId[$k][3] - 1) . '],"card":' . $csym . '}';
                                                    }
                                                }
                                                if( ($s[0] == $csym || in_array($s[0], $wild)) && ($s[1] == $csym || in_array($s[1], $wild)) && ($s[2] == $csym || in_array($s[2], $wild)) && ($s[3] == $csym || in_array($s[3], $wild)) && ($s[4] == $csym || in_array($s[4], $wild)) ) 
                                                {
                                                    ...
                                                    {
                                                        $cWins[$k] = $tmpWin;
                                                        $tmpStringWin = '{"line":' . $k . ',"winAmount":' . ($cWins[$k] * 100) . ',"cells":[0,' . ($linesId[$k][0] - 1) . ',1,' . ($linesId[$k][1] - 1) . ',2,' . ($linesId[$k][2] - 1) . ',3,' . ($linesId[$k][3] - 1) . ',4,' . ($linesId[$k][4] - 1) . '],"card":' . $csym . '}';
                                                    }
                                                }
                                                if( $postData['slotEvent'] == 'freespin' && $csym == $slotSettings->GetGameData('MajesticForestEGTExpandSymbol') ) 
                                                {
                                                    $cLineExp = [];
                                                    $exCount = 0;
                                                    for( $ex = 0; $ex < 5; $ex++ ) 
                                                    {
                                                        ...
                                                        {
                                                            $exCount++;
                                                            $cLineExp[] = $ex . ',' . ($linesId[$k][$ex] - 1);
                                                        }
                                                    }
                                                    $tmpWin2 = $slotSettings->Paytable['SYM_' . $csym][$exCount] * $betline * $bonusMpl;
                                                    if( $cWins2[$k] < $tmpWin2 ) 
                                                    {
                                                        $cWins2[$k] = $tmpWin2;
                                                        $tmpStringWin2 = '{"line":' . $k . ',"winAmount":' . ($cWins2[$k] * 100) . ',"cells":[' . implode(',', $cLineExp) . '],"card":' . $csym . '}';
                                                    }
                                                }
                                            }
                                        }
                                        ...
                                        {
                                            array_push($lineWins2, $tmpStringWin2);
                                            $expandWin += $cWins2[$k];
                                            $totalWin += $cWins2[$k];
                                        }
                                    }
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('MajesticForestEGTBonusWin', $slotSettings->GetGameData('MajesticForestEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('MajesticForestEGTTotalWin', $slotSettings->GetGameData('MajesticForestEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('MajesticForestEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('MajesticForestEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                $firstSpinLog = '';
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                ...
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('MajesticForestEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('MajesticForestEGTFreeGames', $slotSettings->GetGameData('MajesticForestEGTFreeGames') + $slotSettings->slotFreeCount);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('MajesticForestEGTExpandSymbol', rand(0, 8));
                                        $slotSettings->SetGameData('MajesticForestEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('MajesticForestEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('MajesticForestEGTFreeGames', $slotSettings->slotFreeCount);
                                        $slotSettings->SetGameData($slotSettings->slotId . 'FirstSpin', ',"firstSpin":{"expand":[],"reels":[' . $curReels . '],"lines":[],"expand":[],"scatters":[' . $scattersStr . ']}');
                                    }
                                    $fs = $slotSettings->GetGameData('MajesticForestEGTFreeGames');
                                }
                                $winString = implode(',', $lineWins);
                                $winStringExpand = implode(',', $lineWins2);
                                $jsSpin = '' . json_encode($reels) . '';
                                $jsJack = '' . json_encode($slotSettings->Jackpots) . '';
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('MajesticForestEGTFreeGames') . ',"freeSpinsExpandLines": [' . $winStringExpand . '],"freeSpinsExpandSymbol":' . $slotSettings->GetGameData('MajesticForestEGTExpandSymbol') . ',"freeSpinsExpandWinAmount": ' . $expandWin . ',"currentFreeGames":' . $slotSettings->GetGameData('MajesticForestEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('MajesticForestEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('MajesticForestEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('MajesticForestEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('MajesticForestEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('MajesticForestEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                $result_tmp[] = '{"complex":{"gambles":5,"reels":[' . $curReels . '],"lines":[' . $winString . '],"scatters":[' . $scattersStr . '],"freespinScatters":[' . $scatter . '],"expand":[],"freespinScatters":[' . $scatter . '],"freespins":' . $fs . ',"jackpot":' . $isJack . ',"freespinScatters":[9],"freeSpinsExpandSymbol":' . $slotSettings->GetGameData('MajesticForestEGTExpandSymbol') . ',"freeSpinsExpandLines":[' . $winStringExpand . '],"freeSpinsExpandWinAmount":' . $expandWin . ',"gameCommand":"bet"},"state":"' . $state . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":1786348900601,"balance":' . $balanceInCents . ',"sessionKey":"894b17d797618b2d99d0c94df1d88e72","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('MajesticForestEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('MajesticForestEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('MajesticForestEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('MajesticForestEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('MajesticForestEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('MajesticForestEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('MajesticForestEGTGambleStep', $slotSettings->GetGameData('MajesticForestEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('MajesticForestEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
