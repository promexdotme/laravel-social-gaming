                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"paytableCoef":{"0":{"coef":[5,20,100],"multiplier":1},"1":{"coef":[5,20,100],"multiplier":1},"2":{"coef":[5,20,100],"multiplier":1},"3":{"coef":[5,20,100],"multiplier":1},"4":{"coef":[15,35,150],"multiplier":1},"5":{"coef":[15,35,150],"multiplier":1},"6":{"coef":[20,50,200],"multiplier":1},"7":{"coef":[20,50,200],"multiplier":1},"8":{"coef":[25,50,500],"multiplier":1},"9":{"coef":[25,50,500],"multiplier":1},"10":{"coef":[50,200,1000],"multiplier":1},"11":{"coef":[2],"multiplier":1}},"bets":[' . implode(',', $gameBets) . '],"jackpotMinBet":1,"jackpot":true,"mainFakeReels":[[10,2,5,6,3,9,5,7,2,0,9,2,6,1,7,4,8,1,0,3],[10,3,1,8,4,3,9,2,4,8,7,4,11,3,0,6,4,3,5,9],[5,9,2,4,8,5,2,3,4,6,0,11,5,7,3,8,1,10,0,2],[7,0,11,1,5,9,0,8,3,2,10,1,6,11,4,2,10,1,0,3],[3,8,12,6,1,4,7,3,8,10,5,3,2,0,1,3,9,0,4,7]],"jackpotMaxBet":100000,"denominations":[' . implode(',', $denoms) . ']},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":' . time() . '}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('HalloweenEGTCards', $hist);
                                $lastEvent = $slotSettings->GetHistory();
                                $slotSettings->SetGameData('HalloweenEGTBonusWin', 0);
                                $slotSettings->SetGameData('HalloweenEGTFreeGames', 0);
                                $slotSettings->SetGameData('HalloweenEGTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('HalloweenEGTTotalWin', 0);
                                $slotSettings->SetGameData('HalloweenEGTFreeBalance', 0);
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $gtype = 1;
                                    if( $slotSettings->GetGameData('HalloweenEGTCurrentFreeGame') < $slotSettings->GetGameData('HalloweenEGTFreeGames') && $slotSettings->GetGameData('HalloweenEGTFreeGames') > 0 ) 
                                    {
                                        $gtype = 2;
                                    }
                                }
                            ...
                            case 'bet':
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('HalloweenEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('HalloweenEGTTotalWin', 0);
                                    $slotSettings->SetGameData('HalloweenEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('HalloweenEGTCurrentFreeGame', $slotSettings->GetGameData('HalloweenEGTCurrentFreeGame') + 1);
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $isWild ) 
                                    {
                                        for( $r = 1; $r <= 5; $r++ ) 
                                        {
                                            for( $p = 0; $p <= 2; $p++ ) 
                                            {
                                                if( $reels['reel' . $r][$p] == '0' || $reels['reel' . $r][$p] == '1' || $reels['reel' . $r][$p] == '2' || $reels['reel' . $r][$p] == '3' ) 
                                                {
                                                    $reels['reel' . $r][$p] = '12';
                                                }
                                            }
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $slotSettings->SetGameData('HalloweenEGTBonusWin', $slotSettings->GetGameData('HalloweenEGTBonusWin') + $totalWin);
                                    $slotSettings->SetGameData('HalloweenEGTTotalWin', $slotSettings->GetGameData('HalloweenEGTTotalWin') + $totalWin);
                                    $balanceInCents = $slotSettings->GetGameData('HalloweenEGTFreeBalance');
                                }
                                else
                                {
                                    $slotSettings->SetGameData('HalloweenEGTTotalWin', $totalWin);
                                }
                                $fs = 0;
                                if( $scattersCount >= 3 ) 
                                {
                                    if( $slotSettings->GetGameData('HalloweenEGTFreeGames') > 0 ) 
                                    {
                                        $slotSettings->SetGameData('HalloweenEGTFreeGames', $slotSettings->GetGameData('HalloweenEGTFreeGames') + 10);
                                    }
                                    else
                                    {
                                        $slotSettings->SetGameData('HalloweenEGTFreeStartWin', $totalWin);
                                        $slotSettings->SetGameData('HalloweenEGTBonusWin', $totalWin);
                                        $slotSettings->SetGameData('HalloweenEGTFreeGames', 10);
                                    }
                                    $fs = 10;
                                }
                                $curReels = '' . rand(0, 6) . ',' . $reels['reel1'][0] . ',' . $reels['reel1'][1] . ',' . $reels['reel1'][2] . ',' . rand(0, 6);
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"slotLines":' . $lines . '' . $slotSettings->GetGameData($slotSettings->slotId . 'FirstSpin') . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('HalloweenEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('HalloweenEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('HalloweenEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
                                $slotSettings->SaveLogReport($response, $allbet, $lines, $reportWin, $postData['slotEvent']);
                                $winstring = '';
                                $slotSettings->SetGameData('HalloweenEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('HalloweenEGTCards');
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('HalloweenEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                if( $postData['slotEvent'] == 'freespin' ) 
                                {
                                    $totalWin = $slotSettings->GetGameData('HalloweenEGTBonusWin');
                                    $state = 'freespin';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('HalloweenEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('HalloweenEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('HalloweenEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('HalloweenEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('HalloweenEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('HalloweenEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('HalloweenEGTGambleStep', $slotSettings->GetGameData('HalloweenEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('HalloweenEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('HalloweenEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambles":' . $slotSettings->GetGameData('HalloweenEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
