                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('ArcherPTBonusWin', 0);
                                $slotSettings->SetGameData('ArcherPTFreeGames', 0);
                                $slotSettings->SetGameData('ArcherPTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('ArcherPTTotalWin', 0);
                                $slotSettings->SetGameData('ArcherPTFreeBalance', 0);
                                $slotSettings->SetGameData('ArcherPTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('ArcherPTCurrentFreeGame', $slotSettings->GetGameData('ArcherPTCurrentFreeGame') + 1);
                                $tmpMpl = [
                                    2, 
                                    3, 
                                    5
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('ArcherPTBonusWin', $slotSettings->GetGameData('ArcherPTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('ArcherPTTotalWin', $totalWin);
                                $sID = 40022;
                            }
                            else
                            {
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('ArcherPTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('ArcherPTFreeGames', $slotSettings->GetGameData('ArcherPTFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('ArcherPTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('ArcherPTBonusWin', 0);
                                    $slotSettings->SetGameData('ArcherPTFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"BetMultiplier":' . $postData['numCoins'] . ',"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('ArcherPTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('ArcherPTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('ArcherPTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('ArcherPTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
