                    ...
                    try
                    {
                        ...
                        if( isset($postData['spinType']) ) 
                        {
                            ...
                            if( $postData['spinType'] == 'regular' ) 
                            {
                                ...
                                $bonusMpl = 1;
                                $slotSettings->SetGameData('BermudaTrianglePTBonusWin', 0);
                                $slotSettings->SetGameData('BermudaTrianglePTFreeGames', 0);
                                $slotSettings->SetGameData('BermudaTrianglePTCurrentFreeGame', 0);
                                $slotSettings->SetGameData('BermudaTrianglePTTotalWin', 0);
                                $slotSettings->SetGameData('BermudaTrianglePTFreeBalance', 0);
                                $slotSettings->SetGameData('BermudaTrianglePTFreeStartWin', 0);
                            }
                            else if( $postData['spinType'] == 'free' ) 
                            {
                                $umid = '0';
                                $postData['slotEvent'] = 'freespin';
                                $slotSettings->SetGameData('BermudaTrianglePTCurrentFreeGame', $slotSettings->GetGameData('BermudaTrianglePTCurrentFreeGame') + 1);
                                    ...
                                    for( $j = 0; $j < count($slotSettings->SymbolGame); $j++ ) 
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if( $s[0] == $csym && $s[1] == $csym && $s[2] == $csym ) 
                                            {
                                                ...
                                                {
                                                    $cWins[$k] = $tmpWin;
                                                    $tmpStringWin = '{"Count":3,"Line":' . $k . ',"Win":' . $cWins[$k] . ',"stepWin":' . ($cWins[$k] + $totalWin + $slotSettings->GetGameData('BermudaTrianglePTBonusWin')) . ',"winReel1":[' . ($linesId[$k][0] - 1) . ',"' . $s[0] . '"],"winReel2":[' . ($linesId[$k][1] - 1) . ',"' . $s[1] . '"],"winReel3":[' . ($linesId[$k][2] - 1) . ',"' . $s[2] . '"],"winReel4":["none","none"],"winReel5":["none","none"]}';
                                                }
                                            }
                                            if( in_array($s[0], [
                                                4, 
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('BermudaTrianglePTBonusWin', $slotSettings->GetGameData('BermudaTrianglePTBonusWin') + $totalWin);
                                $slotSettings->SetGameData('BermudaTrianglePTTotalWin', $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('BermudaTrianglePTTotalWin', $totalWin);
                            ...
                            if( $scattersCount >= 3 ) 
                            {
                                if( $slotSettings->GetGameData('BermudaTrianglePTFreeGames') > 0 ) 
                                {
                                    $slotSettings->SetGameData('BermudaTrianglePTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('BermudaTrianglePTFreeGames', $slotSettings->GetGameData('BermudaTrianglePTFreeGames') + $slotSettings->slotFreeCount);
                                }
                                else
                                {
                                    $slotSettings->SetGameData('BermudaTrianglePTFreeStartWin', $totalWin);
                                    $slotSettings->SetGameData('BermudaTrianglePTBonusWin', $totalWin);
                                    $slotSettings->SetGameData('BermudaTrianglePTFreeGames', $slotSettings->slotFreeCount);
                                }
                            }
                            $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance()) * 100;
                            ...
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"linesArr":[' . implode(',', $postData['lines']) . '],"slotLines":' . $lines . ',"slotBet":' . $betLine . ',"totalFreeGames":' . $slotSettings->GetGameData('BermudaTrianglePTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BermudaTrianglePTCurrentFreeGame') . ',"Balance":' . $slotSettings->GetBalance() . ',"afterBalance":' . $slotSettings->GetBalance() . ',"bonusWin":' . $slotSettings->GetGameData('BermudaTrianglePTBonusWin') . ',"freeStartWin":' . $slotSettings->GetGameData('BermudaTrianglePTFreeStartWin') . ',"totalWin":' . $totalWin . ',"winLines":[' . $winString . '],"bonusInfo":' . $scattersStr . ',"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
