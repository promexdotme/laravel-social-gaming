                    ...
                    try
                    {
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'login':
                                ...
                                $slotSettings->SetGameData($slotSettings->slotId . 'GunId', 1);
                                $slotSettings->SetGameData($slotSettings->slotId . 'CurrentBet', 1);
                                $balanceInCents = sprintf('%01.2f', $slotSettings->GetBalance());
                            ...
                            case 'changbackstage':
                                $CurrentBet = $slotSettings->GetGameData($slotSettings->slotId . 'CurrentBet');
                                $CurrentSceneTime = $slotSettings->GetGameData($slotSettings->slotId . 'CurrentSceneTime');
                                $CurrentScene = $slotSettings->GetGameData($slotSettings->slotId . 'CurrentScene');
                                $GunId = $slotSettings->GetGameData($slotSettings->slotId . 'GunId');
                                $balanceInCents = round(sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $result_tmp[] = '{"message":{"type":3,"player":{"uid":7401,"ws":0,"pos":5,"gunid":' . $GunId . ',"gunnum":0,"reward_rate":' . $CurrentBet . ',"score":' . $balanceInCents . ',"isvistor":0}},"succ":true,"errinfo":"ok","type":"broadplayer"}';
                                if( $CurrentSceneTime < $curTime ) 
                                {
                                    $CurrentScene++;
                                    if( $CurrentScene > 4 ) 
                                    {
                                    ...
                                    $CurrentSceneTime = $curTime + 5000;
                                    $slotSettings->SetGameData($slotSettings->slotId . 'CurrentScene', $CurrentScene);
                                    $slotSettings->SetGameData($slotSettings->slotId . 'CurrentSceneTime', $CurrentSceneTime);
                                }
                                $result_tmp[1] = '{"message":{"sceneid":' . $CurrentScene . ',"btime":' . ($curTime + 5000) . ',"etime":' . $curTime . '},"succ":true,"errinfo":"ok","type":"changescene"}';
                                break;
                            case 'fire':
                                $CurrentBet = $slotSettings->GetGameData($slotSettings->slotId . 'CurrentBet');
                                $GunId = $slotSettings->GetGameData($slotSettings->slotId . 'GunId');
                                $balanceInCents = round(sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $result_tmp[0] = '{"message":{"type":3,"player":{"uid":7401,"ws":0,"pos":5,"gunid":' . $GunId . ',"gunnum":0,"reward_rate":' . $CurrentBet . ',"score":' . $balanceInCents . ',"isvistor":0}},"succ":true,"errinfo":"ok","type":"broadplayer"}';
                                break;
                            case 'changegun':
                                $GunId = $slotSettings->GetGameData($slotSettings->slotId . 'GunId');
                                $CurrentBet = $slotSettings->GetGameData($slotSettings->slotId . 'CurrentBet');
                            ...
                            case 'hit':
                                ...
                                $balanceInCents = round(sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $GunId = $slotSettings->GetGameData($slotSettings->slotId . 'GunId');
                                $result_tmp[] = '{"message":{"money":0},"succ":true,"errinfo":"ok","type":"userinfo"}';
                                $result_tmp[] = '{"message":{"type":3,"player":{"uid":7401,"ws":0,"pos":5,"gunid":' . $GunId . ',"gunnum":0,"reward_rate":' . $CurrentBet . ',"score":' . $balanceInCents . ',"isvistor":0}},"succ":true,"errinfo":"ok","type":"broadplayer"}';
                            ...
                            case 'quickenterroom':
                                ...
                                {
                                    $rfish = rand(1, 12);
                                    $fishes[] = '{"id":' . rand(1, 9999999) . ',"classid":' . $rfish . ',"fishid":' . $rfish . ',"born_time":' . $curTime . ',"routeid":' . rand(1, 110) . ',"dead_time":' . ($curTime + 60000) . ',"offsettype":0,"offsetx":0,"offsety":0,"offsetr":0,"rate":2,"ext":0}';
                                }
                                $CurrentBet = $slotSettings->GetGameData($slotSettings->slotId . 'CurrentBet');
                                $slotSettings->SetGameData($slotSettings->slotId . 'CurrentScene', 1);
                                $slotSettings->SetGameData($slotSettings->slotId . 'CurrentSceneTime', $curTime);
                                $GunId = $slotSettings->GetGameData($slotSettings->slotId . 'GunId');
                                $balanceInCents = round(sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $result_tmp[0] = '{"message":{"result":1,"roompos":1,"scenestate":2,"sceneid":1,"scene_etime":' . $curTime . ',"scene_btime":' . ($curTime + 3000000) . ',"players":[{"uid":7401,"ws":0,"pos":5,"gunid":' . $GunId . ',"gunnum":0,"reward_rate":' . $CurrentBet . ',"score":' . $balanceInCents . ',"isvistor":false}],"sprites":[' . implode(',', $fishes) . '],"bullets":[],"min":1,"max":200,"coinrate":1000,"bombs":[]},"succ":true,"errinfo":"ok","type":"enterroom"}';
                                break;
                        }
